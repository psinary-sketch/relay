...
# RESOLUTION ADDENDUM — 2026-07-12 (author rulings enacted)

The author ruled on this review the same day. All five rulings are enacted; PLACE-papers `45c404e` (+ `7fe032d`, a hygiene follow-up).

**(1) v2.1 written as the honest-boundary paper** (`phase1.5/method/EXCLUSION_ENGINE.md`, v2.0 → v2.1). The three-grade vocabulary is instituted throughout and defined in a new §0: **DERIVES** (kernel proves the content from structure) / **INTERFACES** (conditional on a *named* manuscript premise) / **ENCODES-CONCLUSION or SHELL** (work-order, never a citation). Section by section: §I withdraws the compiled-Determination claim and regrades `Layer1.SIDE_exclusion` as a proven schema with no live catalogue; §II regrades the four reframings as **nomenclature** — the kernels name the shape and assert the answer; §III withdraws the substance/vacuity distinction *as a kernel fact* while retaining it as the methodological thesis, and states plainly that v2.0's spine sentence is false and why; §IV demotes the E-Difficulty dichotomy from "the rigorous spine" to a conjecture with a partially formalized κ-machinery (κ itself **DERIVES** — it is genuinely computed — but `h_bimodality` *is* the dichotomy, and `kvec` is never linked to `kappa`); §V is untouched and the paper says why (it never rested on a kernel citation); §VI marks the undecidability theory as conjecture and rewrites the self-similarity paragraph around the salt-check as its enforcement; §VII corrects the census to **10 real sorry** and withdraws "clean health," naming the blind instrument. The Misreadings appendix is **kept**, its first guardrail inverted (*do not cite `determination_bridge` as the proof of Determination*), and a fifth added: **zero `sorry` is hygiene, not evidence.**

**The salt-check table is the paper's exhibit** (§VIII), carrying all eighteen audited terminals with grade, source quote and work-order, plus the gate's own definition — including the rule that `#print axioms` is run **only** on terminals that pass (a)–(c), since profiling a tautology launders the failure.

**Provenance states the intent finding** as ruled: *the vacuous application kernels were scaffolding that the papers over-read*, evidenced by the `[M1 retired]` cleanup — the source layer had found the problem and marked it in-source ("said nothing about the zeros"; "is NOT proved here"); the paper layer had not caught up. It is recorded as a documentation failure, not a fabrication — and corrected completely.

**(2) E1 and E5 landed.** REGISTRY gains a new append-only **deposit-gates block** with two gates: **Patent-filing verification** (P-SIDE/P-FORM/P-DARK/P-CLOS confirmed filed before any phase-wave deposit, all results and potential patents accounted for) and the **E0 salt-check** (no terminal cited in a deposited paper without passing it; failures are work-order rows). The IP sentence stays present tense in §VI with one clause noting the gate. E5's glosses are in place: Π-shaped/Σ-shaped (§V), bright access / bright bridge (§IV, at first use), domain Ostrowski (§IV, with the Ostrowski-classification gloss), Ш → Tate–Shafarevich (appendix).

**(3) Work-orders filed** as the E0 table's work-order column (W-1…W-6) and as **OPEN_TRAILS O.16** (append-only): W-1 Determination (real hypothesis + the ξ forward-catalogue `Formation.lean` names and never builds); W-2 real predicates for `MismatchMechanism` / `Massless` / `LSZero`; W-3 bimodality thresholds derived + `kvec ↔ kappa` lemma + the long-deferred `dark_iff_invariant`; W-4 `to_modular` discharge **and** correction of the wrong-but-compiling `shifted` case, plus putting `Module1.lean` in the build target so CI can see its seven sorries; W-5 BSD `Bool`-field conclusions replaced by derivations or the kernels re-filed as scaffolding; W-6 sieve-ceiling terminals restated to bind their subject matter, or retired.

**(4) FINDINGS `F.2026-07-12`** appended, as titled — salt-check instituted; keystone corrected to honest-boundary form; wave-wide Correspondence re-grade and core-terminal statement-read queued.

**(5) Hygiene disclosure.** Commit `45c404e` accidentally swept in two previously-untracked `outputs/` deposit-line snapshots (a `git add -A` overreach on my part). `7fe032d` untracks them; the files remain on disk, untracked as before. If the deposited-line snapshots *should* be tracked, that is a deliberate decision to make separately.

**Scope, restated because it will be asked:** the RH core is untouched by all of this. Routes 1–3 stand at `{propext, Classical.choice, Quot.sound}` (audit of 2026-07-10). What was corrected is the method layer that claimed to justify the architecture and the application kernels that claimed to extend it.

**Loom entry landed (2026-07-12):** PLACE-papers `e07dfd1` — `VERIFICATION_LOOM.md` now records the salt-check as an instance of its own ⚠ rule at federation scale (shell findings, SIDE-interfaces as the exception, the honest-boundary v2.1 resolution, the three-grade rubric DERIVES / INTERFACES / ENCODES-CONCLUSION-or-SHELL), and files the two next-wave gates: wave-wide Correspondence re-grade, and the core-terminal statement-read (the RH core passed (d) in July but has never had (a) run on it).
