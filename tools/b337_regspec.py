# -*- coding: utf-8 -*-
"""b337_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b337_registration_2026-09-06.txt')
SPEC = os.path.join(ROOT, 'data', 'b337_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ("PLACE-papers files written", 1, 1, "files", "### section (H): ERRATA.md, one appended block."),
    ("ERRATA entries moved or edited", 0, 0, "entries", "### the ruling: entries unmoved."),
    ("TECHNE files committed, local", 9, 9, "files", "### the ruling: the nine August module files, by explicit list."),
    ("TECHNE pushes", 0, 0, "pushes", "### the ruling: not pushed."),
    ("deposit actions or Zenodo writes", 0, 0, "actions", "### section (A): a read-only fetch only; must-fail fixture."),
    ("correspondence rows appended", 1, 1, "rows", "### section (H)."),
    ("ledger rows written (faces)", 0, 0, "rows", "none."),
    ("findings sections edited", 0, 0, "sections", "none."),
    ("grades moved or conferred", 0, 0, "grades", "section (A)."),
    ...
    ("patent-package files written", 0, 0, "files", "### section (H): read only."),
...
def main(argv):
    print('=' * 100)
    print('b337_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": "data/b337_registration_2026-09-06.txt -- b337, THE WAVE'S HOUSEKEEPING", "clauses": clauses}
