# -*- coding: utf-8 -*-
"""b344_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b344_registration_2026-09-06.txt')
SPEC = os.path.join(ROOT, 'data', 'b344_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ("owner instrument files edited", 1, 1, "files", "### section (D): `tools/reg_seal.py`, BY THE ORDER'S WORDS, quoted there."),
    ("TECHNE files written", 2, 2, "files", "### section (D): one module; the index, by an appended block."),
    ("TECHNE files edited", 0, 0, "files", "### section (D): the untouched check."),
    ("TECHNE pushes", 0, 0, "pushes", "### PRIVATE; the remote read before and after."),
    ("PLACE-papers files written", 0, 0, "files", "none."),
    ("correspondence rows appended", 1, 1, "rows", "### section (H)."),
    ("faces ledger rows or blocks written", 0, 0, "blocks", "none; no row's status moves."),
    ("ERRATA entries written", 0, 0, "entries", "none."),
    ("existing seals rewritten", 0, 0, "seals", "### section (D): verify unchanged, none rewritten."),
    ("axes moved", 1, 1, "axes", "### section (C): NY; tau and the taper printed, not moved."),
    ("rungs or heights added after a value", 0, 0, "points", "### sections (C) and (E): fixed before any value."),
    ("bars moved", 0, 0, "bars", "none; must-fail fixture."),
    ...
    ("conclusions about the axes held", 0, 0, "conclusions", "### section (C): nothing concluded about them."),
...
def main(argv):
    print('=' * 100)
    print('b344_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": "data/b344_registration_2026-09-06.txt -- b344, THE FLOOR PRICED, THE SEAL'S OWN CLOCK, AND THE ROOM'S EDGE", "clauses": clauses}
