# -*- coding: utf-8 -*-
"""b518_report.py -- COMPONENTS 1-3 FROM THE BANKED CELLS. ### `python tools/b518_report.py`
### Every count recomputed from `b518_cells.jsonl`; the scoring is READING (9)'s; the reading is (R127)(2)'s words, with (d)'s
### growth factor exp(2 delta L) banked beside the narrowest negative width when there is one, and the pair's ratio to the rest
### at the widest cell printed when there is none.
"""
import io
import json
import os
...
def main():
    cells = sorted((json.loads(l) for l in io.open(os.path.join(D, 'b518_cells.jsonl'), encoding='utf-8') if l.strip()),
                   key=lambda c: c['a'])
    upper = set(round(c['a'], 9) for c in cells[UPPER:])
    res = {}
    L = ['=' * 132, 'b518 -- THE TWO-PROPERTY WINDOW: Q0 AT 16.290216 AND XI AT 14.1347 ACROSS THE LADDER.', '=' * 132]
    ...
    q = res['q']
    widest = cells[-1]
    q['widest_a'], q['widest_ratio'] = widest['a'], widest['q']['ratio']
    q['h2_neg_half'] = len(q['h2_neg']) * 2 >= q['verified'] and q['verified'] > 0
    if q['h2_neg']:
        reading = ('a Q0 cell is negative beyond its bound on a closing bank (narrowest a = %s, exp(2 delta L) = %.6g there, (f)`s numerical target): %s'
                   % (q['narrowest'], q['growth_at_narrowest'], WIT))
    else:
        reading = HARD + '; the ratio of the pair`s term to the rest at the widest cell (a = %.6f) is %+.6e' % (widest['a'], widest['q']['ratio'])
    res['reading'] = reading
    L += ['', '### COMPONENT 3 -- THE READING, IN (R127)(2)`S WORDS:', '    ' + reading,
          '    ### Nothing is claimed of the kernel either way.', '=' * 132]
    io.open(os.path.join(D, 'b518_report.txt'), 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    io.open(os.path.join(D, 'b518_results.json'), 'w', encoding='utf-8', newline=NL).write(json.dumps(res, indent=1) + NL)
    print(NL.join(L[-16:]))
    return 0
