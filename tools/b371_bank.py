...
w('### ### **(iii-b) AND THE EOL HAZARD THE TRACKED GUARD CREATES, NAMED AND NOT FIXED.** ### The guard')
w('### is now a TRACKED TEXT FILE, so git rewrites its line endings on checkout: the blob is LF and this')
w('### platform checks it out as CRLF. ### On a clone that does the')
w('### same, ### **`#!/bin/sh` CARRIES A CARRIAGE RETURN AND THE GUARD MAY NOT RUN AT ALL.** ### No')
w('### `.gitattributes` pins it. ### **THE FIX IS ONE LINE PER REPOSITORY AND IS OUTSIDE THIS ACT`S')
w('### ### DECLARED CAPS**, so it is reported and routed rather than half-applied to the repositories')
w('### whose caps happen to allow it -- ### **AN INCONSISTENTLY GUARDED FEDERATION IS WORSE THAN A')
w('### ### UNIFORMLY UNGUARDED ONE.** ### And the identity arm is now EOL-normalised, so ### **THAT ARM')
w('### ### WILL NOT TELL THE NEXT ACT EITHER**, which is why it is written here.')
...
w('### ### ### **(v) THE HEREDOC TRAP, IN ITS SHARPEST FORM YET: IT SUBSTITUTED A CONTROL')
w('### ### ### CHARACTER RATHER THAN DROPPING A BACKSLASH.** ### An arm written through a shell')
w('### heredoc asked for a word boundary; what reached the file was a literal ### **BACKSPACE BYTE**,')
w('### so the regex matched nothing and ### **THE LINE LOOKED CORRECT UNDER `grep`.** ### The arm')
w('### failed, was reproduced correctly in isolation, and took four passes to find -- ### **BECAUSE THE')
w('### ### EYE CANNOT SEE THE DEFECT AND THE ISOLATION TEST DID NOT CARRY IT.** ### Found by dumping')
w('### the bytes.')
w('### ### **THE RECORD ALREADY HELD THIS TRAP AS `A HEREDOC COLLAPSES A BACKSLASH`.** ### That')
w('### statement is too narrow: ### **IT ALSO SUBSTITUTES**, and a substitution is worse than a')
w('### deletion because a deletion usually breaks something visibly. ### The cure is unchanged and was')
w('### ignored again in this act: ### **WRITE THE FILE WITH THE EDITOR, NOT THROUGH A HEREDOC.**')
w('### ### **(vi) AND THIS ACT`S OWN REPAIR CREATED A STALE SURFACE.** ### Moving')
