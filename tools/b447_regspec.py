# -*- coding: utf-8 -*-
"""b447_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b447_registration_2026-09-12.txt')
SPEC = os.path.join(ROOT, 'data', 'b447_satisfiable.json')
...
CLAUSES = [
    ...
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### (P1), (P7): the outlier`s levels and the span are declared.'),
    ('author rulings ratified and entered', 1, 1, 'rulings', '### section (A): (R60).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b447_addendum.txt.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (K) BAR 10.'),
    ('lanes opened', 1, 1, 'lanes', '### section (A): the instrument lane under (R60).'),
    ...
    ('cells the third doubling runs at', 1, 1, 'cells', '### reading (1).'),
    ('loom blocks appended', 1, 1, 'blocks', '### reading (2).'),
    ('lines appended to the bar-floor rule', 1, 1, 'lines', '### reading (2).'),
    ('closings edited', 0, 0, 'closings', '### (K) BAR 5.'),
    ('items proposed', 0, 0, 'proposals', '### (K) BAR 6.'),
    ('declared closure pairs', 5, 5, 'pairs', '### reading (3).'),
    ('errors entered as their owners`', 1, 1, 'errors', '### section (A): the navigator`s wording.'),
    ('chain files edited', 0, 0, 'files', '### (K) BAR 7.'),
    ('new cells or radii', 0, 0, 'cells', '### (R60).'),
    ('factors or tolerances stated after a value', 0, 0, 'rules', '### (K) BAR 2.'),
    ('verdicts forced into a named outcome', 0, 0, 'verdicts', '### (K) BAR 5.'),
    ('candidates run rather than priced', 0, 0, 'candidates', '### (K) BAR 3.'),
...
def main(argv):
    print('=' * 100)
    print('b447_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b447_registration_2026-09-12.txt -- b447, "
                             "THE OUTLIER, THE ARM THAT BITES, AND THE STOCK-TAKE"),
            "clauses": clauses}
