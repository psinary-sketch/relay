# -*- coding: utf-8 -*-
"""b474_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b474_registration_2026-09-22.txt')
SPEC = os.path.join(ROOT, 'data', 'b474_satisfiable.json')
...
CLAUSES = [
    ...
    ('kernel builds', 0, 0, 'builds', '### section (Z): (R80)`s run is over; this act starts none.'),
    ('chains run, or channel values computed', 0, 0, 'runs', '### section (Z): b321_window is NOT executed.'),
    ('lean runs', 0, 0, 'runs', '### section (Z): the log is READ, not re-run.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b474_addendum.txt.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('expectations registered', 0, 0, 'expectations', '### section (J): the order registers none.'),
    ...
    ('PLACE-papers documents written at all', 3, 3, 'documents', '### (W): FINDINGS.md, the digest and OPEN_TRAILS.md, each appended only.'),
...
def main(argv):
    print('=' * 100)
    print('b474_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b474_registration_2026-09-22.txt -- b458, "
