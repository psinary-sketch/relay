# -*- coding: utf-8 -*-
"""b500_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b500_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b500_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 0, 0, 'files', '### (W): the probe `b500_printaxioms.lean` is written by the launched process into relay`s data directory, which is not a repository of Lean, and is not committed; no `.lean` file of any kernel is written.'),
    ...
    ('lanes opened', 0, 0, 'lanes', '### (Z): none; (R112) opens the numerical lane at b501.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b500.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b500_extract.txt`.'),
    ('readings taken silently', 0, 0, 'readings', '### section (A).'),
    ('survey anchor misses', 0, 0, 'misses', '### section (A): 0.'),
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### section (C): Component 0 ran at step zero and is declared there; the log`s size, line count and time were taken, and NO LINE OF IT WAS OPENED.'),
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R112) -- the ladder`s tail to the head of (R104); the numerical lane open for b501 and b502 only; b500 outside the sequence. ### RE-POINTED FROM b498`s (R109).'),
    ('author rulings recorded as entries by this act', 0, 0, 'rulings', '### (Z): (R112) is ratified in the ferry and entered by the trail record.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b500_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written at all', 2, 2, 'documents', '### (W): OPEN_TRAILS.md (one appended record) ALWAYS, and REGISTRY.md (one appended note) ONLY IF (R82) HOLDS. The cap is the maximum the face permits.'),
...
def main(argv):
    print('=' * 100)
    print('b500_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b500_extract.txt'),
    ...
    spec = {"registration": ("data/b500_registration_2026-09-23.txt -- b500, "
                             "THE b498 BUILD LOG READ, AND (R82) DECIDED"),
            "clauses": clauses}
