# -*- coding: utf-8 -*-
"""b448_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b448_registration_2026-09-13.txt')
SPEC = os.path.join(ROOT, 'data', 'b448_satisfiable.json')
...
CLAUSES = [
    ...
    ('bridges typed between the partition and the arc', 0, 0, 'bridges', '### (K) BAR 6.'),
    ('partitions constructed or classes proved silent', 0, 0, 'partitions', '### section (Z).'),
    ...
    ('TECHNE-Core writes', 1, 1, 'writes', '### section (W): one line appended to BAR_FLOOR_RULE.md, local, not pushed.'),
    ('rule corrections filed by the author`s order', 1, 1, 'corrections', '### reading (3): the window`s upper bound, the author`s.'),
    ...
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### section (A): the unbanked channel-change read is declared; (P3), (P6).'),
    ('author rulings ratified and entered', 0, 0, 'rulings', '### section (A): no new ruling.'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b448_addendum.txt.'),
    ...
    ('chains run', 0, 0, 'runs', '### (K) BAR 2.'),
    ('loom blocks appended', 0, 0, 'blocks', '### section (W).'),
    ('lines appended to the bar-floor rule module', 1, 1, 'lines', '### reading (3).'),
    ...
    ('errors entered as their owners`', 0, 0, 'errors', '### section (A).'),
...
def main(argv):
    print('=' * 100)
    print('b448_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b448_registration_2026-09-13.txt -- b448, "
                             "THE PARTITION READ AGAINST THE TAXONOMY, AND THE OUTLIER'S FREE CANDIDATE"),
            "clauses": clauses}
