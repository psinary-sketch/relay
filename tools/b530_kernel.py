# -*- coding: utf-8 -*-
"""b530_kernel.py -- COMPONENTS 1-5 IN SIDE-explicit-formula. ### `python tools/b530_kernel.py install | compile | statement | read | profile`

### `install`   -- the sealed draft (scratchpad) copied into the kernel as the NEW files SIDEExplicitFormula/DecayBound.lean,
###                SIDEExplicitFormula/RestBound.lean and AxiomCheckRest.lean (no prior bytes); sha256 banked. Run ONCE, after the seal.
### `compile decay|rest` -- `lake env lean -o <olean> SIDEExplicitFormula/<Module>.lean`; attempts banked PER MODULE; EVERY RUN IS AN ATTEMPT (READING (4)),
...
### `profile`   -- `lake env lean AxiomCheckRest.lean`: each theorem MATCHED ON THE WHOLE STRING; the `#check` and `#print`.
...
SCR = os.environ.get('B530_DRAFT', '')
MODS = dict(decay=os.path.join('SIDEExplicitFormula', 'DecayBound.lean'), rest=os.path.join('SIDEExplicitFormula', 'RestBound.lean'))
CHK = 'AxiomCheckRest.lean'
OLEANS = dict(decay=os.path.join('.lake', 'build', 'lib', 'lean', 'SIDEExplicitFormula', 'DecayBound.olean'),
              rest=os.path.join('.lake', 'build', 'lib', 'lean', 'SIDEExplicitFormula', 'RestBound.olean'))
...
def install():
    """### the sealed draft copied into the kernel as three NEW files; `prior_sha` is None when no file stood there (it must be)."""
    out = {}
    for name, dst in (('DecayBound.lean', MODS['decay']), ('RestBound.lean', MODS['rest']), (CHK, CHK)):
    ...
    io.open(os.path.join(D, 'b530_install.json'), 'w', encoding='utf-8', newline=NL).write(json.dumps(out, indent=1) + NL)
...
def compile_():
    ...
    print('### %s ATTEMPT %d : exit %d, %d error lines, %.1f s' % (which, n, r.returncode, len(errs), secs))
    return 0


def src():
    return io.open(os.path.join(KER, MODS['decay']), encoding='utf-8').read() + NL + NL + io.open(os.path.join(KER, MODS['rest']), encoding='utf-8').read()
...
def statement():
    d = decls(src())
    io.open(os.path.join(D, 'b530_statement.txt'), 'w', encoding='utf-8', newline=NL).write((NL + NL).join(x['head'] for x in d) + NL)
    io.open(os.path.join(D, 'b530_definitions.txt'), 'w', encoding='utf-8', newline=NL).write((NL + NL).join(x['whole'] for x in d) + NL)
...
def read():
    ...
    io.open(os.path.join(D, 'b530_read.json'), 'w', encoding='utf-8', newline=NL).write(json.dumps(res, indent=1, ensure_ascii=False) + NL)
    for x in res['decls']:
...
def profile():
    ...
    out = r.stdout
    res['checks'] = {}
    for nm in ('HStrip', 'HCount', 'f4_needs', 'HMax'):
        i = out.find('SIDEExplicitFormula.B321.%s :' % nm)
        res['checks'][nm] = out[i:out.find(NL, i)].strip() if i >= 0 else None
    res['prints'] = {}
    for nm in ('restR', 'windowA', 'zeta3Sum', 'f4_needs', 'HMax', 'HCount', 'HStrip', 'pairOrbit'):
        ...
        res['prints'][nm] = out[j:k if k > 0 else len(out)].strip() if j >= 0 else None
    io.open(os.path.join(D, 'b530_profile.json'), 'w', encoding='utf-8', newline=NL).write(json.dumps(res, indent=1, ensure_ascii=False) + NL)
    ...
    for k, v in res['prints'].items():
        print('### #print %s : %s' % (k, (v or 'NONE').replace(NL, ' ')[:400]))
    return 0
