# -*- coding: utf-8 -*-
"""b486_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b486_registration_2026-09-22.txt')
SPEC = os.path.join(ROOT, 'data', 'b486_satisfiable.json')
...
CLAUSES = [
    ...
    ('relations graded and recorded in this act`s record and the trail', 0, 0, 'relations', '### (K) BAR 1: a fold decides nothing new; every verdict is QUOTED from the act that reached it.'),
    ...
    ('ERRATA entries written', 0, 0, 'entries', '### (K) BAR 3: the id collision b485 created is REPORTED and NOT repaired; no ERRATA line is written or renumbered.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b486.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 0, 0, 'rulings', '### the ferry carries no new ruling; (R96) governs and was ratified at b479.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b486_addendum.txt, under the stem glob (R91) permits.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('expectations registered', 0, 0, 'expectations', '### section (J): NONE. The order registers none and this face invents none.'),
...
def main(argv):
    print('=' * 100)
    print('b486_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b486_registration_2026-09-22.txt -- b486, "
                             "THE FOLD, ELEVEN ACTS SINCE b474"),
            "clauses": clauses}
