# -*- coding: utf-8 -*-
"""b495_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b495_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b495_satisfiable.json')
...
CLAUSES = [
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b495.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b495_extract.txt`.'),
    ('readings taken silently', 0, 0, 'readings', '### section (A).'),
    ('survey anchor misses', 0, 0, 'misses', '### section (A): 0.'),
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### section (C): the five readings taken to write this face truthfully are DECLARED THERE, each as a prediction the components re-derive; and section (C2) declares the index queries.'),
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R106) -- b494 Table 2 row 5 corrected to PROSE, NO BANK with W-ORD-EPSTEIN-ISOLATION filed; C7_finite_type_false profiled in the next lv-conservation act; and the creation of SIDE-explicit-formula SPLIT IN TWO, this act creating and launching. ### RE-POINTED FROM b494`s (R105); THE CARRIED GROUND NAMED A RULING THIS ACT DOES NOT RATIFY.'),
    ('author rulings recorded as entries by this act', 0, 0, 'rulings', '### (Z): (R106) is ratified in the ferry and entered by the trail record.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b495_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written at all', 3, 3, 'documents', '### (W): SPIRAL_MAP.md (ONE appended waiver note beside rule 9), REGISTRY.md (ONE appended kernel-table row) and OPEN_TRAILS.md (one appended record). ### THE CLAUSE WAS CARRIED FROM b494 READING 2/2, WHERE IT WAS TRUE, AND IT CONTRADICTED THIS ACT`S OWN FACE, WHICH NAMES THREE IN (W). ### The face is unchanged; the INSTRUMENT was wrong and is repaired, and BOTH RUNS ARE BANKED.'),
    ...
    ('new `relay` act-tool files named on this face', 10, 10, 'files', '### (W): the stem glob (R91) permits, so the bound is on the STEM and not on a list of names. ### RAISED FROM b494`s 8: this act names a creator, a waiver writer and a REGISTRY writer that b494 had no use for.'),
    ...
    ('existing `relay` tools edited in place', 0, 0, 'tools', '### (W): NONE. corr_row.py is CARRIED as b488 guarded it and is not edited here, and NO OTHER relay tool is edited. ### The clause`s b488 history is kept in b488`s own bank; it is TRUE at this act and its ground is this act`s (W).'),
...
def main(argv):
    print('=' * 100)
    print('b495_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b495_extract.txt'),
    ...
    spec = {"registration": ("data/b495_registration_2026-09-23.txt -- b495, "
