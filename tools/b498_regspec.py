# -*- coding: utf-8 -*-
"""b498_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b498_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b498_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 0, 0, 'files', '### (W): the probe `b498_printaxioms.lean` is written by the launched process into relay`s data directory, which is not a repository of Lean, and is not committed; no `.lean` file of any kernel is written.'),
    ('kernel builds', 2, 2, 'builds', '### (B): Component 2`s foreground closure build and Component 3`s detached `lake build`. ### RAISED FROM b497`s 0: THIS ORDER COMMANDS BOTH.'),
    ('chains run, or channel values computed', 0, 0, 'runs', '### (Z): none. Every value is read from the banked listing.'),
    ('lean runs', 1, 1, 'runs', '### (B)(3): the PrintAxioms probe inside the detached launch, and no other.'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z): the kernel`s packages are already on disk under `.lake/packages`.'),
    ('lanes opened', 1, 1, 'lanes', '### the ferry: THE KERNEL LANE OPENS FOR THIS ACT.'),
    ('disproof lanes opened', 0, 0, 'lanes', '### section (Z): (R51) stands.'),
    ('network remotes contacted', 1, 1, 'remotes', '### (B)(2): the Mathlib cache host reached by `lake exe cache get`. ### The pins` ls-remote reads and the ritual`s pushes excepted, as ever.'),
    ...
    ('addresses resolved or documents fetched', 1, 1, 'reads', '### (B)(2): the ONE cache fetch. ### Nothing else is fetched and nothing at Zenodo is read or written.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b498.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b498_extract.txt`.'),
    ('readings taken silently', 0, 0, 'readings', '### section (A).'),
    ('survey anchor misses', 0, 0, 'misses', '### section (A): 0.'),
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### section (C): SEVEN reads made before the seal are declared there, (i) to (vii), each re-derived by the components from the files.'),
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R109) -- (1) the three titles reworded and the seven sentences taken from the zenodo_edits file, the platform session the author`s, an ERRATA entry filed only by the act that fetches back; (2) THE BUILD RE-LAUNCHED in its own process group and console, with the Mathlib cache fetched first. ### RE-POINTED FROM b497`s (R108).'),
    ('author rulings recorded as entries by this act', 0, 0, 'rulings', '### (Z): (R109) is ratified in the ferry and entered by the trail record.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b498_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written at all', 1, 1, 'documents', '### (W): OPEN_TRAILS.md, ONE appended record. ### LOWERED FROM b497`s 2: no REGISTRY note is conditional here, because no profile is read.'),
    ...
    ('new `relay` act-tool files named on this face', 9, 9, 'files', '### (W): the stem glob (R91) permits, so the bound is on the STEM. ### RAISED FROM b497`s 8: this act adds the launcher `b498_launch.py`, which is a separate file because it is the CHILD process.'),
...
def main(argv):
    print('=' * 100)
    print('b498_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b498_extract.txt'),
    ...
    spec = {"registration": ("data/b498_registration_2026-09-23.txt -- b498, "
                             "THE BUILD RE-LAUNCHED IN ITS OWN CONSOLE, WITH THE CACHE"),
            "clauses": clauses}
