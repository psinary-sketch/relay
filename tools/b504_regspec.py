# -*- coding: utf-8 -*-
"""b504_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b504_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b504_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 0, 0, 'files', '### (W): the probe `b504_printaxioms.lean` is written by the launched process into relay`s data directory, which is not a repository of Lean, and is not committed; no `.lean` file of any kernel is written.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): none; the build read is b498`s and is over.'),
    ('chains run, or channel values computed', 2, 2, 'runs', '### (B)(1)-(2) and (B)(4): the chain on the exact transform at all 119 cells with its bound, and the Epstein control on the same transform.'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### (R114)(3): the numerical lane opens for this act alone and shuts at its close.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b504.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b504_extract.txt`.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R114) -- one instrument, one transform; the grid follows the bank; the lane for b504.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b504_addendum.txt, under the stem glob (R91) permits.'),
...
def main(argv):
    print('=' * 100)
    print('b504_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b504_extract.txt'),
    ...
    spec = {"registration": ("data/b504_registration_2026-09-23.txt -- b504, "
                             "THE CHAIN ON ONE TRANSFORM, ALL 119 CELLS RE-VERIFIED"),
            "clauses": clauses}
