# -*- coding: utf-8 -*-
"""b505_complete.py -- THE EPSTEIN BANK FOR Q0 = x^2 + xy + 6y^2 COUNTED BELOW 150 BY THE ARGUMENT PRINCIPLE.

### `python tools/b505_complete.py fixture | run | price J0 J1`
...
NL = chr(10)
BANK = os.path.join(D, 'b505_edges.jsonl')
LOG = os.path.join(D, 'b505_c2_log.txt')
OUT = os.path.join(D, 'b505_c2_results.json')
...
def route_a(rho):
    ...
    return float(abs(BZ.CE.Lam(s) / g))


def fixture():
    """### the evaluator against route A (dps 119, K 240) at both off-line zeros, 20 on-line zeros and five points
    ### off the zeros; and |Z| by the evaluator at EVERY banked zero -- the re-examination of the bank."""
    import b326_zeros as BZ
    rows = []
    offl = [complex(*o['rho_a']) for o in LIB['offline']]
    on = [z['gamma_a'] for z in LIB['zeros']]
    sub = offl + [complex(0.5, g) for g in on[::max(1, len(on) // 20)]]
    pts = [complex(-0.5, 7.5), complex(-0.5, 149.5), complex(1.5, 149.5), complex(1.2, 0.0), complex(0.25, 88.8)]
    for z in sub + pts:
        BZ.bind(119, 240)
        s = mp.mpc(z)
        g = (mp.sqrt(23) / (2 * mp.pi)) ** s * mp.gamma(s)
        ...
        mp.mp.dps = DPS
        b = zcs(s)
        rows.append(dict(s=[z.real, z.imag], route_a=float(abs(a)), cs=float(abs(b)),
                         rel=float(abs(a - b) / max(abs(a), mp.mpf(1))), zero=z in sub))
    mp.mp.dps = DPS
    allz = [dict(s=[z.real, z.imag], cs=float(abs(zcs(mp.mpc(z))))) for z in offl + [complex(0.5, g) for g in on]]
    worst_zero = max(r['cs'] for r in allz)
    worst_rel = max(r['rel'] for r in rows)
    log('fixture: route A against the evaluator at %d banked zeros and %d points off them, largest |A - CS|/max(1,|A|)'
        ' %.2e ; |Z| by the evaluator at all %d banked zeros, largest %.2e'
        % (len(sub), len(pts), worst_rel, len(allz), worst_zero))
    return dict(route_rows=rows, all_zeros=allz), worst_zero, worst_rel
...
def run():
    t00 = time.time()
    log('b505 C2 -- start ; SIG_MAX %.2f ; columns %s ; dps %d ; Bessel terms %d' % (SIG_MAX, sig_lines(), DPS, NBES))
    rows, wz, wr = fixture()
    if not (wz < 1e-10 and wr < 1e-8):
    ...
    res = dict(sig_max=SIG_MAX, dps=DPS, nbes=NBES, step0=STEP0, darg=DARG, fixture=rows, fixture_worst_zero=wz,
               fixture_worst_rel=wr, boxes=boxes, not_near=len(bad), minmod=minmod, whole=whole, per=per, total=tot,
    ...
    log('b505 C2 -- done in %.0f s ; results written' % res['seconds'])
    return 0
...
def price(j0, j1):
    ...
    global BANK
    BANK = os.path.join(os.environ.get('TEMP', '.'), 'b505_price_edges.jsonl')
