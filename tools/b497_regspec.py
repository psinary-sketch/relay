# -*- coding: utf-8 -*-
"""b497_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b497_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b497_satisfiable.json')
...
CLAUSES = [
    ...
    ('platform calls of any kind', 0, 0, 'calls', '### (Z): this seat calls no platform. The pins` ls-remote reads and the ritual`s pushes are the ritual`s own and are excepted.'),
    ('.lean files written or edited in any repository', 0, 0, 'files', '### section (Z).'),
    ('kernel builds', 0, 0, 'builds', '### (Z): none; b475`s log is not opened.'),
    ...
    ('repositories cloned', 0, 0, 'repositories', '### (Z): every repository this act reads is already on disk.'),
    ('lanes opened', 0, 0, 'lanes', '### (Z): none. The numerical lane was shut at b492`s end and this act opens nothing.'),
    ...
    ('ERRATA entries written', 0, 0, 'entries', '### (W): ERRATA.md is not written by this act at all; b487 did that work under (R97).'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b497.zip, under (R69).'),
    ...
    ('statements imported', 0, 0, 'statements', '### (Z): nothing is imported; b486`s bank is read as text.'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b497_extract.txt`.'),
    ('readings taken silently', 0, 0, 'readings', '### section (A).'),
    ('survey anchor misses', 0, 0, 'misses', '### section (A): 0.'),
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### section (C): Component 0 RAN at step zero because the order makes every other component conditional on it, and its result is declared there. ### AND SECTION (C) DECLARES A READ THE SEAT ALREADY MADE OF THE SUBJECT ITSELF -- the log`s first eight and last six lines, taken to decide the process state -- together with the three things that read showed, because A PREDICTION DRAWN FROM A PEEK MUST SAY IT WAS DRAWN FROM A PEEK. ### (C2) declares the four index queries.'),
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R108) -- THE GRADE CELL MAY BE SPLIT where a terminal is a biconditional or conjunction whose parts grade differently, in the form DERIVES(->) / INTERFACES(<-); the four grades are unchanged and this is a CELL FORM, not a fifth grade. e_difficulty is routed to a statement-read act of its own and THIS ACT DOES NOT CHOOSE. And the navigator`s counts are corrected FROM FILES: 44 repositories, 16 keystones under the census`s test, 1,021 terminal rows. ### RE-POINTED FROM b496`s (R107).'),
    ('author rulings recorded as entries by this act', 0, 0, 'rulings', '### (Z): (R108) is ratified in the ferry and entered by the trail record.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b497_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written at all', 2, 2, 'documents', '### (W): OPEN_TRAILS.md (one appended record) ALWAYS, and REGISTRY.md (one appended note on the SIDE-explicit-formula profile cell) ### **CONDITIONALLY, AND ONLY IF COMPONENT 2 PASSES**. ### THE CAP IS THE MAXIMUM THE FACE PERMITS, NOT THE NUMBER THE ACT EXPECTS TO WRITE: the seat`s own (S1) predicts Component 2 will STOP and REGISTRY will go untouched, and a cap set to that prediction would make the face unsatisfiable if the prediction were wrong. ### RAISED FROM b496`s 1/1.'),
    ...
    ('new `relay` act-tool files named on this face', 8, 8, 'files', '### (W): the stem glob (R91) permits, so the bound is on the STEM and not on a list of names. ### LOWERED FROM b495`s 10: this act has ONE substantive tool and it is NOT act-stemmed -- `tools/terminal_table.py` is SHARED, because (R107) has the closing suite re-run it at every later close.'),
    ...
    ('existing `relay` tools edited in place', 0, 0, 'tools', '### (W): NONE. ### `terminal_table.py` is CARRIED and UNEDITED -- b496 built it and (R107) has this act RE-RUN it, not change it. ### corr_row.py is carried as b488 guarded it. ### LOWERED FROM b496`s 1/1, where the suite genuinely gained the generator call; that edit is b496`s and is not repeated here.'),
...
def main(argv):
    print('=' * 100)
    print('b497_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b497_extract.txt'),
    ...
    spec = {"registration": ("data/b497_registration_2026-09-23.txt -- b497, "
                             "THE FOLD'S SECTION WRITTEN HOME, AND THE ROW WRITER GUARDED"),
            "clauses": clauses}
