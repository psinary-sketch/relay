# -*- coding: utf-8 -*-
"""b513_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b512's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b513_registration_2026-09-24.txt')
OT = os.path.join(PP, 'OPEN_TRAILS.md')
PRIOR_RELAY = 'e3bc8942'      # ### b512's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
ML = os.path.join(KER, '.lake', 'packages', 'mathlib')
KER_PRIOR = 'b52c1d7'         # ### the kernel's tip before this act
MODF = os.path.join(KER, 'SIDEExplicitFormula', 'RHChain.lean')
ACCF = os.path.join(KER, 'AxiomCheckChain.lean')
NEWK = {'SIDEExplicitFormula/RHChain.lean', 'AxiomCheckChain.lean'}
PRIOR_PP = '42ea427'           # ### b512's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b513 —'
...
def sources():
    C = runs('b513_compile_log')
    PR = runs('b513_profile_log')
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b513_ferry.txt')),
        scan=read(os.path.join(D, 'b513_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b513_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b513_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b513_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b513_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b512_closing.txt')),
        addendum=read(os.path.join(D, 'b513_addendum.txt')),
        comp=read(os.path.join(D, 'b513_components.txt')),
        desk=read(os.path.join(D, 'b513_desk_notes.txt')),
        stmt=read(os.path.join(D, 'b513_statement.txt')),
        prof=json.loads(read(os.path.join(D, 'b513_profile.json')) or '{}'),
        plog=PR[-1]['text'] if PR else '',
        compiles=C, clast=C[-1]['text'] if C else '',
        tb=json.loads(read(os.path.join(D, 'b513_table.json')) or '{}'), tbtxt=read(os.path.join(D, 'b513_table.txt')),
        sc=json.loads(read(os.path.join(D, 'b513_scores.json')) or '{}'),
        mod=read(MODF), acc=read(ACCF),
        ker_vendored_changed=gits(KER, 'diff', '--name-only', KER_PRIOR, '--', '.', ':!SIDEExplicitFormula/RHChain.lean', ':!AxiomCheckChain.lean'),
        ...
        suite=read(os.path.join(T, 'b513_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b513.zip'))),
        chain_clean=(gits(ROOT, 'status', '--porcelain', '--', 'tools/b321_window.py', 'tools/e16/carto_atlas.py',
                          ...
                          'tools/b504_chain.py', 'tools/b506_kernel.py', 'tools/b511_families.py') == ''),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b513 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b513_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (MODF, ACCF, os.path.join(D, 'b513_table.json'))),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b513 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[012]_|^b334_', f)]
...
def decl_head(mod, name):
    """### the declaration from its line to its FIRST `:=`, recomputed from the module -- or EMPTY."""
    m = re.search(r'^theorem %s\b' % re.escape(name), mod or '', re.M)
    return mod[m.start():mod.index(':=', m.start()) + 2] if m else ''


def proof_lines(mod):
    m = re.search(r'^theorem rh_imp_h2_sign\b', mod or '', re.M)
    if not m:
        return -1
...
def need(S, k):
    return next((n for n in (S['tb'].get('needs') or []) if n['key'] == k), {})
...
def corr_row(S):
    return next((l for l in S['corr'].split(NL) if l.startswith('| 362 |')), '')
...
def attempts_ok(S):
    C = S['compiles']
    first = next((i + 1 for i, a in enumerate(C) if a['ok']), None)
    return C, first


ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R122) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b513' in S['ferry'],
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b512`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 361 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 361 |', '| 3610 |'))),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b513' in S['ferry'] and 'ACT b513' in S['face'] and not glob.glob(os.path.join(D, 'b514_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b513')),
    ...
    ('G-R122-ENTERED', 'the banked ferry AND the trail', lambda S: 'RULING (R122) END' in S['ferry'] and S['ot'].count('**(R122) ratified') == 1,
     lambda S: put(S, 'ot', S['ot'].replace('**(R122) ratified', '(R122) noted'))),
    ...
    ('G-VENDORED-UNTOUCHED', 'the kernel`s git state -- nothing but the two new files changed',
     ...
     lambda S: put(S, 'ker_vendored_changed', 'SIDEExplicitFormula/Registers.lean')),
    ('G-TWO-THEOREMS', 'the module`s text -- exactly the two theorems READING (2) names',
     lambda S: re.findall(r'^theorem (\S+)', S['mod'], re.M) == ['rh_imp_h2_sign', 'rh_imp_cell_form'],
     lambda S: put(S, 'mod', S['mod'] + NL + 'theorem rh_h2_extra : True := trivial')),
    ('G-PROFILE-WHOLE-STRING', 'the profile run file -- each link`s line equal to the standard-three string',
     ...
     lambda S: put(S, 'plog', S['plog'].replace(STD3 % q('rh_imp_h2_sign'), (STD3 % q('rh_imp_h2_sign')).replace(', Quot.sound', ', sorryAx')))),
    ('G-NO-SORRY', 'Lean`s own output in the last compile run file and the profile run file, and the module text',
    ...
    ('G-STATEMENT-PRINTED', 'the table against the module -- the theorem from its line to its `:=`, recomputed',
     lambda S: bool(decl_head(S['mod'], 'rh_imp_h2_sign')) and (S['tb'].get('C1') or {}).get('statement') == decl_head(S['mod'], 'rh_imp_h2_sign')
     and 'theorem rh_imp_h2_sign : RiemannHypothesis → h2_sign :=' in S['tbtxt'],
     lambda S: put(S, 'tb', dict(S['tb'], C1=dict(S['tb'].get('C1') or {}, statement='theorem ghost : True :=')))),
    ('G-PROOF-LENGTH', 'the table`s line count against the module, recomputed',
     lambda S: proof_lines(S['mod']) > 0 and (S['tb'].get('C1') or {}).get('proof_lines') == proof_lines(S['mod']) == S['sc'].get('proof_lines'),
    ...
    ('G-CHAIN-ONE-LINE', 'the module -- the composite`s proof term on one line, the two links named',
     lambda S: bool(re.search(r'^theorem rh_imp_cell_form [^\n]*\n[^\n]*:= fun hRH => h2_sign_imp_cell F W hF \(rh_imp_h2_sign hRH\)$', S['mod'], re.M)),
     lambda S: put(S, 'mod', S['mod'].replace('fun hRH => h2_sign_imp_cell', 'fun hRH =>\n  h2_sign_imp_cell'))),
    ('G-CONVERSE-STATED', 'the module AND the last compile -- the converse a Prop, tied to b512`s, compiled',
     lambda S: 'def h2_sign_imp_rh : Prop := h2_sign → RiemannHypothesis' in S['mod']
     and 'example : h2_sign_imp_rh = h2_imp_rh := rfl' in S['mod'] and bool(S['compiles']) and S['compiles'][-1]['ok'],
     lambda S: cut(S, 'mod', 'def h2_sign_imp_rh : Prop := h2_sign → RiemannHypothesis')),
    ('G-CONVERSE-WORDS', 'the docstring as the table read it from the module -- (R122)(2)`s words',
     lambda S: 'makes zeroSide negative at an off-line zero' in ' '.join(((S['tb'].get('converse') or {}).get('docstring') or '').split())
     and 'attempted numerically' in ((S['tb'].get('converse') or {}).get('docstring') or ''),
     lambda S: put(S, 'tb', dict(S['tb'], converse=dict(S['tb'].get('converse') or {}, docstring='a converse')))),
    ('G-ABSENT-SEARCHED', 'every ABSENT need -- its declaration needles` yields printed and zero in both trees',
     lambda S: bool(S['tb'].get('absent')) and all(n['decl'] and all(y['kernel_files'] == 0 and y['mathlib_files'] == 0 for y in n['decl'])
                                                   for n in S['tb'].get('needs') or [] if not n['found'])
     and S['tb']['absent'] == [n['key'] for n in S['tb']['needs'] if not n['found']],
     lambda S: put(S, 'tb', dict(S['tb'], needs=[dict(n, decl=[dict(y, mathlib_files=1) for y in n['decl']]) for n in S['tb'].get('needs') or []]))),
    ('G-FOUND-FOUND', 'every FOUND need -- its files exist, re-checked on disk',
     lambda S: all(n['files'] and all(os.path.exists(os.path.join(KER, f)) or os.path.exists(os.path.join(ML, f)) for f in n['files'])
                   for n in S['tb'].get('needs') or [] if n['found']) and any(n['found'] for n in S['tb'].get('needs') or []),
     lambda S: put(S, 'tb', dict(S['tb'], needs=[dict(n, files=['nowhere.lean']) if n['found'] else n for n in S['tb'].get('needs') or []]))),
    ('G-GRADES-WRITTEN', 'the row and the trail -- each link beside DERIVES, and no other grade word in the record',
     lambda S: all(('`%s` DERIVES' % n) in corr_row(S) for n in LINKS)
     and sorted(re.findall(BT + r'([a-z0-9_]+)' + BT + ' — DERIVES', ' '.join(trail_lines(S)))) == sorted(LINKS)
     and len(trail_lines(S)) == 3 and all(len(GRADE_RE.findall(l)) == 1 for l in trail_lines(S)),
     lambda S: put(S, 'ot', S['ot'].replace('`rh_imp_cell_form` — DERIVES', '`rh_imp_cell_form` — DERIVES, beside a SHELL'))),
    ('G-TABLE-CHAIN', 'the regenerated terminal table -- a row per link, graded DERIVES, its statement read',
     lambda S: all(table_row(S, n) and '| DERIVES |' in table_row(S, n).replace('**', '') and 'theorem %s' % n in table_row(S, n) for n in LINKS),
     lambda S: put(S, 'table', S['table'].replace('`%s`' % q('rh_imp_cell_form'), '`ghost`'))),
    ('G-N1-SCORED', 'the desk against the profile and the run files', lambda S: 'n1' in S['sc'] and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == (bool((S['prof'].get('std3') or {}).get(q('rh_imp_h2_sign'))) and not S['prof'].get('sorry', True)
    ...
    ('G-N2-SCORED', 'the desk against the module`s line count', lambda S: 'n2' in S['sc'] and desk_word(S, 'N2') == word_of(S['sc']['n2'])
     and S['sc']['n2'] == (0 < proof_lines(S['mod']) < 80),
     lambda S: put(S, 'sc', dict(S['sc'], n2=not S['sc'].get('n2')))),
    ('G-N3-SCORED', 'the desk against the needs', lambda S: 'n3' in S['sc'] and desk_word(S, 'N3') == word_of(S['sc']['n3'])
     and S['sc']['n3'] == (sum(1 for n in S['tb'].get('needs') or [] if not n['found']) >= 1),
    ...
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from its bank',
     ...
     and desk_word(S, 'S2') == word_of(0 < proof_lines(S['mod']) < 50)
     and desk_word(S, 'S3') == word_of(bool(need(S, 'a').get('found')) and not need(S, 'f').get('found', True)),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b513 -'))),
    ...
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 362 once, six cells as it landed',
     lambda S: S['corr'].count('| 362 |') == 1 and S['corr'].count('(b513, under (R122))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 362 | a duplicate row |')),
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b512`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b512_table.py'])),
    ('G-WRITELIST-KINDS', 'every b513 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b513')" in S['suite']
                and "data/b513_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b513_components.txt' in gits(ROOT, 'show'")),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b513')
              and 'data/b513_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    deferred = [] if pushed else ['G-MIRROR-TAGGED-BUILD', 'G-TABLE-CHAIN']   # ### both read what exists only after the push
    S['declared_eq_run'] = (set(names) - set(deferred)) == (set(declared) - set(deferred))

    rec('=' * 104)
    rec('b513 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b513_checks_postpush.txt' if pushed else 'b513_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b513_exercise.json'), 'w', encoding='utf-8', newline=NL),
