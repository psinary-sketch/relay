# -*- coding: utf-8 -*-
"""b306_correspondence.py -- TWO ROWS: WHERE THE ARITHMETIC ENTERS, AND THE SPECIES MATCH.
...
ROWS = [
    ("THE CORPUS'S DIFFERENCE IS NOT THE SOURCE'S (b306)",
     ...
     "THE CORPUS'S DIFFERENCE IS NOT THE SOURCE'S (b306): decided by definitions, constituent by "
     ...
     "`tools/b306_difference.py`, which parses BOTH tables out of their emitting files — b254's "
    ...
    ("THE SHARED-TARGET STEM SWEEP (b306)",
     ...
     "THE SHARED-TARGET STEM SWEEP (b306): the stem sweep is extended to the files every act "
     ...
     "**NO TERMINAL.** `tools/b306_stem_scope.py`; the stems are read from `ferry_scan.stems()` "
...
def main():
    ...
    print('=' * 100)
    print('b306 -- THE DECISION\'S ROW, AND THE SWEEP-SCOPE ROW.')
    print('=' * 100)
    ...
    g1 = ('NO TERMINAL' in r1[2] and 'NO PRINT' in r1[3]
          and 'NO MEASUREMENT IS DISTURBED AND NO GRADE MOVES' in r1[4])
    g2 = ('repair what this seat wrote and mis-fixed' in r2[4].lower()
          and 'NO TERMINAL' in r2[2])
    print('  row 1 declares no terminal/print and that no measurement is disturbed : %s  %s'
          % (g1, 'PASS' if g1 else '### FAIL ###'))
    print('  row 2 states the repair boundary it drew : %s  %s'
