# -*- coding: utf-8 -*-
"""b349_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b349_registration_2026-09-07.txt')
SPEC = os.path.join(ROOT, 'data', 'b349_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ("new seeds built in part (a)", 0, 0, "seeds", "### section (C): part (a) reads banked rows and runs none."),
    ("new seeds built in part (b)", 3, 3, "seeds", "### section (E): three sealed heights, and ONLY if (b) runs."),
    ("heights added after a value", 0, 0, "heights", "section (E)."),
    ("aims added, removed or re-ordered", 0, 0, "aims", "section (C)."),
    ("degenerate seeds charted", 0, 0, "seeds", "### section (E): reported as degenerate, not charted."),
    ("relay tools created", 3, 3, "files", "### quote_norm.py, b349_relative.py, b349_checks.py."),
    ("owner instrument files edited", 0, 0, "files", "none."),
    ("PLACE-papers files written", 0, 0, "files", "### no leg of this act moves a ledger row."),
    ...
    ("banked figures re-computed", 0, 0, "figures", "### section (C): part (a) reads them."),
    ...
    ("crossings claimed", 0, 0, "claims", "section (I)."),
...
def main(argv):
    print('=' * 100)
    print('b349_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": "data/b349_registration_2026-09-07.txt -- b349, THE LI CONTROL, RE-RUN", "clauses": clauses}
