# -*- coding: utf-8 -*-
"""b475_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b475_registration_2026-09-22.txt')
SPEC = os.path.join(ROOT, 'data', 'b475_satisfiable.json')
...
CLAUSES = [
    ...
    ('kernel builds', 0, 0, 'builds', '### section (Z): no build of the CORPUS; the detached run builds the third party`s clone under (R80).'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b475_addendum.txt.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('expectations registered', 1, 1, 'expectations', '### section (J): (N1), scored by the act that reads the log.'),
...
def main(argv):
    print('=' * 100)
    print('b475_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b475_registration_2026-09-22.txt -- b458, "
