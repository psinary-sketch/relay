# -*- coding: utf-8 -*-
"""b487_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b487_registration_2026-09-22.txt')
SPEC = os.path.join(ROOT, 'data', 'b487_satisfiable.json')
...
CLAUSES = [
    ...
    ('repositories cloned', 0, 0, 'repositories', '### (K) BAR 1: SIDE-lv-conservation is ALREADY on disk and is read at a tag by git show.'),
    ('lanes opened', 0, 0, 'lanes', '### (K) BAR 1: statements are READ AS TEXT by git show; no lane is opened and nothing is compiled.'),
    ...
    ('prior acts` banks edited', 0, 0, 'files', '### (W): b485`s bank gains ONE APPENDED dated pointer by (R97)`s express direction; NO line of it is edited.'),
    ...
    ('ERRATA entries written', 0, 0, 'entries', '### (K) BAR 2: NO NEW ENTRY. One dated RENUMBER LINE is APPENDED beneath b485`s entry under (R97); no existing line is edited and no id is rewritten in place.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b487.zip, under (R69).'),
    ...
    ('statements imported', 0, 0, 'statements', '### (K) BAR 1: the eight facts are READ and SCORED; not one is imported, copied or vendored.'),
    ...
    ('repositories hard-failing at step zero', 0, 0, 'repositories', '### section (A) READING (0): the FIRST run read 4, all ls-remote UNRESOLVED at once -- a transient network failure; the retry reads 0 of 4 and BOTH runs are banked.'),
    ('repositories ahead of origin at step zero', 0, 0, 'repositories', '### section (A): nothing pushed because nothing was ahead.'),
    ('readings of the order declared in advance on this face', 7, 7, 'readings', '### section (A): (1) to (7), seven.'),
    ('pre-face reads declared', 5, 5, 'reads', '### section (A): (P1), (P2), (P3), (P3b), (P4).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R97) resolves the ERRATA id collision by APPENDED NOTE.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b487_addendum.txt, under the stem glob (R91) permits.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('expectations registered', 3, 3, 'expectations', '### section (J): (N1), (N2), (N3), each with the seat`s own call made before the drafts are composed.'),
    ...
    ('PLACE-papers documents written other than by appending', 0, 0, 'documents', '### (K) BAR 2: every write in every file is an APPEND.'),
    ('PLACE-papers documents written at all', 2, 2, 'documents', '### (W): OPEN_TRAILS.md (two appends) and ERRATA.md (one appended renumber line).'),
...
def main(argv):
    print('=' * 100)
    print('b487_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b487_registration_2026-09-22.txt -- b487, "
                             "THE ERRATA RENUMBER AND THE DESCRIPTION DRAFTS"),
            "clauses": clauses}
