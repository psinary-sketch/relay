# -*- coding: utf-8 -*-
"""b529_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b529's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b529_registration_2026-09-25.txt')
OT = os.path.join(PP, 'OPEN_TRAILS.md')
PRIOR_RELAY = '68351ad9'      # ### b528's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
KER_TIP = '334f0ed'           # ### the kernel's tip before this act
PRIOR_PP = '291d6ee'           # ### b528's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b529 —'
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b529_ferry.txt')),
        scan=read(os.path.join(D, 'b529_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b529_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b529_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b529_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b529_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b528_closing.txt')),
        addendum=read(os.path.join(D, 'b529_addendum.txt')),
        comp=read(os.path.join(D, 'b529_components.txt')),
        desk=read(os.path.join(D, 'b529_desk_notes.txt')),
        report=read(os.path.join(D, 'b529_report.txt')),
        mod=read(os.path.join(KER, 'SIDEExplicitFormula', 'PairTerm.lean')),
        acc=read(os.path.join(KER, 'AxiomCheckPair.lean')),
        att=json.loads(read(os.path.join(D, 'b529_attempts.json')) or '[]'),
        att_files={int(re.search(r'attempt(\d+)\.lean$', p).group(1)): hashlib.sha256(open(p, 'rb').read()).hexdigest()
                   for p in glob.glob(os.path.join(D, 'b529_attempt[0-9]*.lean')) if re.search(r'attempt(\d+)\.lean$', p)},
        att0=read(os.path.join(D, 'b529_attempt0_stopped_by_host.txt')),
        clogs=len(glob.glob(os.path.join(D, 'b529_compile_log*.txt'))),
        clast=read(sorted(glob.glob(os.path.join(D, 'b529_compile_log*.txt')), key=os.path.getmtime)[-1]),
        plog=read(sorted(glob.glob(os.path.join(D, 'b529_profile_log*.txt')), key=os.path.getmtime)[-1]),
        inst=json.loads(read(os.path.join(D, 'b529_install.json')) or '{}'),
        prof=json.loads(read(os.path.join(D, 'b529_profile.json')) or '{}'),
        rd=json.loads(read(os.path.join(D, 'b529_read.json')) or '{}'),
        stmt=read(os.path.join(D, 'b529_statement.txt')),
        vals=json.loads(read(os.path.join(D, 'b529_values.json')) or '{}'),
        rd524=json.loads(read(os.path.join(D, 'b524_read.json')) or '{}'),
        bank34=[r for r in jsonl(os.path.join(D, 'b522_cells.jsonl')) if r['a'] == 34.0 and r['p'] == 7],
        prior_mods={f: (blob(KER, '334f0ed:' + f), blob(KER, 'HEAD:' + f), open(os.path.join(KER, f), 'rb').read().replace(b'\r', b''))
                    for f in ('SIDEExplicitFormula/TwoPropertyWindow.lean', 'AxiomCheckWindow.lean')},
        ker_changed=set(x for x in gits(KER, 'diff', '--name-only', '334f0ed').split(NL) if x.strip())
                    | set(p[3:].strip() for p in git(KER, 'status', '--porcelain').split(NL) if p.strip()),
        ker_tags=gits(KER, 'tag', '--points-at', 'HEAD'),
        res=json.loads(read(os.path.join(D, 'b529_results.json')) or '{}'),
        sc=json.loads(read(os.path.join(D, 'b529_scores.json')) or '{}'),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b529_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b529.zip'))),
        chain_clean=(gits(ROOT, 'status', '--porcelain', '--', 'tools/b321_window.py', 'tools/e16/carto_atlas.py',
                          ...
                          'tools/b528_smooth.py', 'tools/b504_chain.py', 'tools/b506_kernel.py', 'tools/b511_families.py', 'tools/b503_zero.py', 'tools/b514_window.py', 'tools/b515_window.py', 'tools/b518_window.py', 'tools/b519_window.py', 'tools/b520_widen.py', 'tools/b521_tail.py', 'tools/b522_reach.py', 'tools/b526_xi7.py') == ''),
        ker_clean=(gits(KER, 'rev-parse', '--short=7', 'HEAD') == KER_TIP and gits(KER, 'status', '--porcelain', '--untracked-files=no') == ''),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b529 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b529_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (os.path.join(D, x) for x in ('b529_install.json', 'b529_attempts.json', 'b529_profile.json', 'b529_values.json', 'b529_run_log.txt'))),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b529 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[0-9]_|^b52[0-8]_|^b334_', f)]
...
C12 = ['pairTwo_factored', 'pairTwo_near_far', 'nearInt_ge', 'realizedGrowth_eq', 'realizedGrowth_ge_one', 'nearPair_eq', 'pair_near_sign']
DERIVED = ['pairTwo_factored', 'pairTwo_near_far', 'nearInt_ge', 'realizedGrowth_ge_one', 'pair_near_sign']
NEWK = {'SIDEExplicitFormula/PairTerm.lean', 'AxiomCheckPair.lean'}
...
def eps_now(v):
    return abs(complex(*v['F'])) / v['N'], abs(complex(*v['F0'])) / v['S']
...
def corr_row(S):
    return next((l for l in S['corr'].split(NL) if l.startswith('| 378 |')), '')
...
ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R139) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b529' in S['ferry'],
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b529' in S['ferry'] and 'ACT b529' in S['face'] and not glob.glob(os.path.join(D, 'b530_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b529')),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b529 -'))),
    ...
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b528`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b519_window.py'])),
    ('G-WRITELIST-KINDS', 'every b529 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b529')" in S['suite']
                and "data/b529_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b529_components.txt' in gits(ROOT, 'show'")),
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b528`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 377 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 377 |', '| 3770 |'))),
    ('G-R139-ENTERED', 'the banked ferry AND the trail', lambda S: 'RULING (R139) END' in S['ferry'] and S['ot'].count('**(R139) ratified') == 1,
     lambda S: put(S, 'ot', S['ot'].replace('**(R139) ratified', '(R139) noted'))),
    ('G-INSTALLED-FROM-DRAFT', 'the install bank against the banked sources -- the sealed draft is what first ran, onto no prior file',
     lambda S: S['inst']['PairTerm.lean']['prior_sha'] is None and S['inst']['AxiomCheckPair.lean']['prior_sha'] is None
     and S['inst']['PairTerm.lean']['draft_sha'] == S['inst']['PairTerm.lean']['installed_sha'] == S['att'][0]['source_sha']
     == S['att_files'].get(1) == hashlib.sha256(open(os.path.join(D, 'b529_attempt0_stopped.lean'), 'rb').read()).hexdigest(),
     lambda S: put(S, 'inst', {**S['inst'], 'PairTerm.lean': dict(S['inst']['PairTerm.lean'], draft_sha='0' * 64)})),
    ('G-ATTEMPTS-BANKED', 'the attempts bank against the banked sources and logs; attempt 0 a host stop with its note',
     ...
     and 'STOPPED BY HOST, attempt 0' in S['att0'],
     lambda S: put(S, 'att', S['att'][:-1])),
    ('G-PRIOR-MODULES-UNTOUCHED', 'the kernel`s two earlier files at 334f0ed, at HEAD and on disk -- byte-identical',
    ...
    ('G-FARSMALL-A-PROP', 'Lean`s own #check of farSmall in the profile run file',
     lambda S: (S['prof'].get('check_prop') or '').endswith('→ Prop') and 'farSmall : ℝ → (ℝ → ℝ) → ℝ → ℝ → Prop' in S['plog'],
     lambda S: put(S, 'prof', dict(S['prof'], check_prop='farSmall : ℝ → (ℝ → ℝ) → ℝ → ℝ → ℝ'))),
    ('G-CONSTANTS-PRINTED', 'Lean`s own #print of pairConst and pairEps -- c = 2 and eps` as the face states it',
     lambda S: re.search(r'pairConst : ℝ :=\s*2\s*$', (S['prof'].get('prints') or {}).get('pairConst') or '') is not None
     and ' '.join(((S['prof'].get('prints') or {}).get('pairEps') or '').split()).endswith(
         'pairConst - (2 * (1 - (2 * ε + ε ^ 2)) - δ ^ 2 / (2 * γ₀ ^ 2) * (1 + (2 * ε + ε ^ 2))) / (1 + ε) ^ 2'),
     lambda S: put(S, 'prof', dict(S['prof'], prints=dict(S['prof'].get('prints') or {}, pairConst='def SIDEExplicitFormula.B321.pairConst : ℝ := 4')))),
    ('G-VENDORED-REFS-READ', 'the module`s code, recomputed HERE, against the read bank and b524`s',
     lambda S: set(vrefs(S['mod'])) == set(r['name'] for r in S['rd']['refs'])
     and S['rd']['b524_lemma_refs'] == S['rd524']['lemma_refs']
     and S['rd']['lemmas_beyond_b524'] == [r for r in S['rd']['lemma_refs'] if r not in S['rd524']['lemma_refs']],
     lambda S: put(S, 'mod', S['mod'] + NL + 'theorem ghost := Zeta23.EF.ghost_lemma')),
    ('G-HYPOTHESIS-VALUE', 'the values bank, eps recomputed HERE from F and N, G against b522`s banked row',
     lambda S: len(S['bank34']) == 1 and abs(S['vals']['G'] - S['bank34'][0]['growth']) < 1e-12
     and all(abs(x - y) <= 1e-15 * max(1.0, abs(y)) for x, y in zip(eps_now(S['vals']), (S['vals']['eps_delta'], S['vals']['eps_zero'])))
     and abs(pair_eps(max(eps_now(S['vals'])), S['vals']['delta'], S['vals']['gamma0']) - S['vals']['pairEps']) < 1e-12
     and S['vals']['bound_holds_on_instance'] == (S['vals']['pairTwo_khat'][0] <= S['vals']['rhs']),
     lambda S: put(S, 'vals', dict(S['vals'], eps_delta=S['vals']['eps_delta'] * 10))),
    ('G-GRADES-BY-READING', 'this act`s correspondence row and trail record -- the grades of READING (8), nothing else graded',
     lambda S: all('`%s` DERIVES' % n in corr_row(S) for n in DERIVED) and '`pair_bound` INTERFACES' in corr_row(S)
     and 'farSmall STATED, no grade' in corr_row(S) and 'Component 3 INTERFACES on the named hypothesis' in trail(S)
     and sorted(set(re.findall(r'`(\w+)` (?:DERIVES|INTERFACES|SHELL|ENCODES)', corr_row(S)))) == sorted(DERIVED + ['pair_bound']),
     lambda S: put(S, 'corr', S['corr'].replace('`pair_bound` INTERFACES', '`pair_bound` DERIVES'))),
    ('G-F2-MARKED', 'the trail record`s re-printed table -- f2 marked, f3 and f4 OPEN unchanged',
     lambda S: '| (f2) | b529: Components 1 and 2 DERIVES; Component 3 INTERFACES on `farSmall`, not proved |' in seg(S['ot'], TRAILH, 99999)
     and '| (f3) | OPEN, unchanged |' in seg(S['ot'], TRAILH, 99999) and '| (f4) | OPEN, unchanged |' in seg(S['ot'], TRAILH, 99999),
     lambda S: put(S, 'ot', S['ot'].replace('| (f3) | OPEN, unchanged |', '| (f3) | DONE |'))),
    ('G-TABLE-ROW', 'the regenerated terminal table -- the six graded rows PROFILED and carrying their grade',
     lambda S: all(table_row(S, n) and 'DERIVES' in table_row(S, n) and 'NOT PROFILED' not in table_row(S, n) for n in DERIVED)
     and 'INTERFACES' in table_row(S, 'pair_bound') and 'NOT PROFILED' not in table_row(S, 'pair_bound'),
     lambda S: put(S, 'table', S['table'].replace('`%spair_bound`' % NS, '`ghost`'))),
    ('G-N1-SCORED', 'the desk against the attempts and the profile', lambda S: 'n1' in S['sc'] and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == (first_clean(S) is not None and first_clean(S) <= 3
                           and all((S['prof'].get('std3') or {}).get(NS + n) is True for n in C12)),
     lambda S: put(S, 'sc', dict(S['sc'], n1=not S['sc'].get('n1')))),
    ('G-N2-SCORED', 'the desk against eps recomputed HERE', lambda S: 'n2' in S['sc'] and desk_word(S, 'N2') == word_of(S['sc']['n2'])
     and S['sc']['n2'] == (eps_now(S['vals'])[0] < 0.1), lambda S: put(S, 'sc', dict(S['sc'], n2=not S['sc'].get('n2')))),
    ('G-N3-SCORED', 'the desk against the vendored references recomputed HERE', lambda S: 'n3' in S['sc'] and desk_word(S, 'N3') == word_of(S['sc']['n3'])
     and S['sc']['n3'] == all(r in S['rd524']['lemma_refs'] for r in vrefs(S['mod']) if r in S['rd']['lemma_refs']),
    ...
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from the banks',
     ...
     and desk_word(S, 'S1') == word_of(first_clean(S) is not None and first_clean(S) <= 2)
     and desk_word(S, 'S2') == word_of(len(S['prof'].get('std3') or {}) == 19 and all((S['prof'].get('std3') or {}).get(NS + n) is True for n in THMS))
     and desk_word(S, 'S3') == word_of(max(eps_now(S['vals'])) < 1e-3),
    ...
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 378 once, six cells as it landed',
     lambda S: S['corr'].count('| 378 |') == 1 and S['corr'].count('(b529, under (R139))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 378 | a duplicate row |')),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b529')
              and 'data/b529_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b529 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b529_checks_postpush.txt' if pushed else 'b529_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b529_exercise.json'), 'w', encoding='utf-8', newline=NL),
