# -*- coding: utf-8 -*-
"""b306_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b306_registration_2026-09-03.txt')
SPEC = os.path.join(ROOT, 'data', 'b306_satisfiable.json')
...
CLAUSES = [
    ("`.lean` files created", 0, 0, "files",
     ...
     " ### RE-MEASURED IN THE CLOSING BY `b306_checks.py`'s G-NOBUILD."),
    ...
    ("grades moved", 0, 0, "grades",
     "G13, and the order's own clause: in every branch of Component 4, no measurement is disturbed"
     " and no grade moves. ### RE-MEASURED BY `b306_checks.py`'s G-NOMOVE over this act's own"
     " emitted prose."),
    ("ancestors' correspondence rows rewritten", 0, 0, "rows",
     "G14 and the append-only law. ### The extended sweep found two ancestors' rows carrying a"
     " banned stem; ### **ONLY THIS SEAT'S OWN INDEX ROW WAS REPAIRED.** ### RE-MEASURED IN THE"
     " CLOSING BY DIFFING `CORRESPONDENCE.md` AGAINST `git HEAD` FOR PRE-EXISTING ROWS."),
]


def main(argv):
    print('=' * 100)
    print('b306_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration":
            "data/b306_registration_2026-09-03.txt -- b306, THE DIFFERENCE",
            "clauses": clauses}
