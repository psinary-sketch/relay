...
def ei_marks(rows):
    """### (R154)(4): the E/I mark of CP-3 (b544) as a column -- read from data/b544_ei.json; a row whose (repo, name) is a ZETA
    ### declaration there takes its mark, every other row prints the dash. The mark is read, never computed here."""
    p = os.path.join(D, 'b544_ei.json')
    marks = json.loads(read(p)).get('marks', {}) if os.path.exists(p) else {}
    by = {}
    for k, v in marks.items():
        repo, full = k.split('|', 1)
        by.setdefault(repo, []).append((full, v))
    for r in rows:
        hit = [v for full, v in by.get(r['repo'], []) if full == r['name'] or full.endswith('.' + r['name']) or r['name'].endswith('.' + full)]
        r['zeta_row'] = bool(hit)
        r['ei'] = hit[0] if hit else '—'
...
def emit(R):
    ...
    M = ['# TERMINAL TABLE', '',
         ...
         '| repo | name | pin | HEAD | statement | profile | grade | act:line | conflict | E/I (b544) |',
         '|:--|:--|:--|:--|:--|:--|:--|:--|:--|:--|']
    for r in sorted(rows, key=lambda x: (x['repo'], x['name'])):
        ...
        M.append('| `%s` | `%s` | %s | `%s` | `%s` | %s | **%s** | %s %s | %s | %s |'
                 % (r['repo'], r['name'], ('`%s`' % r['pin']) if r['pin'] else '*none cited*',
                    r['head'], st, r['profile'][:60], r['grade'], act, where, cf, r['ei']))
