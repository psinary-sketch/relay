# -*- coding: utf-8 -*-
"""b450_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b450_registration_2026-09-13.txt')
SPEC = os.path.join(ROOT, 'data', 'b450_satisfiable.json')
...
CLAUSES = [
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### section (W): mirror-refresh-2026-09-13-b450.zip.'),
    ...
    ('pre-face reads declared', 7, 7, 'reads', '### section (A): the survey (P1) to (P7).'),
    ('readings taken silently', 0, 0, 'readings', '### section (A).'),
    ('survey anchor misses', 0, 0, 'misses', '### section (A).'),
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### section (A): no keystone opened.'),
    ('author rulings ratified and entered', 0, 0, 'rulings', '### section (A): no new ruling.'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b450_addendum.txt.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (K) BAR 10.'),
    ('lanes opened', 0, 0, 'lanes', '### section (A): both instrument lanes parked.'),
    ...
    ('conditions added to the quoted rule and applied', 0, 0, 'conditions', '### (K) BAR 1: W1, W2 printed, not applied.'),
    ('correspondence tables written, extended or re-graded', 0, 0, 'tables', '### (K) BAR 4.'),
    ('lines removed from any keystone', 0, 0, 'lines', '### (K) BAR 5.'),
    ('registry rows edited', 0, 0, 'rows', '### section (Z).'),
    ('prose passages rewritten', 0, 0, 'passages', '### (K) BAR 4.'),
    ...
    ('rule differences printed', 2, 2, 'differences', '### section (A): D1, D2.'),
    ...
    ('corpus documents written other than by append, beyond version strings on currency-repaired lines', 0, 0, 'documents', '### (W) and reading (2).'),
...
def main(argv):
    print('=' * 100)
    print('b450_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b450_registration_2026-09-13.txt -- b450, "
                             "THE RECONCILIATION'S ELIGIBLE SET RE-MEASURED, AND THE BATCH READ"),
            "clauses": clauses}
