# -*- coding: utf-8 -*-
"""b508_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b507's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the fold's
### ### clauses are dropped, the instrument clauses set to zero, and the one computation this act runs counted.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b508_registration_2026-09-24.txt')
SPEC = os.path.join(ROOT, 'data', 'b508_satisfiable.json')
...
CLAUSES = [
    ...
    ('chains run, or channel values computed', 1, 1, 'runs', '### (B)(0): the pair split of the off-line part; nothing of either places side is recomputed.'),
    ('lean runs', 0, 0, 'runs', '### (Z).'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z).'),
    ('lanes opened', 1, 1, 'lanes', '### (R117)(4): the numerical lane for this act alone.'),
    ...
    ('candidate zeros, seeds, families or arrangements constructed', 0, 0, 'candidates', '### (Z): no zero added or moved.'),
    ('claims about RH, h2 or any zero beyond quoting a verified source', 0, 0, 'claims', '### section (Z).'),
    ('crossing widths asserted', 0, 0, 'widths', '### READING (6) and (Z): an extrapolation is printed and labelled; none asserted.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b508.zip, under (R69).'),
    ...
    ('ferry parts received', 1, 1, 'parts', '### section (A): part 2 of 2, received IN FULL at b507 and banked there.'),
    ...
    ('author rulings ratified and entered', 0, 0, 'rulings', '### (A): (R117) was entered at b507.'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b508_addendum.txt, under the stem glob.'),
    ...
    ('bars declared', 'BARS', 'BARS', 'bars', '### MEASURED: this face has no (K) section; its one fixture bar is in (B)(0) with its floor.'),
...
def main(argv):
    print('=' * 100)
    print('b508_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b508_extract.txt'),
    ...
    spec = {"registration": ("data/b508_registration_2026-09-24.txt -- b508, "
                             "THE EPSTEIN MARGIN READ ON A BANK THAT CLOSES"),
            "clauses": clauses}
