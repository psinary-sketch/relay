# -*- coding: utf-8 -*-
"""b428_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b428_registration_2026-09-12.txt')
SPEC = os.path.join(ROOT, 'data', 'b428_satisfiable.json')
...
CLAUSES = [
    ...
    ('candidates beyond the declared cap', 0, 0, 'candidates', '### (K) BAR 5: TWENTY-TWO.'),
    ...
    ('lanes opened', 0, 0, 'lanes', '### (K) BAR 13.'),
    ('external reads run', 0, 0, 'reads', '### (K) BAR 13.'),
    ('grades conferred on a proof this seat has not read', 0, 0, 'grades', '### (K) BAR 14.'),
    ('files of the external proof read at content', 0, 0, 'files', '### (K) BAR 10.'),
    ('claims that a counterexample exists or is findable', 0, 0, 'claims', '### section (Z).'),
    ('ledger-row quotations resolved against another row', 0, 0, 'quotations', '### (K) BAR 4.'),
...
def main(argv):
    print('=' * 100)
    print('b428_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b428_registration_2026-09-12.txt -- b428, SITE (iii) AND TWO FILINGS FROM OUTSIDE"),
            "clauses": clauses}
