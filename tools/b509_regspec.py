# -*- coding: utf-8 -*-
"""b509_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b508's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the numerical
### ### clauses are set to zero and the kernel clauses to the one module this act writes.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b509_registration_2026-09-24.txt')
SPEC = os.path.join(ROOT, 'data', 'b509_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): the new module in the kernel, and the print file in relay`s data directory.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): no `lake build`; `lake env lean` on one module.'),
    ('chains run, or channel values computed', 0, 0, 'runs', '### (Z): one regularity measurement on one window, no channel.'),
    ('lanes opened', 1, 1, 'lanes', '### (R118)(4): the kernel lane for this act alone.'),
    ...
    ('grades moved on any row of the corpus, or grade names minted', 0, 0, 'grades', '### section (Z): Component 3 grades one terminal against one claim, as ordered.'),
    ...
    ('prior acts` banks edited', 1, 1, 'files', '### (W) and (R118)(3): ONE appended note to b508_desk_notes.txt, prefix proved.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b509.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R118).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b509_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b509_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b509_extract.txt'),
    ...
    spec = {"registration": ("data/b509_registration_2026-09-24.txt -- b509, "
                             "(R104)'S ACT 3"),
            "clauses": clauses}
