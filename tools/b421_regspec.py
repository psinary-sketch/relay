# -*- coding: utf-8 -*-
"""b421_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b421_registration_2026-09-11.txt')
SPEC = os.path.join(ROOT, 'data', 'b421_satisfiable.json')
...
CLAUSES = [
    ...
    ('platform calls of any kind', 0, 0, 'calls', '### (K) BAR 10; section (Z).'),
    ('kernel .lean files touched unless the verdict of Component 1 is PROVED', 0, 0, 'files', '### (K) BAR 1.'),
    ('existing kernel .lean files edited', 0, 0, 'files', '### (K) BAR 1.'),
    ('terminals moved', 0, 0, 'terminals', '### (K) BAR 1.'),
    ...
    ('folds run', 0, 0, 'folds', '### (K) BAR 8.'),
    ('lines of FINDINGS.md written', 0, 0, 'lines', '### (K) BAR 8.'),
    ...
    ('objects the lemma relates, named in its statement', 2, 2, 'objects', '### (A) reading (1).'),
    ('probe compilations of Component 1', 16, None, 'probes', '### (A) reading (2): b418`s size, MEASURED.'),
    ('minutes of probing from the first probe', 150, None, 'minutes', '### (A) reading (2): MEASURED.'),
    ('probes inside a repository', 0, 0, 'probes', '### (A) reading (2).'),
    ('verdicts of Component 1', 1, 1, 'verdicts', '### (A) reading (2): exactly one of three.'),
    ('verdicts read from an exit code', 0, 0, 'verdicts', '### (K) BAR 2.'),
    ('sorry terminals', 0, 0, 'terminals', '### (K) BAR 2.'),
    ('Mathlib imports', 0, 0, 'imports', '### (K) BAR 6.'),
    ('probe profiles not banked', 0, 0, 'profiles', '### (K) BAR 5.'),
    ('bytes of the prior profile changed on PROVED', 0, 0, 'bytes', '### (K) BAR 3.'),
    ('AllPrints lines changed other than one inserted import and appended prints', 0, 0, 'lines', '### (K) BAR 3.'),
    ('definitions of determined read in Component 2', 2, 2, 'definitions', '### (A) reading (3).'),
    ('classical theorems cited as the record`s', 0, 0, 'citations', '### (K) BAR 7.'),
    ('prior verdicts re-verdicted', 0, 0, 'verdicts', '### (K) BAR 7.'),
    ('search hits of Component 2 left unread', 0, 0, 'hits', '### (A) reading (3).'),
    ('span counts printed', 2, 2, 'counts', '### (A) reading (4).'),
    ('conflations entered with their corrections', 3, 3, 'conflations', '### (A) reading (5).'),
    ('ferry paraphrases deciding over their source', 0, 0, 'paraphrases', '### (K) BAR 9.'),
...
def main(argv):
    print('=' * 100)
    print('b421_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b421_registration_2026-09-11.txt -- b421, THE FINITE AMBIENT BUILT OR "
                             "PRICED, AND THE SPAN COUNTED"),
            "clauses": clauses}
