# -*- coding: utf-8 -*-
"""b426_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b426_registration_2026-09-11.txt')
SPEC = os.path.join(ROOT, 'data', 'b426_satisfiable.json')
...
CLAUSES = [
    ...
    ('query strings fixed on this face', 3, 3, 'queries', '### (A) reading (3): A1 to A3.'),
    ('sources fetched beyond the cap', 0, 0, 'sources', '### (K) BAR 5: ONE address.'),
    ('sources quoted before their text is on disk', 0, 0, 'sources', '### (K) BAR 4.'),
    ('fits, samples or likelihoods computed', 0, 0, 'computations', '### (K) BAR 6.'),
    ('parts of the tension stated', 3, 3, 'parts', '### (A) reading (5): (a), (b), (c).'),
    ('author rulings ratified by this paste', 2, 2, 'rulings', '### section (A): (R38) and (R39).'),
    ('rulings extended beyond their own words', 0, 0, 'rulings', '### section (A).'),
    ('lane documents edited', 1, 1, 'documents', '### (W) KIND 5: FORMATION_DISTANCE.md, one inserted line.'),
    ('register rows whose status gains a note', 1, 1, 'rows', '### (W) KIND 6: p2-d6, per (R39).'),
    ('bytes of FORMATION_DISTANCE.md line 160 changed', 0, 0, 'bytes', '### (K) BAR 9.'),
    ('cells of the p2-d6 row changed other than its status cell', 0, 0, 'cells', '### (K) BAR 9.'),
    ('corpus writes made without a preservation record first', 0, 0, 'writes', '### (K) BAR 9.'),
    ('answers of the rule`s two clauses suppressed', 0, 0, 'answers', '### (K) BAR 13.'),
    ('significances converted, interpolated or inferred', 0, 0, 'figures', '### (K) BAR 6.'),
...
def main(argv):
    print('=' * 100)
    print('b426_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b426_registration_2026-09-11.txt -- b426, SORTIE LEG 1, THE FALSIFIER RE-READ BY ADDRESS"),
            "clauses": clauses}
