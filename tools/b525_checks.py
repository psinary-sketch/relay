# -*- coding: utf-8 -*-
"""b525_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b523's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b525_registration_2026-09-25.txt')
FIND = os.path.join(PP, 'FINDINGS.md')
DIG = os.path.join(PP, 'phase2', 'method', 'THE_FINDINGS_AS_THEY_STAND.md')
OT = os.path.join(PP, 'OPEN_TRAILS.md')
PRIOR_RELAY = '66b4939a'      # ### b524's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
KER_TIP = 'ffc09da'           # ### the kernel's tip, untouched by this act
PRIOR_PP = '9272b57'           # ### b524's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b525 —'
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b525_ferry.txt')),
        scan=read(os.path.join(D, 'b525_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b525_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b525_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b525_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b525_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b524_closing.txt')),
        addendum=read(os.path.join(D, 'b525_addendum.txt')),
        comp=read(os.path.join(D, 'b525_components.txt')),
        desk=read(os.path.join(D, 'b525_desk_notes.txt')),
        report=read(os.path.join(D, 'b525_report.txt')),
        fold=json.loads(read(os.path.join(D, 'b525_fold.json')) or '{}'),
        span=json.loads(read(os.path.join(D, 'b525_span.json')) or '{}'),
        runlog=read(os.path.join(D, 'b525_run_log.txt')),
        ...
        res=json.loads(read(os.path.join(D, 'b525_results.json')) or '{}'),
        sc=json.loads(read(os.path.join(D, 'b525_scores.json')) or '{}'),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b525_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b525.zip'))),
        ...
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b525 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b525_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (os.path.join(D, x) for x in ('b525_fold.json', 'b525_run_log.txt'))),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b525 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[0-9]_|^b52[0-4]_|^b334_', f)]
...
def xi_form(r134):
    ...
    return ' '.join(r134[i + 1:j - 1].split())


def corr_row(S):
    return next((l for l in S['corr'].split(NL) if l.startswith('| 374 |')), '')


ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R135) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b525' in S['ferry'],
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b524`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 373 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 373 |', '| 3730 |'))),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b525' in S['ferry'] and 'ACT b525' in S['face'] and not glob.glob(os.path.join(D, 'b526_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b525')),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b525 -'))),
    ...
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 374 once, six cells as it landed',
     lambda S: S['corr'].count('| 374 |') == 1 and S['corr'].count('(b525, under (R135))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 374 | a duplicate row |')),
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b524`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b519_window.py'])),
    ('G-WRITELIST-KINDS', 'every b525 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b525')" in S['suite']
                and "data/b525_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b525_components.txt' in gits(ROOT, 'show'")),
    ('G-PEEK-DECLARED', 'the face`s (C) block AND the fold record`s time against the lock',
     lambda S: 'THE FOLD TOOL IS WRITTEN BEFORE THE SEAL AND NOT RUN' in flat(S['face']) and S['after_lock'],
     lambda S: put(S, 'after_lock', False)),
    ('G-R135-ENTERED', 'the banked ferry AND the trail', lambda S: 'RULING (R135) END' in S['ferry'] and S['ot'].count('**(R135) ratified') == 1,
     lambda S: put(S, 'ot', S['ot'].replace('**(R135) ratified', '(R135) noted'))),
    ('G-SPAN-READ', 'the fold record against the span record -- b517 to b524, eight acts, the tool reading nine',
     ...
     and S['span']['span_starts_at'] == 517 and S['span']['this_act'] == 525,
     lambda S: put(S, 'fold', dict(S['fold'], span=dict((S['fold'].get('span') or {}), lo=516)))),
    ('G-QUOTES-MATCHED', 'every quoted verdict re-matched HERE in its own closing bank',
     lambda S: len(S['fold'].get('quotes') or []) == 16 and all(fnorm(q['quote']) in fnorm(read(os.path.join(D, 'b%d_closing.txt' % q['act'])))
                                                                 for q in S['fold']['quotes'])
     and sorted(set(q['act'] for q in S['fold']['quotes'])) == list(range(517, 525)),
     lambda S: put(S, 'fold', dict(S['fold'], quotes=S['fold']['quotes'][:-1] + [dict(act=517, quote='A VERDICT NO BANK PRINTS', found=True)]))),
    ('G-DEFECTS-MATCHED', 'every act`s defect count recounted HERE from its defects bank, and the total the section prints',
     lambda S: all(d['n'] == len(re.findall(r'^    \([a-z]\) ###', read(os.path.join(D, 'b%d_defects.txt' % d['act'])), re.M))
                   for d in S['fold'].get('defects') or [])
     and len(S['fold'].get('defects') or []) == 8 and ('%d defects' % sum(d['n'] for d in S['fold']['defects'])) in sec(S),
     lambda S: put(S, 'fold', dict(S['fold'], defects=[dict(d, n=d['n'] + 1) for d in S['fold']['defects']]))),
    ('G-RULINGS-MATCHED', 'each ruling`s BEGIN and END re-matched HERE in the ferry named',
     lambda S: [r['ruling'] for r in S['fold'].get('rulings') or []] == list(range(126, 136))
    ...
    ('G-ERRORS-BLOCK', 'the section -- the refuted growth claim, the corrected form of words, the instance discrepancy, each quoted',
     lambda S: 'refuted by the record and marked so' in flat(sec(S)) and xi_form(S['r134']) in flat(sec(S))
     and 'the theorems cover both and the INSTANCES DIFFER' in flat(sec(S)),
     lambda S: put(S, 'ft', S['ft'].replace('INSTANCES DIFFER', 'instances agree'))),
    ...
    ('G-FOLD-ADDITIVE', 'FINDINGS against its blob at b524`s commit -- a true prefix, one heading',
     lambda S: S['ft_prefix'] and S['ft'].count(S['fold'].get('heading', '#none#')) == 1,
     lambda S: put(S, 'ft_prefix', False)),
    ('G-DIGEST-ADDITIVE', 'the digest against its blob at b524`s commit -- a true prefix, one marker',
     lambda S: S['dg_prefix'] and S['dg'].count('<!-- b525 orientation refresh') == 1,
     lambda S: put(S, 'dg_prefix', False)),
    ('G-WORKORDERS-CARRIED', 'the section`s work-order table -- two rows, each with its trigger, (f)(i) done and f2-f4 open',
     lambda S: sec(S).count('| `W-ORD-') == 2 and '| `W-ORD-XI-P7` |' in sec(S)
     and '(f)(i) = f1 DONE at b524; f2, f3, f4 OPEN' in sec(S) and '(R135)(3)(a)' in sec(S),
     lambda S: put(S, 'ft', S['ft'].replace('f2, f3, f4 OPEN', 'f2 OPEN'))),
    ('G-HOST-STOP-FILED', 'the section`s filed block -- (R134)(2)`s rule',
     lambda S: 'a host stop is not an attempt' in sec(S) and 'STOPPED BY HOST, attempt 0' in sec(S),
     lambda S: put(S, 'ft', S['ft'].replace('a host stop is not an attempt', 'a host stop counts'))),
    ('G-INSTANCES-DIFFER-SAID', 'the section`s errors block AND the trail -- the smooth bump against the order-7 B-spline',
     lambda S: 'the kernel’s named instance is Mathlib’s smooth bump' in flat(sec(S)) and 'the instances differ' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace('the instances differ', 'the instances agree'))),
    ('G-N1-SCORED', 'the desk against the span record', lambda S: 'n1' in S['sc'] and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == (S['span']['span_starts_at'] == 517 and S['span']['this_act'] - 1 == 524),
    ...
    ('G-N3-SCORED', 'the desk against the section', lambda S: 'n3' in S['sc'] and desk_word(S, 'N3') == word_of(S['sc']['n3'])
     and S['sc']['n3'] == ('| `W-ORD-XI-P7` |' in sec(S) and '(f)(i) = f1 DONE at b524; f2, f3, f4 OPEN' in sec(S)),
    ...
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from the banks',
     ...
     and desk_word(S, 'S1') == word_of(S['runlog'].split('### tools/b525_fold.py write')[0].count('**CHECK : PASS**') == 1)
     and desk_word(S, 'S2') == word_of(sum(d['n'] for d in S['fold']['defects']) == 30)
     and desk_word(S, 'S3') == word_of(sec(S).split('### The three columns')[0].count('*BORDERLINE --') == 4),
     lambda S: put(S, 'desk', S['desk'].replace('**(S1)** ### **', '**(S1)** ### **NOT'))),
    ('G-NOGRADE-CONFERRED', 'this act`s trail record and row -- one grade word, b524`s restated, and the row NO GRADE',
     lambda S: GRADE_RE.findall(trail(S)) == ['DERIVES'] and 'DERIVES by statement-read' in trail(S) and '| NO GRADE |' in corr_row(S),
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b525')
              and 'data/b525_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b525 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b525_checks_postpush.txt' if pushed else 'b525_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b525_exercise.json'), 'w', encoding='utf-8', newline=NL),
