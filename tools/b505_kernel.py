# -*- coding: utf-8 -*-
"""b505_kernel.py -- COMPONENT 3: b326`S DERIVED KERNEL ON THE COMPLETED Q0 BANK, AT THE WIDE CELLS (a >= 5).

### `python tools/b505_kernel.py run | price`
### ### RUNS ONLY IF COMPONENT 2 CLOSED THE BANK: its whole-strip count equal to the located count, every box near
### an integer, the positive control recovered, and every located zero confirmed by route A. Otherwise it refuses.
...
NL = chr(10)
TQ = 150.0
CELLS = os.path.join(D, 'b505_cells.jsonl')
C2 = os.path.join(D, 'b505_c2_results.json')
...
def bank_complete():
    ...
    ok = (r['not_near'] == 0 and all(r['control']) and all(z.get('inside') for z in r['found'])
          and all(z.get('route_a', 1) < 1e-10 for z in found) and len(found) == len(r['found'])
...
def price():
    ...
    A = wide()
    t0 = time.time()
    cell(A[0], K, np.array(CH.GQ), [])
    t1 = time.time()
    cell(A[-1], K, np.array(CH.GQ), [])
    t2 = time.time()
    print('wide cells %d ; first %.1f s ; last %.1f s ; priced %.0f min' % (len(A), t1 - t0, t2 - t1, len(A) * (t2 - t0) / 2 / 60))


if __name__ == '__main__':
    sys.exit({'run': run, 'price': price}[sys.argv[1]]())
