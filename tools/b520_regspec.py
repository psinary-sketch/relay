# -*- coding: utf-8 -*-
"""b520_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b518's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the
### ### numerical clauses re-read for this act's widening of variant (B); no work-order is filed.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b520_registration_2026-09-24.txt')
SPEC = os.path.join(ROOT, 'data', 'b520_satisfiable.json')
...
CLAUSES = [
    ...
    ('chains run, or channel values computed', 1, 1, 'runs', '### (B)(1)-(2): the chain on variant (B) widened past the ladder, xi and Q0, at the ladder`s widths.'),
    ('lean runs', 0, 0, 'runs', '### (Z).'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z).'),
    ('lanes opened', 1, 1, 'lanes', '### (R129)(3): the numerical lane for this act alone.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b520.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R129).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b520_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b520_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b520_extract.txt'),
    ...
    spec = {"registration": ("data/b520_registration_2026-09-24.txt -- b520, "
                             "VARIANT (B) WIDENED PAST THE LADDER"),
            "clauses": clauses}
