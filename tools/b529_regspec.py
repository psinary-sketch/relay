# -*- coding: utf-8 -*-
"""b529_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b527's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the kernel
### ### clauses count the two NEW kernel files; the grades clause counts Components 1, 2 and 3; farSmall takes none.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b529_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b529_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): PairTerm.lean and AxiomCheckPair.lean in the kernel, both NEW.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): no `lake build`; `lake env lean` on one module.'),
    ('chains run, or channel values computed', 1, 1, 'runs', '### READING (7): b529_values.py evaluates the pair`s term on b522`s instance (two khat values) beside the bound; no chain run.'),
    ('lanes opened', 1, 1, 'lanes', '### (R139)(4): the kernel lane for this act alone.'),
    ...
    ('grades conferred on Components 1, 2 and 3, by statement-read', 3, 3, 'grades', '### READING (8): C1 and C2 DERIVES, C3 INTERFACES on farSmall; farSmall takes none.'),
    ('grades moved on any other row of the corpus, or grade names minted', 0, 0, 'grades', '### section (Z): the three grades are this act`s own rows.'),
    ('kernel tags made', 0, 0, 'tags', '### section (Z).'),
    ('vendored modules edited', 0, 0, 'modules', '### READING (2).'),
    ('props stated and proved', 0, 0, 'props', '### READING (5): farSmall is stated, not proved.'),
    ('work-orders filed', 0, 0, 'work-orders', '### section (A): W-ORD-BSPLINE-INSTANCE is filed by (R139)(3), the author`s; the seat files none.'),
    ('premises discharged', 0, 0, 'premises', '### section (Z).'),
    ('routes proposed', 0, 0, 'routes', '### section (Z).'),
    ('claims about RH, h2 or any zero beyond quoting a verified source', 0, 0, 'claims', '### section (A): a bound on a transform under a named hypothesis is not a claim about any zero.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b529.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R139).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b529_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b529_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b529_extract.txt'),
    ...
    spec = {"registration": ("data/b529_registration_2026-09-25.txt -- b529, "
                             "(f)(ii), THE PAIR`S TERM, AS FAR AS IT DERIVES"),
            "clauses": clauses}
