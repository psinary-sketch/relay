# -*- coding: utf-8 -*-
"""b510_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b509's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the kernel
### ### clauses count the two kernel files and the print file this act writes; the strike clause is dropped.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b510_registration_2026-09-24.txt')
SPEC = os.path.join(ROOT, 'data', 'b510_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 3, 3, 'files', '### (W): H2Sign.lean and AxiomCheck.lean in the kernel, and the print file in relay`s data directory.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): no `lake build`; `lake env lean` on one module.'),
    ('chains run, or channel values computed', 0, 0, 'runs', '### (Z): none; (b) is priced, not taken.'),
    ('lanes opened', 1, 1, 'lanes', '### (R119)(4): the kernel lane for this act alone.'),
    ...
    ('work-orders filed', 1, 1, 'work-orders', '### (R119)(2): W-ORD-PL-CLASS, both dispositions priced, neither taken.'),
    ('dispositions taken', 0, 0, 'dispositions', '### (R119)(2).'),
    ('theorems proved beyond h2_sign_imp_aim', 0, 0, 'theorems', '### the ferry.'),
    ...
    ('keystones edited or annotated', 0, 0, 'keystones', '### (W) and READING (7).'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b510.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R119).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b510_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b510_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b510_extract.txt'),
    ...
    spec = {"registration": ("data/b510_registration_2026-09-24.txt -- b510, "
                             "(R104)'S ACT 4"),
            "clauses": clauses}
