# -*- coding: utf-8 -*-
"""b543_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b541's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): no kernel file;
### ### no ERRATA entry filed; two PLACE-papers documents appended to.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b543_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b543_satisfiable.json')
...
CLAUSES = [
    ...
    ('kernel builds', 0, 0, 'builds', '### (Z): the #check of READING (7) is not a build.'),
    ...
    ('branches merged or fetched', 0, 0, 'branches', '### READING (6): two branch refs are deleted by name, none merged or fetched.'),
    ...
    ('ERRATA entries written', 0, 0, 'entries', '### READING (4): E-2026-09-25-6 is drafted in relay, not filed.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R153).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b543_addendum.txt, under the stem glob.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('PLACE-papers documents written other than by appending', 0, 0, 'documents', '### READING (5): nothing is written at FINDINGS.md:1109.'),
...
def main(argv):
    print('=' * 100)
    print('b543_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b543_extract.txt'),
    ...
    spec = {"registration": ("data/b543_registration_2026-09-25.txt -- b543, "
                             "THE MONOGRAPH READ, ACT TWO, UNDER (R153)"),
            "clauses": clauses}
