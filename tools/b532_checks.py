# -*- coding: utf-8 -*-
"""b532_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b532's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b532_registration_2026-09-25.txt')
OT = os.path.join(PP, 'OPEN_TRAILS.md')
PRIOR_RELAY = '5ac84ce6'      # ### b531's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
KER_TIP = '2f88aa8'           # ### the kernel's tip before this act
PRIOR_PP = '5263dd3'           # ### b531's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b532 —'
...
def sources():
    ...
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b532_ferry.txt')),
        scan=read(os.path.join(D, 'b532_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b532_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b532_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b532_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b532_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b531_closing.txt')),
        addendum=read(os.path.join(D, 'b532_addendum.txt')),
        comp=read(os.path.join(D, 'b532_components.txt')),
        desk=read(os.path.join(D, 'b532_desk_notes.txt')),
        se=json.loads(read(os.path.join(D, 'b532_sentences.json')) or '{}'),
        rows=json.loads(read(os.path.join(D, 'b532_rows.json')) or '{}'),
        erratum=read(os.path.join(D, 'b532_erratum_draft.md')),
        errata=read(os.path.join(PP, 'ERRATA.md')),
        mono_md5=_h.md5(open(os.path.join(PP, 'outputs', 'DEPOSITED-v1.1.2', 'A_Place_to_Stand.md'), 'rb').read()).hexdigest(),
        mono=re.sub(r'\s+', ' ', read(os.path.join(PP, 'outputs', 'DEPOSITED-v1.1.2', 'A_Place_to_Stand.md'))),
        f1=json.loads(read(os.path.join(D, 'b359_fetch_F1.json')) or '{}'),
        mod=read(os.path.join(KER, 'SIDEExplicitFormula', 'H2Bridge.lean')),
        acc=read(os.path.join(KER, 'AxiomCheckBridge.lean')),
        att=json.loads(read(os.path.join(D, 'b532_attempts.json')) or '[]'),
        att_files={int(re.search(r'attempt(\d+)\.lean$', p).group(1)): hashlib.sha256(open(p, 'rb').read()).hexdigest()
                   for p in glob.glob(os.path.join(D, 'b532_attempt[0-9]*.lean'))},
        clogs=len(glob.glob(os.path.join(D, 'b532_compile_log*.txt'))),
        clast=read(sorted(glob.glob(os.path.join(D, 'b532_compile_log*.txt')), key=os.path.getmtime)[-1]),
        plog=read(sorted(glob.glob(os.path.join(D, 'b532_profile_log*.txt')), key=os.path.getmtime)[-1]),
        inst=json.loads(read(os.path.join(D, 'b532_install.json')) or '{}'),
        prof=json.loads(read(os.path.join(D, 'b532_profile.json')) or '{}'),
        stmt=read(os.path.join(D, 'b532_statement.txt')),
        probes=read(os.path.join(D, 'b532_probe1.txt')) + read(os.path.join(D, 'b532_probe2.txt')),
        fl_now=open(os.path.join(PP, 'FACES_LEDGER.md'), 'rb').read().replace(b'\r', b''),
        fl_prior=blob(PP, PRIOR_PP + ':FACES_LEDGER.md'),
        ...
        sc=json.loads(read(os.path.join(D, 'b532_scores.json')) or '{}'),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b532_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b532.zip'))),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b532 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b532_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (os.path.join(D, x) for x in ('b532_install.json', 'b532_attempts.json', 'b532_profile.json', 'b532_rows.json', 'b532_erratum_draft.md'))),
        probe_before=all(os.path.getmtime(os.path.join(D, x)) < os.path.getmtime(FACE) for x in ('b532_probe1.txt', 'b532_probe2.txt')),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b532 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[0-9]_|^b52[0-9]_|^b53[01]_|^b334_', f)]
...
ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R142) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b532' in S['ferry'],
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b532' in S['ferry'] and 'ACT b532' in S['face'] and not glob.glob(os.path.join(D, 'b533_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b532')),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b532 -'))),
    ...
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b531`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b519_window.py'])),
    ('G-WRITELIST-KINDS', 'every b532 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b532')" in S['suite']
                and "data/b532_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b532_components.txt' in gits(ROOT, 'show'")),
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b531`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 380 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 380 |', '| 3800 |'))),
    ('G-PEEK-DECLARED', 'the face`s (C) block, the probes` times before the lock, the components` after it',
     lambda S: 'THE TWO PROBES OF READING (5) WERE RUN BEFORE THE SEAL' in flat(S['face']) and S['probe_before'] and S['after_lock'],
     lambda S: put(S, 'after_lock', False)),
    ('G-R142-ENTERED', 'the banked ferry AND the trail', lambda S: 'RULING (R142) END' in S['ferry'] and S['ot'].count('**(R142) ratified') == 1,
     lambda S: put(S, 'ot', S['ot'].replace('**(R142) ratified', '(R142) noted'))),
    ('G-MD5-MATCHED', 'the monograph`s bytes, hashed HERE, against the checksum b359 fetched from Zenodo',
     lambda S: 'md5:' + S['mono_md5'] == next((f['checksum'] for f in S['f1'].get('files', []) if f.get('key') == 'A_Place_to_Stand.md'), ''),
     lambda S: put(S, 'mono_md5', '0' * 32)),
    ('G-EXTRACT-RECOMPUTED', 'every monograph row`s sentence found HERE in the monograph`s own text',
     lambda S: all(r['text'][:80] in S['mono'] for r in S['se']['rows'] if r['rec'] == 'M') and len([r for r in S['se']['rows'] if r['rec'] == 'M']) > 0,
     lambda S: put(S, 'se', dict(S['se'], rows=[dict(r, text='ghost ' + r['text']) for r in S['se']['rows']]))),
    ('G-EVERY-ROW-VERDICTED', 'the extract against the verdict bank -- one verdict per row, each in the vocabulary',
     lambda S: [r['id'] for r in S['se']['rows']] == [r['id'] for r in S['rows']['rows']]
     and all(r['verdict'] in ('STANDS', 'RESTS', 'EXCEEDS') and r['reason'] for r in S['rows']['rows']),
     lambda S: put(S, 'rows', dict(S['rows'], rows=S['rows']['rows'][:-1]))),
    ('G-RESTS-HAVE-REPLACEMENTS', 'the verdict bank -- every RESTS row carries a replacement naming the erratum',
     lambda S: bool(rests(S)) and all(EID in r['replacement'] for r in S['rows']['rows'] if r['verdict'] == 'RESTS'),
     lambda S: put(S, 'rows', dict(S['rows'], rows=[dict(r, replacement='') for r in S['rows']['rows']]))),
    ('G-ERRATUM-DRAFTED', 'the draft -- its id, DRAFT and NOT FILED, both lemmas, the kernel`s words, every RESTS row',
     lambda S: EID in S['erratum'] and 'DRAFT' in S['erratum'] and 'NOT FILED' in S['erratum']
     and 'structural_exhaustiveness_from_rh' in S['erratum'] and 'balance_theorem' in S['erratum']
     and '<-> s = 1 / 2' in S['erratum'] and all('`%s`' % r in S['erratum'] for r in rests(S)),
     lambda S: put(S, 'erratum', S['erratum'].replace('NOT FILED', 'FILED'))),
    ('G-ERRATA-UNWRITTEN', 'ERRATA.md itself -- the id absent; the PLACE-papers commit does not touch it',
     lambda S: EID not in S['errata'] and 'ERRATA.md' not in S['tracked'],
     lambda S: put(S, 'errata', S['errata'] + NL + EID)),
    ('G-INSTALLED-FROM-DRAFT', 'the install bank against attempt 1`s banked source -- the sealed draft first ran, onto no prior file',
     lambda S: S['inst']['H2Bridge.lean']['prior_sha'] is None and S['inst']['AxiomCheckBridge.lean']['prior_sha'] is None
     and S['inst']['H2Bridge.lean']['draft_sha'] == S['inst']['H2Bridge.lean']['installed_sha'] == S['att'][0]['source_sha'] == S['att_files'].get(1),
     lambda S: put(S, 'inst', {**S['inst'], 'H2Bridge.lean': dict(S['inst']['H2Bridge.lean'], draft_sha='0' * 64)})),
    ...
    ('G-PROFILE-WHOLE-STRING', 'the profile run file -- each theorem`s line equal to the standard-three string',
     ...
     lambda S: put(S, 'plog', S['plog'].replace(STD3 % (NS + 'ch_iff_rh'), (STD3 % (NS + 'ch_iff_rh')).replace(', Quot.sound', ', sorryAx')))),
    ('G-STATEMENTS-PRINTED', 'the banked statements against the module -- each declaration from its line to its `:=`, recomputed',
     lambda S: bool(heads(S['mod'])) and all(h in S['stmt'] for h in heads(S['mod'])),
     lambda S: put(S, 'stmt', S['stmt'].replace('theorem ch_iff_rh', 'theorem ghost'))),
    ('G-PROBES-BANKED', 'the two banked probes -- sources and outputs; no theorem; one declared tactic example',
     lambda S: S['probes'].count('### output') == 2 and S['probes'].count('#check @') >= 10 and not re.search(r'^theorem ', S['probes'], re.M)
     and S['probes'].count('example (') == 1 and 'rw [h]' in S['probes'],
     lambda S: put(S, 'probes', S['probes'] + NL + 'theorem x : True := trivial')),
    ('G-FACES-NOTE-APPENDED', 'FACES_LEDGER on disk against its blob at b531`s commit -- a true prefix, one note citing both lemmas',
     lambda S: S['fl_prior'] and S['fl_now'].startswith(S['fl_prior'].replace(b'\r', b'')) and S['fl_now'].count(b'NOTE ON ROW R2') == 1
    ...
    ('G-CONFLICT-ANNOTATED', 'the trail, the row and the regenerated table -- the CONFLICT read as two claims under (R40)',
     ...
     and 'CONFLICT' in table_row_any(S, 'ConservationBridge.riemann_hypothesis'),
     lambda S: put(S, 'corr', S['corr'].replace('two claims under (R40)', 'one claim'))),
    ('G-N1-SCORED', 'the desk against the verdict bank', lambda S: 'n1' in S['sc'] and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == (len(rests(S)) >= 5 and any(r in rests(S) for r in ('M-20', 'M-21', 'M-22'))),
     lambda S: put(S, 'sc', dict(S['sc'], n1=not S['sc'].get('n1')))),
    ('G-N2-SCORED', 'the desk against the verdict bank`s Z21520474 rows', lambda S: 'n2' in S['sc'] and desk_word(S, 'N2') == word_of(S['sc']['n2'])
     and S['sc']['n2'] == any(r.startswith('Z21520474') for r in rests(S)),
     lambda S: put(S, 'sc', dict(S['sc'], n2=not S['sc'].get('n2')))),
    ('G-N3-SCORED', 'the desk against the attempts and the profile', lambda S: 'n3' in S['sc'] and desk_word(S, 'N3') == word_of(S['sc']['n3'])
     and S['sc']['n3'] == (first_clean(S) is not None and first_clean(S) <= 2 and all((S['prof'].get('std3') or {}).get(NS + n) is True for n in THMS)),
    ...
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from the banks',
     ...
     and desk_word(S, 'S2') == word_of((S['prof'].get('std3') or {}).get(NS + 'ch_iff_rh') is True)
     and desk_word(S, 'S3') == word_of(bool(S['fl_prior']) and S['fl_now'].startswith(S['fl_prior'].replace(b'\r', b''))),
     lambda S: put(S, 'desk', S['desk'].replace('**(S1)** ### **', '**(S1)** ### **NOT'))),
    ('G-CORPUS-SCOPE', 'the PLACE-papers file list -- the trail and the faces ledger; the kernel`s changes the two NEW files',
     lambda S: sorted(S['tracked']) == ['FACES_LEDGER.md', 'OPEN_TRAILS.md'] and S['ker_changed'] == NEWK and S['ker_tags'] == '',
     lambda S: put(S, 'ker_changed', NEWK | {'Zeta23/Defs.lean'})),
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 381 once, six cells as it landed',
     lambda S: S['corr'].count('| 381 |') == 1 and S['corr'].count('(b532, under (R142))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 381 | a duplicate row |')),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b532')
              and 'data/b532_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b532 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b532_checks_postpush.txt' if pushed else 'b532_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b532_exercise.json'), 'w', encoding='utf-8', newline=NL),
