# -*- coding: utf-8 -*-
"""b369_desk.py -- THE DESK, SWEPT ONCE MORE UNDER THE FRESHNESS RULE. ### **MARKS ONLY.**
...
def newest_confirming(pat, needle):
    ...
    for p in sorted(G.glob(os.path.join(ROOT, pat.replace('/', os.sep)))):
        if os.path.basename(p).startswith('b369_'):
            continue
        try:
...
def main():
    rec('=' * 100)
    rec('b369 -- THE DESK, SWEPT UNDER THE FRESHNESS RULE. ### **MARKS, NOT VERDICTS.**')
    rec('=' * 100)
    rec('')
    ...
    if not rule_ok:
        ...
        run_clock.write(D, 'b369_desk_notes', LINES)
        return 2
    ...
    rec('    ### ### ### **AND THAT IS THE SECOND TIME THIS RESULT HAS BEEN REPORTED WITH ITS OWN')
    rec('    ### ### ### CAVEAT AND THE FIRST TIME IT HAS BEEN REPORTED TWICE.** ### A sweep whose')
    rec('    ### result never changes and whose caveat never changes is ### **A MEASUREMENT NOBODY IS')
    rec('    ### ### USING.** ### `b369`s own draft asks the next act to re-verify ONE item properly;')
    rec('    ### until an act does, this mark is a shape and not news.')
    rec('=' * 100)
    p = run_clock.write(D, 'b369_desk_notes', LINES)
    io.open(os.path.join(D, 'b369_desk.json'), 'w', encoding='utf-8', newline=chr(10)).write(json.dumps(
