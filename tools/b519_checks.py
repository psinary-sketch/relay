# -*- coding: utf-8 -*-
"""b519_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b518's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b519_registration_2026-09-24.txt')
OT = os.path.join(PP, 'OPEN_TRAILS.md')
PRIOR_RELAY = 'a9147ab8'      # ### b518's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
KER_TIP = 'ffc09da'           # ### the kernel's tip, untouched by this act
PRIOR_PP = '59d0685'           # ### b518's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b519 —'
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b519_ferry.txt')),
        scan=read(os.path.join(D, 'b519_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b519_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b519_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b519_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b519_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b518_closing.txt')),
        addendum=read(os.path.join(D, 'b519_addendum.txt')),
        comp=read(os.path.join(D, 'b519_components.txt')),
        desk=read(os.path.join(D, 'b519_desk_notes.txt')),
        report=read(os.path.join(D, 'b519_report.txt')),
        cells={v: sorted(jsonl(os.path.join(D, 'b519_cells_%s.jsonl' % v)), key=lambda c: c['a']) for v in ('A', 'B', 'C')},
        r518=json.loads(read(os.path.join(D, 'b518_results.json')) or '{}'),
        ladder=sorted(c['a'] for c in jsonl(os.path.join(D, 'b511_cells.jsonl')) if c.get('family') == 'bspline'),
        res=json.loads(read(os.path.join(D, 'b519_results.json')) or '{}'),
        fx=json.loads(read(os.path.join(D, 'b519_fixture.json')) or '[]'),
        mt=json.loads(read(os.path.join(D, 'b519_matched.json')) or '[]'),
        sc=json.loads(read(os.path.join(D, 'b519_scores.json')) or '{}'),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b519_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b519.zip'))),
        chain_clean=(gits(ROOT, 'status', '--porcelain', '--', 'tools/b321_window.py', 'tools/e16/carto_atlas.py',
                          ...
                          'tools/b504_chain.py', 'tools/b506_kernel.py', 'tools/b511_families.py', 'tools/b503_zero.py', 'tools/b514_window.py', 'tools/b515_window.py', 'tools/b518_window.py') == ''),
        ker_clean=(gits(KER, 'rev-parse', '--short=7', 'HEAD') == KER_TIP and gits(KER, 'status', '--porcelain', '--untracked-files=no') == ''),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b519 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b519_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (os.path.join(D, x) for x in ('b519_fixture.json', 'b519_matched.json', 'b519_cells_A.jsonl', 'b519_cells_B.jsonl', 'b519_cells_C.jsonl'))),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b519 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[0-8]_|^b334_', f)]
...
CRIT = 'h2_sign is Weil`s positivity criterion on classK, and the record says so in those words'.replace('`', "'")
...
def phihat(v, a, z, m=None):
    """### the base bump's transform recomputed HERE from READING (1), sharing no code with the act's tool."""
    L = math.log(a)
    if v == 'A':
        s = 2.0 * L / 10.0
        return csinc(z * s / 2.0) ** 10
    mm = 5 if v == 'B' else 9
    R = 0.25 * L / 2.0
    W = L - R
    return csinc(z * W) * csinc(z * (2.0 * R / mm) / 2.0) ** mm


def hhat(v, obj, a, gs, z):
    g0 = gs[0]
    p = 1.0
    for g in gs:
        p = p * (g * g - z * z)
    return p * 0.5 * (phihat(v, a, z - g0) + phihat(v, a, z + g0))


def objs(S):
    return [(v, c, o) for v in VARIANTS for c in S['cells'][v] for o in ('q', 'xi')]


def recount(S, v, o):
    C = S['cells'][v]
    V = [c for c in C if abs(c[o]['r']) <= c[o]['B']]
    return dict(verified=len(V), h2_neg=[c['a'] for c in V if c[o]['h2'] < -c[o]['B']],
...
def corr_row(S):
    return next((l for l in S['corr'].split(NL) if l.startswith('| 368 |')), '')
...
ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R128) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b519' in S['ferry'],
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b518`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 367 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 367 |', '| 3670 |'))),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b519' in S['ferry'] and 'ACT b519' in S['face'] and not glob.glob(os.path.join(D, 'b520_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b519')),
    ...
    ('G-R128-ENTERED', 'the banked ferry AND the trail', lambda S: 'RULING (R128) END' in S['ferry'] and S['ot'].count('**(R128) ratified') == 1,
     lambda S: put(S, 'ot', S['ot'].replace('**(R128) ratified', '(R128) noted'))),
    ('G-CRITERION-WORDS', 'the trail`s own record -- (R128)(1) in its own words, the deposit`s phrase, the three contributions',
     lambda S: CRIT in trail(S) and '"an equivalence, classical, and not ours"' in trail(S) and 'three things and no fourth' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace('positivity criterion on classK', 'criterion'))),
    ('G-WORKORDER-FILED', 'the trail`s own record -- W-ORD-H2-BRIDGE by name, once, at the head of K1',
     lambda S: trail(S).count('`W-ORD-H2-BRIDGE`') == 1 and 'head of the K1 programme' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace('`W-ORD-H2-BRIDGE`', 'a work-order'))),
    ('G-FIXTURE-BAR', 'every fixture -- each u`s difference against sqrt(n) times its floor, recomputed',
     lambda S: len(S['fx']) == 6 and all(r['diff'] == abs(r['closed'] - r['numeric']) and r['diff'] <= r['bar']
                                         and rel(r['bar'], r['floor'] * math.sqrt(f['nodes'])) for f in S['fx'] for r in f['rows']),
     lambda S: put(S, 'fx', [dict(f, rows=[dict(r, numeric=r['numeric'] + 1e-6 * max(1.0, abs(r['closed'])),
                                                 diff=abs(r['closed'] - r['numeric'] - 1e-6 * max(1.0, abs(r['closed'])))) for r in f['rows']]) for f in S['fx']])),
    ('G-CLASSK-STATED', 'every fixture AND the components -- generating h named, k even, h C^2 with a live probe',
     lambda S: all(f['generating_h'].startswith('h(u) = (') and f['even'] < 1e-9 * max(1.0, f['scale'])
                   and f['knot_jumps']['D3'] > 1e-6 * f['d3_scale'] for f in S['fx'])
     and S['comp'].count('classK (READING (2)): k = weilTest h h with h = h(u) = (') == 6,
     lambda S: put(S, 'fx', [dict(f, even=1e6 * max(1.0, f['scale'])) for f in S['fx']])),
    ('G-ZERO-CONTROL', 'the control bank -- per variant, three widths, two objects; h-hat(gamma_0) zero, the slope recomputed here',
     lambda S: len(S['mt']) == 18 and all(abs(complex(*m['at_gamma0'])) < 1e-12 * m['line_peak'] for m in S['mt'])
     and all(rel(m['slope'], m['slope_numeric'], 1e-4) for m in S['mt'])
     and all(rel(m['slope'], (-2.0 * m['gammas'][0] * np.prod([g * g - m['gammas'][0] ** 2 for g in m['gammas'][1:]])
                              * 0.5 * (1.0 + phihat(m['variant'], m['a'], 2.0 * m['gammas'][0]))).real, 1e-8) for m in S['mt']),
     lambda S: put(S, 'mt', [dict(m, at_gamma0=[1e-3 * m['line_peak'], 0.0]) for m in S['mt']])),
    ('G-WIDTHS-COMPLETE', 'each variant`s cells against b511`s ladder -- 119 widths, every one',
     lambda S: len(S['ladder']) == 119 and all([round(c['a'], 9) for c in S['cells'][v]] == [round(a, 9) for a in S['ladder']] for v in VARIANTS),
     lambda S: put(S, 'cells', dict(S['cells'], C=S['cells']['C'][:-1]))),
    ('G-WINDOW-AS-WRITTEN', 'each variant`s pole term recomputed from READING (1)`s formula at three widths',
     lambda S: all(rel(c[o]['P'], (hhat(v, o, c['a'], c[o]['gammas'], 0.5j) ** 2 + hhat(v, o, c['a'], c[o]['gammas'], -0.5j) ** 2).real, 1e-7)
                   for v in VARIANTS for c in (S['cells'][v][0], S['cells'][v][59], S['cells'][v][118]) for o in ('q', 'xi')),
     lambda S: put(S, 'cells', dict(S['cells'], B=[dict(c, q=dict(c['q'], P=c['q']['P'] * 1.01 + 1e-6)) for c in S['cells']['B']]))),
    ('G-SPLIT-SUMS', 'every cell -- the zero side equals the pair, the other off-line pairs and the on-line part',
     lambda S: all(rel(c[o]['Z'], c[o]['on_rest'] + c[o]['pair'] + c[o]['others']) for v, c, o in objs(S)),
     lambda S: put(S, 'cells', dict(S['cells'], A=[dict(c, q=dict(c['q'], others=c['q']['others'] + 0.1 * abs(c['q']['Z']) + 1.0)) for c in S['cells']['A']]))),
    ('G-RATIO-RECOMPUTED', 'every cell -- pair / (Z - pair), signed, recomputed',
     lambda S: all(rel(c[o]['rest'], c[o]['Z'] - c[o]['pair']) and rel(c[o]['ratio'], c[o]['pair'] / (c[o]['Z'] - c[o]['pair']), 1e-7) for v, c, o in objs(S)),
     lambda S: put(S, 'cells', dict(S['cells'], A=[dict(c, q=dict(c['q'], ratio=0.1)) for c in S['cells']['A']]))),
    ('G-VERIFIED-RECOUNTED', 'every cell`s flag against |r| <= B, and the results` counts',
     lambda S: all(c[o]['verified'] == (abs(c[o]['r']) <= c[o]['B']) for v, c, o in objs(S))
     and all(S['res'][v][o]['verified'] == recount(S, v, o)['verified'] for v in VARIANTS for o in ('q', 'xi')),
     lambda S: put(S, 'res', dict(S['res'], B=dict(S['res']['B'], xi=dict(S['res']['B']['xi'], verified=-1))))),
    ('G-BOUND-FORM', 'every cell -- B the sum of the four terms, the shortfall not in it',
     lambda S: all(rel(c[o]['B'], c[o]['Eu'] + c[o]['Ek'] + c[o]['Etail'] + c[o]['Eround']) for v, c, o in objs(S)),
     lambda S: put(S, 'cells', dict(S['cells'], C=[dict(c, xi=dict(c['xi'], B=c['xi']['B'] * 1.5 + 1e-9)) for c in S['cells']['C']]))),
    ('G-SHORTFALL-PRINTED', 'every cell carries its ESTIMATE; the report prints the recount',
     lambda S: all(c[o].get('shortfall') is not None and c[o]['shortfall'] >= 0 for v, c, o in objs(S)) and 'shortfall' in S['report']
     and all(S['res'][v][o]['shortfall_over_B'] == recount(S, v, o)['shortfall_over_B'] for v in VARIANTS for o in ('q', 'xi')),
     lambda S: put(S, 'report', S['report'].replace('shortfall', 'x'))),
    ('G-SIGNS-RECOUNTED', 'the results` sign lists against the cells, recounted, per variant',
     lambda S: all(S['res'][v][o][k] == recount(S, v, o)[k] for v in VARIANTS for o in ('q', 'xi') for k in ('h2_neg', 'h2_pos', 'pair_neg', 'pair_pos')),
     lambda S: put(S, 'res', dict(S['res'], A=dict(S['res']['A'], q=dict(S['res']['A']['q'], pair_neg=[5.0]))))),
    ('G-GROWTH-RECOMPUTED', 'every cell`s realized growth recomputed from the base transform at i delta, and its bound',
     lambda S: all(rel(c[o]['growth'], phihat(v, c['a'], 1j * DELTA).real, 1e-9) and rel(c[o]['growth_bound'], math.exp(DELTA * math.log(c['a'])))
                   for v, c, o in objs(S)),
     lambda S: put(S, 'cells', dict(S['cells'], B=[dict(c, q=dict(c['q'], growth=c['q']['growth'] * 10)) for c in S['cells']['B']]))),
    ('G-NARROWEST-GROWTH', 'per variant -- the narrowest negative width and its growth, recomputed, or both absent',
     lambda S: all((S['res'][v]['q']['narrowest'] == (min(recount(S, v, 'q')['h2_neg']) if recount(S, v, 'q')['h2_neg'] else None))
                   and ((S['res'][v]['q']['growth_at_narrowest'] is None) == (S['res'][v]['q']['narrowest'] is None)) for v in VARIANTS),
     lambda S: put(S, 'res', dict(S['res'], C=dict(S['res']['C'], q=dict(S['res']['C']['q'], narrowest=5.0))))),
    ('G-READING-WORDS', 'per variant -- the reading against the recount AND the trail, (R127)(2)`s words',
     lambda S: all(((S['res'][v]['q']['reading'].startswith(HARD5) and ('%+.6e' % S['cells'][v][-1]['q']['ratio']) in S['res'][v]['q']['reading'])
                    if not recount(S, v, 'q')['h2_neg'] else WIT5 in S['res'][v]['q']['reading']) and S['res'][v]['q']['reading'] in trail(S)
                   for v in VARIANTS),
     lambda S: put(S, 'res', dict(S['res'], A=dict(S['res']['A'], q=dict(S['res']['A']['q'], reading='the window is a witness'))))),
    ('G-WITNESS-NAMED', 'the results` witness list against the recount, and named in the trail',
     lambda S: S['res']['witness'] == [v for v in VARIANTS if recount(S, v, 'q')['h2_neg']]
     and ('COMPONENT 3 — the witness at Q0:' in trail(S)),
     lambda S: put(S, 'res', dict(S['res'], witness=['C']))),
    ('G-N1-SCORED', 'the desk against the recount', lambda S: 'n1' in S['sc'] and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == bool(recount(S, 'C', 'q')['h2_neg']), lambda S: put(S, 'sc', dict(S['sc'], n1=not S['sc'].get('n1')))),
    ('G-N2-SCORED', 'the desk against the recount', lambda S: 'n2' in S['sc'] and desk_word(S, 'N2') == word_of(S['sc']['n2'])
     and S['sc']['n2'] == (not recount(S, 'A', 'q')['h2_neg']), lambda S: put(S, 'sc', dict(S['sc'], n2=not S['sc'].get('n2')))),
    ('G-N3-SCORED', 'the desk against the growth recomputed', lambda S: 'n3' in S['sc'] and desk_word(S, 'N3') == word_of(S['sc']['n3'])
     and S['sc']['n3'] == (S['cells']['B'][-1]['q']['growth'] / g518(S['cells']['B'][-1]['a']) > 10.0),
    ...
    ('G-N4-SCORED', 'the desk against the recount', lambda S: 'n4' in S['sc'] and desk_word(S, 'N4') == word_of(S['sc']['n4'])
     and S['sc']['n4'] == all(len(recount(S, v, 'xi')['h2_pos']) == recount(S, v, 'xi')['verified'] for v in VARIANTS)
     and (all(recount(S, v, 'xi')['verified'] > 0 for v in VARIANTS) or 'VACUOUS ON' in S['desk']),
    ...
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from the banks',
     ...
     and desk_word(S, 'S1') == word_of(S['cells']['B'][-1]['q']['growth'] / g518(S['cells']['B'][-1]['a']) < 10.0)
     and desk_word(S, 'S2') == word_of(abs(S['cells']['A'][-1]['q']['ratio']) > abs(S['r518']['q']['widest_ratio']))
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b519 -'))),
    ...
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 368 once, six cells as it landed',
     lambda S: S['corr'].count('| 368 |') == 1 and S['corr'].count('(b519, under (R128))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 368 | a duplicate row |')),
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b518`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b518_window.py'])),
    ('G-WRITELIST-KINDS', 'every b519 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b519')" in S['suite']
                and "data/b519_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b519_components.txt' in gits(ROOT, 'show'")),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b519')
              and 'data/b519_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b519 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b519_checks_postpush.txt' if pushed else 'b519_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b519_exercise.json'), 'w', encoding='utf-8', newline=NL),
