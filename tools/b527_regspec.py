# -*- coding: utf-8 -*-
"""b527_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b524's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the kernel
### ### clauses count the two EDITED kernel files; no grade is conferred.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b527_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b527_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): TwoPropertyWindow.lean and AxiomCheckWindow.lean in the kernel, both EDITED.'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### (R137)(4): the kernel lane for this act alone.'),
    ...
    ('grades conferred', 0, 0, 'grades', '### (Z): no grade conferred; b527`s grades stand on unchanged statements.'),
    ...
    ('claims about RH, h2 or any zero beyond quoting a verified source', 0, 0, 'claims', '### section (A): a plateau`s definition is not a claim about any zero.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b527.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R137).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b527_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b527_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b527_extract.txt'),
    ...
    spec = {"registration": ("data/b527_registration_2026-09-25.txt -- b527, "
