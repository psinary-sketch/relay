# -*- coding: utf-8 -*-
"""b451_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b451_registration_2026-09-13.txt')
SPEC = os.path.join(ROOT, 'data', 'b451_satisfiable.json')
...
CLAUSES = [
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### section (W): mirror-refresh-2026-09-13-b451.zip.'),
    ...
    ('TECHNE-Core writes', 1, 1, 'writes', '### section (W): one line appended to PREDICATE_ONE_SHAPE.md, local, not pushed.'),
    ('work-orders filed', 1, 1, 'work-orders', '### reading (3): W-ORD-MATCHER-SHAPE.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b451_addendum.txt.'),
    ...
    ('rulings proposed or items answered', 0, 0, 'rulings', '### (K) BAR 3.'),
    ('keystones opened beyond naming a need', 0, 0, 'keystones', '### (K) BAR 4.'),
    ('keystones edited', 0, 0, 'keystones', '### section (Z).'),
    ...
    ('matchers edited', 0, 0, 'matchers', '### (K) BAR 5.'),
...
def main(argv):
    print('=' * 100)
    print('b451_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b451_registration_2026-09-13.txt -- b451, "
                             "THE RECONCILIATION'S REMAINDER NAMED, AND THE ROUTED ITEMS GROUPED BY THE RULING EACH NEEDS"),
            "clauses": clauses}
