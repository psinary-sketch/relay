# -*- coding: utf-8 -*-
"""b530_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b530's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b530_registration_2026-09-25.txt')
OT = os.path.join(PP, 'OPEN_TRAILS.md')
PRIOR_RELAY = '5ce832fe'      # ### b529's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
KER_TIP = 'a00aa21'           # ### the kernel's tip before this act
PRIOR_PP = '818a94f'           # ### b529's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b530 —'
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b530_ferry.txt')),
        scan=read(os.path.join(D, 'b530_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b530_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b530_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b530_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b530_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b529_closing.txt')),
        addendum=read(os.path.join(D, 'b530_addendum.txt')),
        comp=read(os.path.join(D, 'b530_components.txt')),
        desk=read(os.path.join(D, 'b530_desk_notes.txt')),
        report=read(os.path.join(D, 'b530_report.txt')),
        mod=read(os.path.join(KER, 'SIDEExplicitFormula', 'DecayBound.lean')) + NL + NL + read(os.path.join(KER, 'SIDEExplicitFormula', 'RestBound.lean')),
        rmod=read(os.path.join(KER, 'SIDEExplicitFormula', 'RestBound.lean')),
        acc=read(os.path.join(KER, 'AxiomCheckRest.lean')),
        att={w: json.loads(read(os.path.join(D, 'b530_attempts_%s.json' % w)) or '[]') for w in ('decay', 'rest')},
        att_files={w: {int(re.search(r'attempt_%s(\d+)\.lean$' % w, p).group(1)): hashlib.sha256(open(p, 'rb').read()).hexdigest()
                       for p in glob.glob(os.path.join(D, 'b530_attempt_%s[0-9]*.lean' % w))} for w in ('decay', 'rest')},
        clogs={w: len(glob.glob(os.path.join(D, 'b530_compile_log_%s*.txt' % w))) for w in ('decay', 'rest')},
        clast={w: read(sorted(glob.glob(os.path.join(D, 'b530_compile_log_%s*.txt' % w)), key=os.path.getmtime)[-1]) for w in ('decay', 'rest')},
        probes=read(os.path.join(D, 'b530_probe1.txt')) + read(os.path.join(D, 'b530_probe2.txt')),
        plog=read(sorted(glob.glob(os.path.join(D, 'b530_profile_log*.txt')), key=os.path.getmtime)[-1]),
        inst=json.loads(read(os.path.join(D, 'b530_install.json')) or '{}'),
        prof=json.loads(read(os.path.join(D, 'b530_profile.json')) or '{}'),
        rd=json.loads(read(os.path.join(D, 'b530_read.json')) or '{}'),
        stmt=read(os.path.join(D, 'b530_statement.txt')),
        vals=json.loads(read(os.path.join(D, 'b530_values.json')) or '{}'),
        prior_mods={f: (blob(KER, 'a00aa21:' + f), blob(KER, 'HEAD:' + f), open(os.path.join(KER, f), 'rb').read().replace(b'\r', b''))
                    for f in ('SIDEExplicitFormula/TwoPropertyWindow.lean', 'SIDEExplicitFormula/PairTerm.lean',
                              'SIDEExplicitFormula/GrowthBound.lean', 'AxiomCheckPair.lean')},
        ker_changed=set(x for x in gits(KER, 'diff', '--name-only', 'a00aa21').split(NL) if x.strip())
                    | set(p[3:].strip() for p in git(KER, 'status', '--porcelain').split(NL) if p.strip()),
        ker_tags=gits(KER, 'tag', '--points-at', 'HEAD'),
        res=json.loads(read(os.path.join(D, 'b530_results.json')) or '{}'),
        sc=json.loads(read(os.path.join(D, 'b530_scores.json')) or '{}'),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b530_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b530.zip'))),
        chain_clean=(gits(ROOT, 'status', '--porcelain', '--', 'tools/b321_window.py', 'tools/e16/carto_atlas.py',
                          ...
                          'tools/b528_smooth.py', 'tools/b529_values.py', 'tools/b504_chain.py', 'tools/b506_kernel.py', 'tools/b511_families.py', 'tools/b503_zero.py', 'tools/b514_window.py', 'tools/b515_window.py', 'tools/b518_window.py', 'tools/b519_window.py', 'tools/b520_widen.py', 'tools/b521_tail.py', 'tools/b522_reach.py', 'tools/b526_xi7.py') == ''),
        ker_clean=(gits(KER, 'rev-parse', '--short=7', 'HEAD') == KER_TIP and gits(KER, 'status', '--porcelain', '--untracked-files=no') == ''),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b530 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b530_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (os.path.join(D, x) for x in ('b530_install.json', 'b530_attempts_decay.json', 'b530_attempts_rest.json', 'b530_profile.json', 'b530_values.json', 'b530_run_log.txt'))),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b530 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[0-9]_|^b52[0-9]_|^b334_', f)]
...
REST = ['rest_bound', 'rest_bound_closed', 'rest_bound_zeta', 'finite_rest_bound', 'rest_le_of_finite', 'restR_nonneg',
        'fiber_count', 'summable_zeta3', 'zeta3Sum_nonneg', 'zero_term_bound', 'weight_ineq', 'kWin_bound', 'window_bound',
        'cosWin_bound', "gammaOf_re'", "gammaOf_im'", 'cosh_le_exp_abs', 'realizedGrowth_le_exp', 'two_delta_lt_one',
        'not_f4_needs', 'f4_needs_of_half_le']
THMS = DECAY + REST
NEWK = {'SIDEExplicitFormula/DecayBound.lean', 'SIDEExplicitFormula/RestBound.lean', 'AxiomCheckRest.lean'}
RESTR_TXT = ('768 * A₀ * SIDEExplicitFormula.B321.windowA γ₀ φ ^ 2 * Real.exp L * (γ₀ ^ 2 + 1) ^ 2 * '
             'SIDEExplicitFormula.B321.zeta3Sum')


def first_clean_w(S, w):
    ok = [a['attempt'] for a in S['att'][w] if a['exit'] == 0 and a['errors'] == 0 and not a['sorry']]
    return ok[0] if ok else None


def rest_head(S):
    m = re.search(r'^theorem rest_bound \(Z[\s\S]*?:=', S['rmod'], re.M)
...
def corr_row(S):
    return next((l for l in S['corr'].split(NL) if l.startswith('| 379 |')), '')
...
ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R140) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b530' in S['ferry'],
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b530' in S['ferry'] and 'ACT b530' in S['face'] and not glob.glob(os.path.join(D, 'b531_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b530')),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b530 -'))),
    ...
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b529`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b519_window.py'])),
    ('G-WRITELIST-KINDS', 'every b530 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b530')" in S['suite']
                and "data/b530_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b530_components.txt' in gits(ROOT, 'show'")),
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b529`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 378 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 378 |', '| 3780 |'))),
    ('G-PEEK-DECLARED', 'the face`s (C) block AND the component files` times against the lock',
     lambda S: 'TWO NAME PROBES WERE RUN BEFORE THE SEAL, DECLARED' in flat(S['face'])
     and 'elaborating NO proof of either module' in flat(S['face']) and S['after_lock'],
     lambda S: put(S, 'after_lock', False)),
    ('G-R140-ENTERED', 'the banked ferry AND the trail', lambda S: 'RULING (R140) END' in S['ferry'] and S['ot'].count('**(R140) ratified') == 1,
     lambda S: put(S, 'ot', S['ot'].replace('**(R140) ratified', '(R140) noted'))),
    ('G-INSTALLED-FROM-DRAFT', 'the install bank against both modules` attempt-1 sources -- the sealed drafts first ran, onto no prior file',
     lambda S: all(S['inst'][n]['prior_sha'] is None for n in ('DecayBound.lean', 'RestBound.lean', 'AxiomCheckRest.lean'))
     and S['inst']['DecayBound.lean']['draft_sha'] == S['att']['decay'][0]['source_sha'] == S['att_files']['decay'].get(1)
     and S['inst']['RestBound.lean']['draft_sha'] == S['att']['rest'][0]['source_sha'] == S['att_files']['rest'].get(1),
     lambda S: put(S, 'inst', {**S['inst'], 'RestBound.lean': dict(S['inst']['RestBound.lean'], draft_sha='0' * 64)})),
    ('G-ATTEMPTS-BANKED', 'both attempts banks against their banked sources and logs',
     lambda S: all([a['attempt'] for a in S['att'][w]] == list(range(1, len(S['att'][w]) + 1)) == sorted(S['att_files'][w])
                   and all(S['att_files'][w][a['attempt']] == a['source_sha'] for a in S['att'][w])
                   and S['clogs'][w] == len(S['att'][w]) for w in ('decay', 'rest')),
     lambda S: put(S, 'att', {**S['att'], 'rest': S['att']['rest'][:-1]})),
    ('G-MODULE-NO-SORRY', 'both modules` text, their last compile logs and the profile log',
     lambda S: bool(S['mod']) and not re.search(r'\bsorry\b', S['mod'])
     and all('exit 0 ;' in S['clast'][w] and ': error' not in S['clast'][w] for w in ('decay', 'rest'))
     and 'sorryAx' not in S['plog'],
     lambda S: put(S, 'mod', S['mod'] + NL + 'theorem x : False := sorry')),
    ('G-PROFILE-WHOLE-STRING', 'the profile run file -- each of the 30 theorems` line equal to the standard-three string',
     ...
     lambda S: put(S, 'plog', S['plog'].replace(STD3 % (NS + 'rest_bound'), (STD3 % (NS + 'rest_bound')).replace(', Quot.sound', ', sorryAx')))),
    ('G-STATEMENTS-PRINTED', 'the banked statements against both modules -- each declaration from its line to its `:=`, recomputed',
     lambda S: bool(heads(S['mod'])) and all(h in S['stmt'] for h in heads(S['mod'])),
     lambda S: put(S, 'stmt', S['stmt'].replace('theorem rest_bound (Z', 'theorem ghost (Z'))),
    ('G-PRIOR-MODULES-UNTOUCHED', 'the kernel`s earlier files at a00aa21, at HEAD and on disk -- byte-identical',
     lambda S: all(a == b == c and a for a, b, c in S['prior_mods'].values()),
     lambda S: put(S, 'prior_mods', {k: (a, b, c + b'x') for k, (a, b, c) in S['prior_mods'].items()})),
    ('G-PROBES-BANKED', 'the two banked probe files -- sources and outputs, no theorem among them',
     lambda S: S['probes'].count('### output') == 2 and S['probes'].count('#check @') >= 40
     and not re.search(r'^theorem ', S['probes'], re.M) and 'b530_probe' in S['face'],
     lambda S: put(S, 'probes', S['probes'].replace('### output', '### out', 1))),
    ('G-HYPOTHESES-NAMED', 'rest_bound`s statement in the module, recomputed HERE -- its Z-hypotheses exactly HStrip and HCount',
     lambda S: z_hyps(rest_head(S)) == ['HCount Z A₀', 'HStrip Z'] and rest_head(S) in S['stmt'],
     lambda S: put(S, 'rmod', S['rmod'].replace('(hcount : HCount Z A₀) (ρ₀ : ℂ) :\n    restSum', '(hcount : HCount Z A₀) (hmax : HMax Z) (ρ₀ : ℂ) :\n    restSum'))),
    ('G-PROPS-PRINTED', 'Lean`s own #check lines -- HStrip, HCount, f4_needs and HMax each a Prop',
     lambda S: all((((S['prof'].get('checks') or {}).get(n)) or '').endswith('→ Prop') for n in ('HStrip', 'HCount', 'f4_needs', 'HMax')),
     lambda S: put(S, 'prof', dict(S['prof'], checks=dict(S['prof'].get('checks') or {}, HMax='HMax : Zeta23.ZeroConfig → ℝ')))),
    ('G-R-TRANSCRIBED', 'Lean`s own #print of restR AND the values bank -- R recomputed HERE from the printed formula`s factors',
     lambda S: ' '.join(((S['prof'].get('prints') or {}).get('restR') or '').split()).endswith(RESTR_TXT)
     and abs(768 * S['vals']['A0_measured'] * S['vals']['windowA'] ** 2 * math.exp(S['vals']['L']) * (S['vals']['gamma0'] ** 2 + 1) ** 2
             * S['vals']['zeta3Sum'] - S['vals']['R']) <= 1e-9 * S['vals']['R'],
     lambda S: put(S, 'vals', dict(S['vals'], R=S['vals']['R'] * 2))),
    ('G-VALUES-RECOMPUTED', 'the values bank -- the ratios and the verdict recomputed HERE; SUM (1+|m|)^-3 against 2 zeta(3) - 1',
     lambda S: abs(S['vals']['ratio_R_rest'] - S['vals']['R'] / S['vals']['rest_measured']) <= 1e-9 * S['vals']['ratio_R_rest']
     and S['vals']['exceeds_100'] == (S['vals']['R'] > 100 * S['vals']['rest_measured'])
     and abs(S['vals']['zeta3Sum'] - (1 + 2 * sum(1.0 / (1 + n) ** 3 for n in range(1, 200000)))) < 1e-9
     and S['vals']['A0_measured'] >= 1.0,
     lambda S: put(S, 'vals', dict(S['vals'], exceeds_100=not S['vals']['exceeds_100']))),
    ('G-GRADES-BY-READING', 'this act`s correspondence row and trail record -- rest_bound DERIVES, nothing else graded',
     lambda S: '`rest_bound` DERIVES' in corr_row(S) and 'HMax STATED, no grade' in corr_row(S)
     and sorted(set(re.findall(r'`(\w+)` (?:DERIVES|INTERFACES|SHELL|ENCODES)', corr_row(S)))) == ['rest_bound']
     and '`rest_bound` DERIVES of its model statement under the two named hypotheses' in trail(S),
     lambda S: put(S, 'corr', S['corr'].replace('`rest_bound` DERIVES', '`rest_bound_zeta` DERIVES ; `rest_bound` DERIVES'))),
    ('G-F3-MARKED', 'the trail record`s re-printed table -- f3 marked, f4 OPEN with (A) and (B)',
     lambda S: '| (f3) | b530: DERIVES under H-STRIP and H-COUNT (`rest_bound`) |' in seg(S['ot'], TRAILH, 99999)
     and '| (f4) | OPEN; (A) H-MAX, UNKNOWN; (B) the sequence route, two lemmas ABSENT |' in seg(S['ot'], TRAILH, 99999),
     lambda S: put(S, 'ot', S['ot'].replace('| (f4) | OPEN; (A) H-MAX', '| (f4) | DONE; (A) H-MAX'))),
    ('G-TABLE-ROW', 'the regenerated terminal table -- rest_bound PROFILED and DERIVES; the decay lemma PROFILED',
     lambda S: 'DERIVES' in table_row(S, 'rest_bound') and 'NOT PROFILED' not in table_row(S, 'rest_bound')
     and bool(table_row(S, 'paperFT_decay')) and 'NOT PROFILED' not in table_row(S, 'paperFT_decay'),
     lambda S: put(S, 'table', S['table'].replace('`%srest_bound`' % NS, '`ghost`'))),
    ('G-N1-SCORED', 'the desk against the decay attempts and the profile', lambda S: 'n1' in S['sc'] and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == (first_clean_w(S, 'decay') is not None and first_clean_w(S, 'decay') <= 3
                           and all((S['prof'].get('std3') or {}).get(NS + n) is True for n in DECAY)),
     lambda S: put(S, 'sc', dict(S['sc'], n1=not S['sc'].get('n1')))),
    ('G-N2-SCORED', 'the desk against rest_bound`s statement recomputed HERE', lambda S: 'n2' in S['sc'] and desk_word(S, 'N2') == word_of(S['sc']['n2'])
     and S['sc']['n2'] == ((S['prof'].get('std3') or {}).get(NS + 'rest_bound') is True and z_hyps(rest_head(S)) == ['HCount Z A₀', 'HStrip Z']),
     lambda S: put(S, 'sc', dict(S['sc'], n2=not S['sc'].get('n2')))),
    ('G-N3-SCORED', 'the desk against the profile', lambda S: 'n3' in S['sc'] and desk_word(S, 'N3') == word_of(S['sc']['n3'])
     and S['sc']['n3'] == all((S['prof'].get('std3') or {}).get(NS + n) is True for n in ('two_delta_lt_one', 'not_f4_needs')),
     lambda S: put(S, 'sc', dict(S['sc'], n3=not S['sc'].get('n3')))),
    ('G-N4-SCORED', 'the desk against the values recomputed HERE', lambda S: 'n4' in S['sc'] and desk_word(S, 'N4') == word_of(S['sc']['n4'])
     and S['sc']['n4'] == (S['vals']['R'] > 100 * S['vals']['rest_measured']),
    ...
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from the banks',
     ...
     and desk_word(S, 'S1') == word_of(first_clean_w(S, 'decay') == 1)
     and desk_word(S, 'S2') == word_of(first_clean_w(S, 'rest') is not None and first_clean_w(S, 'rest') <= 3
                                       and len(S['prof'].get('std3') or {}) == 30 and all((S['prof'].get('std3') or {}).values()))
     and desk_word(S, 'S3') == word_of(S['vals']['R'] / S['vals']['rest_measured'] > 1e10),
     lambda S: put(S, 'desk', S['desk'].replace('**(S1)** ### **', '**(S1)** ### **NOT'))),
    ('G-CORPUS-SCOPE', 'the PLACE-papers file list -- the trail alone; no chain file edited; the kernel`s changes the three NEW files',
    ...
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 379 once, six cells as it landed',
     lambda S: S['corr'].count('| 379 |') == 1 and S['corr'].count('(b530, under (R140))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 379 | a duplicate row |')),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b530')
              and 'data/b530_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b530 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b530_checks_postpush.txt' if pushed else 'b530_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b530_exercise.json'), 'w', encoding='utf-8', newline=NL),
