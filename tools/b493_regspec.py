# -*- coding: utf-8 -*-
"""b493_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b493_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b493_satisfiable.json')
...
CLAUSES = [
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b493.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b493_extract.txt`.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R104) -- the forward order of acts is fixed. ### It opens nothing by itself, and the census it calls for belongs to the act AFTER this one.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b493_addendum.txt, under the stem glob (R91) permits.'),
...
def main(argv):
    print('=' * 100)
    print('b493_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b493_extract.txt'),
    ...
    spec = {"registration": ("data/b493_registration_2026-09-23.txt -- b493, "
