# -*- coding: utf-8 -*-
"""b526_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b523's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the
### ### numerical clauses re-read for this act's three re-pricings of b526's cells; no prior bank is touched.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b526_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b526_satisfiable.json')
...
CLAUSES = [
    ...
    ('chains run, or channel values computed', 1, 1, 'runs', '### (B)(1): the chain on variant (B) at order 7, xi alone; Component 2 HALTED, no chain on the kernel`s phi.'),
    ('lean runs', 0, 0, 'runs', '### (Z).'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z).'),
    ('lanes opened', 1, 1, 'lanes', '### (R136)(3): the numerical lane for this act alone.'),
    ...
    ('expectations scored on a population other than the one named', 0, 0, 'expectations', '### READINGS (6) and (8).'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b526.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R136).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b526_addendum.txt, under the stem glob.'),
    ...
    ('PLACE-papers documents written at all', 3, 3, 'documents', '### (W): FINDINGS.md and the digest, one note each; OPEN_TRAILS.md, one record.'),
    ('closings edited', 0, 0, 'closings', '### section (Z).'),
    ('lines removed from any document', 0, 0, 'lines', '### section (Z).'),
    ('bars declared', 'BARS', 'BARS', 'bars', '### MEASURED: this face has no (K) section; no (K) section; the controls are in READINGS (1) and (3).'),
    ...
    ('new `relay` act-tool files named on this face', 9, 9, 'files', '### (W): the stem glob (R91) permits.'),
...
def main(argv):
    print('=' * 100)
    print('b526_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b526_extract.txt'),
    ...
    spec = {"registration": ("data/b526_registration_2026-09-25.txt -- b526, "
                             "XI AT ORDER 7, AND BOTH OBJECTS ON THE KERNEL`S OWN phi"),
            "clauses": clauses}
