# -*- coding: utf-8 -*-
"""b484_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b484_registration_2026-09-22.txt')
SPEC = os.path.join(ROOT, 'data', 'b484_satisfiable.json')
...
CLAUSES = [
    ...
    ('platform calls of any kind', 0, 0, 'calls', '### (K) BAR 1: nothing fetched; the pins` ls-remote reads and the ritual`s pushes excepted.'),
    ...
    ('repositories cloned', 0, 0, 'repositories', '### section (Z): the comparator`s four pinned checkouts are NOT fetched.'),
    ('lanes opened', 0, 0, 'lanes', '### section (B)(3): the support-edge test is A READ; no lane is opened.'),
    ...
    ('keystones edited or annotated', 0, 0, 'keystones', '### (W): INVARIANCE_BARRIERS.md is REGISTRY row 1.5h-8, a paper and not a keystone; a bibliography line is repaired and no claim of it is touched.'),
    ('state terms changed on any line', 2, 2, 'terms', '### (W) and (R92): the version word and the version DOI at INVARIANCE_BARRIERS.md:580, corrected to REGISTRY`s own row values. The concept DOI already agrees and is NOT touched.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b484.zip, under (R69).'),
    ...
    ('readings of the order declared in advance on this face', 6, 6, 'readings', '### section (A): (1) to (6).'),
    ('pre-face reads declared', 6, 6, 'reads', '### section (A): (P1) to (P6).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R93) settles (R92)`s addressee as THIS act.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b484_addendum.txt, under the stem glob (R91) permits.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('expectations registered', 3, 3, 'expectations', '### section (J): (N1), (N2), (N3), each with the seat`s own call made before the components run.'),
    ...
    ('prose passages of the corpus rewritten', 0, 0, 'passages', '### (K) BAR 5: two state words on one line, by (R92); no passage is rewritten and the original is preserved verbatim in an appended annotation.'),
    ('species summed', 0, 0, 'sums', '### section (Z).'),
    ('loom blocks appended', 0, 0, 'blocks', '### section (Z).'),
    ('PLACE-papers documents written other than by appending', 1, 1, 'documents', '### (W): INVARIANCE_BARRIERS.md, two state words in place PLUS an appended annotation, as (R92) directs.'),
    ('PLACE-papers documents written at all', 2, 2, 'documents', '### (W): OPEN_TRAILS.md appended, and INVARIANCE_BARRIERS.md repaired under (R92).'),
    ...
    ('lines removed from any document', 0, 0, 'lines', '### (K) BAR 4.'),
...
def main(argv):
    print('=' * 100)
    print('b484_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b484_registration_2026-09-22.txt -- b484, "
                             "THE CURRENCY REPAIR, THE WORK-ORDER, THE SUPPORT-EDGE TEST"),
            "clauses": clauses}
