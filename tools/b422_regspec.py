# -*- coding: utf-8 -*-
"""b422_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b422_registration_2026-09-11.txt')
SPEC = os.path.join(ROOT, 'data', 'b422_satisfiable.json')
...
CLAUSES = [
    ...
    ('doors restated without a span bank naming their depth', 0, 0, 'doors', '### (K) BAR 7.'),
    ...
    ('fold sections appended', 1, 1, 'sections', '### (W) KIND 3.'),
    ('corpus bytes changed other than by append', 0, 0, 'bytes', '### (K) BAR 2.'),
    ('rules struck, amended, widened or re-ruled', 0, 0, 'rules', '### section (Z): (R37) executed, not extended.'),
    ...
    ('orientation-layer lines edited', 0, 0, 'lines', '### section (Z): dated blocks appended only.'),
    ...
    ('author rulings ratified by this paste', 1, 1, 'rulings', '### section (A): (R37).'),
    ('author rulings extended beyond what they say', 0, 0, 'rulings', '### (K) BAR 9.'),
    ('censuses run at step zero', 2, 2, 'censuses', '### section (A).'),
    ('repositories hard-failing at step zero', 0, 0, 'repositories', '### section (A).'),
    ('readings of the order declared in advance on this face', 9, 9, 'readings', '### section (A): (0) to (8).'),
    ...
    ('words of Definition 2.1 changed', 0, 0, 'words', '### (K) BAR 9.'),
    ('prior verdicts re-verdicted', 0, 0, 'verdicts', '### (K) BAR 4.'),
    ('grade strings in the section absent from their attributed bank', 0, 0, 'strings', '### (K) BAR 3.'),
    ('folded acts summarised as overturning another', 0, 0, 'acts', '### (K) BAR 4.'),
    ('columns in the count', 3, 3, 'columns', '### (A) reading (3).'),
    ('statements of the third column merged into another', 0, 0, 'statements', '### (K) BAR 5.'),
    ('orientation blocks appended', 2, 2, 'blocks', '### (A) reading (5).'),
    ('conflations carried into the section', 3, 3, 'conflations', '### (A) reading (6).'),
    ('witness candidates attempted', 0, 0, 'candidates', '### (K) BAR 8.'),
    ('sites the witness arc is priced over', 6, 6, 'sites', '### (A) reading (7).'),
...
def main(argv):
    print('=' * 100)
    print('b422_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b422_registration_2026-09-11.txt -- b422, THE FOLD, b413 TO b421, AND THE "
                             "WITNESS ARC NAMED"),
            "clauses": clauses}
