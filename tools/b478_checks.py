# -*- coding: utf-8 -*-
"""b478_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE.
...
FACE = os.path.join(D, 'b478_registration_2026-09-22.txt')
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b478_ferry.txt')),
        scan=read(os.path.join(D, 'b478_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b478_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b478_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b478_pins_stepzero.txt')),
        extract=read(os.path.join(D, 'b478_extract.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b478_lockgate_notes*.txt')))[-1]),
        lock_runs=len(glob.glob(os.path.join(D, 'b478_lockgate_notes*.txt'))),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        # ### **THE PRIOR ACT'S CLOSING IS b462'S.** ### A wholesale re-point from b462 to b478
...
# ### ### **THE PRIOR CLOSED ACT IS b464.** ### A wholesale re-point moves every b464 name
        ...
        prior=read(os.path.join(D, 'b475_closing.txt')),
        addendum=read(os.path.join(D, 'b478_addendum.txt')),
        census=read(os.path.join(D, 'b478_census.txt')),
        span=read(os.path.join(D, 'b478_span_notes.txt')),
        scores=read(os.path.join(D, 'b478_scores.json')),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b478_checks.py')),
        sv=json.loads(read(os.path.join(D, 'b478_survey.json')) or '{}'),
        table=json.loads(read(os.path.join(D, 'b478_table.json')) or '{}'),
        comp=read(os.path.join(D, 'b478_components.txt')),
        refusal=read(os.path.join(D, 'b476_b477_refusal.txt')),
        scan476=read(os.path.join(D, 'b476_ferry_scan.txt')),
        scan477=read(os.path.join(D, 'b477_ferry_scan.txt')),
        ...
        findings_dirty=(gits(PP, 'status', '--porcelain', '--', 'FINDINGS.md') != ''),
        extract_src=read(os.path.join(T, 'b478_extract.py')),
        # ### The regspec file NAMES `b321_window.py` in the clause that FORBIDS running it, and the
        # ### first form of the lane arm read that name as a call. ### The search is confined to the
        # ### two tools that could compute anything at all.
        computing_tools=(read(os.path.join(T, 'b478_extract.py'))
                         + read(os.path.join(T, 'b478_components.py'))),
        tools478=''.join(read(os.path.join(T, f)) for f in sorted(os.listdir(T))
                         if f.startswith('b478_') and f.endswith('.py') and f != 'b478_checks.py'),
        ...
        desk=read(os.path.join(D, 'b478_desk_notes.txt')),
        # ### The first form counted the REFUSED act's own banked ferry and scan as lane outputs.
        # ### A lane output is a COMPUTED bank, not a ferry: json/npy/csv under those act numbers,
        # ### or any file whose name carries `gram`.
        # ### `'gram' in f` also matched `program` and `programmes`: three banks from b182 and b415
        # ### read as lane outputs. ### `gram` is now required to start a word.
        lanefiles=[f for f in os.listdir(D)
                   if (f.startswith(('b476_', 'b477_')) and f.endswith(('.json', '.npy', '.csv'))
                       and 'satisfiable' not in f)
                   or re.search(r'(^|[_-])gram', f.lower())],
        docs=sorted(f for f in os.listdir(D) if f.startswith('b478_')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads',
                                           'mirror-refresh-*-b478.zip'))),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b478')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b478_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b478'):
...
ARMS = [
    ...
    ('G-SISTERS-REFUSED-BANKED', 'the two refused ferries` own scans and the refusal record',
     lambda S: ('carries: NONE' in S['scan476'] and 'carries: NONE' in S['scan477']
                and 'REFUSED UNDER (R81)' in S['refusal'] and 'b478' in S['refusal']),
     lambda S: put(S, 'refusal', '')),
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b475 closing -- the prior closed act',
     lambda S: 'row 324' in S['prior'], lambda S: cut(S, 'prior', 'row 324')),
    ...
    ('G-C1-FORTY-EIGHT-CELLS', 'the table bank',
     lambda S: len(S['table']['cells']) == 48 and len(S['sv']['cells']) == 48,
     lambda S: put(S, 'table', dict(S['table'], cells=dict(list(S['table']['cells'].items())[:40])))),
    ('G-C1-THREE-WORDS-ONLY', 'the table bank',
     lambda S: set(c['verdict'] for c in S['table']['cells'].values()) <= {'SAME OBJECT', 'TOUCHES', 'APART'},
     lambda S: put(S, 'table', dict(S['table'], cells=dict(S['table']['cells'],
                                                           **{'i|K3': dict(verdict='NEARLY')})))),
    ('G-C1-QUOTES-VERIFIED-BOTH-SIDES', 'each quotation against its own source',
     lambda S: all((c['site_found'] and c['k_found']) for c in S['table']['cells'].values()
                   if c['verdict'] != 'APART')
     and all(X478.norm(c['site_words']) in X478.norm(S['row']) for c in S['table']['cells'].values()
             if c['verdict'] != 'APART'),
     lambda S: put(S, 'row', 'a row that says none of it')),
    ('G-C1-REHEARSAL-SAME-OBJECT', 'the survey bank and the components record',
     lambda S: S['table']['cells']['i|K8']['verdict'] == 'SAME OBJECT'
     and 'THE REHEARSAL UNDER (R70)' in S['extract'],
     lambda S: put(S, 'table', dict(S['table'], cells=dict(S['table']['cells'],
                                                           **{'i|K8': dict(S['table']['cells']['i|K8'],
                                                                           verdict='TOUCHES')})))),
    # ### ### **THE FIRST FORM COMPARED POSITIONS INSIDE A CONCATENATION OF FIVE TOOLS**, so the rule in
    # ### one file was measured against a `CELLS = {` in another, and its control then raised on a needle
    # ### it had just removed. ### It now reads THE ONE TOOL IT IS ABOUT, with `find`, not `index`.
    ('G-C1-RULE-BEFORE-CELLS', 'the survey tool`s own source, alone',
     lambda S: (0 <= S['extract_src'].find('SAME OBJECT` -- the site')
                < S['extract_src'].find('CELLS = {')),
     lambda S: put(S, 'extract_src', S['extract_src'].replace('SAME OBJECT` -- the site', 'x'))),
    ('G-C1-NO-BRIDGE-TYPED', 'the components record and the trail',
     # ### The third needle spanned a line break in the components record ("not between two" /
     # ### "### MATHEMATICAL OBJECTS"), so it could never match. ### Shortened to a phrase the bank
     # ### carries on ONE line -- the wrapping trap, caught by the arm itself.
     lambda S: 'NO BRIDGE IS TYPED' in S['comp'] and 'no bridge was typed' in S['ot']
     and 'MATHEMATICAL OBJECTS' in S['comp'],
     lambda S: cut(S, 'comp', 'NO BRIDGE IS TYPED')),

    ('G-C2-REVERSE-READS-BOTH', 'the table bank',
     lambda S: (sorted(S['table']['k_untouched']) == ['K3', 'K6', 'K7']
                and sorted(S['table']['s_untouched']) == ['v', 'vi']),
     lambda S: put(S, 'table', dict(S['table'], k_untouched=['K3']))),
    ('G-C2-EMPTY-ROWS-PRINTED', 'the components record',
     lambda S: 'NO CELL IS ANYTHING BUT APART' in S['comp'] and S['comp'].count('NO CELL IS ANYTHING BUT APART') == 2,
     lambda S: put(S, 'comp', S['comp'].replace('NO CELL IS ANYTHING BUT APART', 'omitted'))),

    ('G-C3-SENTENCE-BOUNDED', 'the components record',
     lambda S: ('WOULD MOVE THE GRADES OF' in S['comp'] and 'WOULD LEAVE' in S['comp']
                and 'IT SAYS NOTHING ABOUT RH' in S['comp']),
     lambda S: cut(S, 'comp', 'IT SAYS NOTHING ABOUT RH')),
    ('G-C3-NO-GRADE-CONFERRED', 'the commit file list and the components record',
     lambda S: 'CONFERS NO' in S['comp'].upper()
     and not any(x.endswith(('FINDINGS.md', 'FACES_LEDGER.md', 'REGISTRY.md')) for x in S['tracked']),
     lambda S: put(S, 'tracked', list(S['tracked']) + ['FINDINGS.md'])),

    ('G-NOLANE-OPENED', 'the data directory and this act`s own tools',
     # ### And a clause STRING naming `b321_window is NOT executed` is not a call of it: the arm now
     # ### looks for an IMPORT or an invocation, not for the name anywhere in any tool's text.
     lambda S: (not S['lanefiles']
                and not re.search(r'^\s*import\s+(numpy|mpmath|scipy)|run\(\[[^]]*b321',
                                  S['computing_tools'], re.M)),
     lambda S: put(S, 'lanefiles', ['b477_gram.json'])),
    ('G-U1-UNEDITED', 'FACES_LEDGER.md`s own working state',
     lambda S: S['faces_dirty'] is False and S['row'].startswith('| U1 |'),
     lambda S: put(S, 'faces_dirty', True)),
    ('G-E0-UNEDITED', 'FINDINGS.md`s own working state',
     lambda S: S['findings_dirty'] is False and '### The E0 gate: every constituent unfolded to its owner' in S['findings'],
     lambda S: put(S, 'findings_dirty', True)),

    ('G-N1-SCORED', 'the scores bank', lambda S: '"N1"' in S['scores'], lambda S: cut(S, 'scores', '"N1"')),
    ('G-N2-SCORED', 'the scores bank',
     lambda S: '"N2"' in S['scores'] and 'SPLIT' in S['scores'], lambda S: cut(S, 'scores', 'SPLIT')),
    ...
    ('G-TRAIL-APPEND-ONLY', 'the trail own text',
     lambda S: S['ot'].count('### b478 \u2014') == 1,
     lambda S: put(S, 'ot', S['ot'] + NL + '### b478 \u2014 a second record that must not exist')),
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger',
     lambda S: '| 325 |' in S['corr'], lambda S: cut(S, 'corr', '| 325 |')),
    ('G-WRITELIST-KINDS', 'every b478 commit in three repositories, against (R85)`s GLOBS',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: "data/b478_components.txt' in gits(ROOT, 'show'" in S['suite'],
     lambda S: cut(S, 'suite', "data/b478_components.txt' in gits(ROOT, 'show'")),
]
...
def main():
    ...
    # ### before the act ran, because THE REFUSED ISSUE OF b478 HAD ITS OWN COMMIT ON main. ### So the
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b478')
              and 'data/b478_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    deferred = [] if pushed else ['G-MIRROR-TAGGED-BUILD']
    S['declared_eq_run'] = (set(names) - set(deferred)) == (set(declared) - set(deferred))

    rec('=' * 104)
    rec('b478 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b478_checks_postpush.txt' if pushed else 'b478_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred),
              io.open(os.path.join(D, 'b478_exercise.json'), 'w', encoding='utf-8', newline=NL),
