# -*- coding: utf-8 -*-
"""b340_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b340_registration_2026-09-06.txt')
SPEC = os.path.join(ROOT, 'data', 'b340_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ("PLACE-papers files written", 1, 1, "files", "### section (G): FACES_LEDGER.md, one block through the writer."),
    ("ERRATA entries written", 0, 0, "entries", "none."),
    ("keystone or bench edits", 0, 0, "edits", "### section (G): owner files read, never edited."),
    ("correspondence rows appended", 1, 1, "rows", "### section (G)."),
    ("findings sections edited", 0, 0, "sections", "none."),
    ("bars moved", 0, 0, "bars", "### section (D): a bar not met is not rewritten; must-fail fixture."),
    ("grades conferred by a seat", 0, 0, "grades", "section (A)."),
    ("indices added or removed after a value", 0, 0, "indices", "### section (F4): the keystone's rows."),
...
def main(argv):
    print('=' * 100)
    print('b340_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": "data/b340_registration_2026-09-06.txt -- b340, THE LI FAMILY CONTROL", "clauses": clauses}
