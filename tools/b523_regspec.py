# -*- coding: utf-8 -*-
"""b523_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b522's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the
### ### numerical clauses re-read for this act's three re-pricings of b523's cells; no prior bank is touched.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b523_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b523_satisfiable.json')
...
CLAUSES = [
    ...
    ('chains run, or channel values computed', 0, 0, 'runs', '### (B)(1)-(4): sigma_max, the count bound, and b522`s 33 banked cells re-read under the re-priced tail -- no chain run, no channel recomputed.'),
    ('lean runs', 0, 0, 'runs', '### (Z).'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z).'),
    ('lanes opened', 1, 1, 'lanes', '### (R132)(5): the numerical lane for this act alone.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b523.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R132).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b523_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b523_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b523_extract.txt'),
    ...
    spec = {"registration": ("data/b523_registration_2026-09-25.txt -- b523, "
                             "THE WITNESS`S THREE CAVEATS PRICED"),
            "clauses": clauses}
