# -*- coding: utf-8 -*-
"""b505_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE.
...
FACE = os.path.join(D, 'b505_registration_2026-09-23.txt')
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b505_ferry.txt')),
        scan=read(os.path.join(D, 'b505_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b505_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b505_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b505_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b505_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b504_closing.txt')),
        addendum=read(os.path.join(D, 'b505_addendum.txt')),
        comp=read(os.path.join(D, 'b505_components.txt')),
        desk=read(os.path.join(D, 'b505_desk_notes.txt')),
        span=read(os.path.join(D, 'b505_span_notes2.txt')) + read(os.path.join(D, 'b505_span_notes.txt')),
        c1=json.loads(read(os.path.join(D, 'b505_c1.json')) or '{}'),
        c2=json.loads(read(os.path.join(D, 'b505_c2_results.json')) or '{}'),
        c2log=read(os.path.join(D, 'b505_c2_log.txt')),
        diag=read(os.path.join(D, 'b505_c2_fixture_diagnosis.txt')),
        edges_exist=os.path.exists(os.path.join(D, 'b505_edges.jsonl')),
        c2_exists=os.path.exists(os.path.join(D, 'b505_c2_results.json')),
        ann_unchanged=(gits(ROOT, 'status', '--porcelain', '--', 'data/b477_components.txt', 'data/b502_components.txt',
                            'data/b504_components.txt', 'data/b502_ferry.txt') == ''),
        ann=json.loads(read(os.path.join(D, 'b505_annotations.json')) or '[]'),
        annfiles={n: read(os.path.join(D, n)) for n in ('b477_components.txt', 'b502_components.txt', 'b504_components.txt', 'b502_ferry.txt')},
        sc=json.loads(read(os.path.join(D, 'b505_scores.json')) or '{}'),
        cells=sorted([json.loads(l) for l in read(os.path.join(D, 'b505_cells.jsonl')).split(NL) if l.strip()], key=lambda r: r['a']),
        tool=read(os.path.join(T, 'b505_complete.py')), ktool=read(os.path.join(T, 'b505_kernel.py')), dtool=read(os.path.join(T, 'b505_decomp.py')),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b505_checks.py')),
        artefacts_tracked=gits(ROOT, 'ls-files', 'data/anthropic-zeta23'),
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b505.zip'))),
        ...
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b505 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b505_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b505 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-4]_', f)]
...
MARKA = 'ANNOTATION APPENDED AT b505 UNDER (R115)'

ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry', lambda S: 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b505' in S['ferry'],
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b504`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 353 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 353 |', '| 3530 |'))),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b505' in S['ferry'] and 'ACT b505' in S['face'] and not os.path.exists(os.path.join(D, 'b506_registration_2026-09-23.txt')),
     lambda S: cut(S, 'face', 'ACT b505')),
    ('G-PEEK-DECLARED', 'the face -- the pre-seal reads declared, and no count read before the seal',
     lambda S: 'NO ARGUMENT-PRINCIPLE COUNT, NO DECOMPOSITION AND NO EPSTEIN RESIDUAL WAS READ BEFORE THE SEAL' in S['face']
     and 'NO WINDING WAS' in S['face'],
     lambda S: cut(S, 'face', 'NO ARGUMENT-PRINCIPLE COUNT, NO DECOMPOSITION AND NO EPSTEIN RESIDUAL WAS READ BEFORE THE SEAL')),
    ('G-PRICE-ON-FACE', 'the face`s (C2) block and the log`s stamps -- the run ended inside the twenty minutes',
     lambda S: '(C2) THE PRICE, UNDER (R115)(5)' in S['face'] and log_span(S['c2log']) is not None and 0 <= log_span(S['c2log']) < 1200,
     lambda S: put(S, 'c2log', S['c2log'] + '[2026-09-24 03:00:00] late line' + NL)),
    ('G-R115-ENTERED', 'the banked ferry AND the trail',
     lambda S: 'RULING (R115) END' in S['ferry'] and S['ot'].count('**(R115) ratified**, as amended a second time') == 1,
     lambda S: put(S, 'ot', S['ot'].replace('**(R115) ratified**', '(R115) noted'))),
    ('G-DECOMP-EXACT', 'Component 1`s bank -- c1, c2 as fractions and the exact test over 10^4',
     lambda S: S['c1'].get('N') == 10000 and S['c1'].get('c1_frac') == '2/3' and S['c1'].get('c2_frac') == '4/3'
     and S['c1'].get('exact_fail') == 0 and 'Fraction(r[n]) != f1 * A[n] + f2 * a[n]' in S['dtool'],
     lambda S: put(S, 'c1', dict(S['c1'], exact_fail=3))),
    ('G-LAMQ-COMPARED', 'Component 1`s bank and tool -- b325`s LAMQ against the exact inverse, 2..4096',
     lambda S: S['c1'].get('lam_n') == 4095 and S['c1'].get('lam_agree') == 4095 and 'EP.von_mangoldt_q(K=NL4' in S['dtool'],
     lambda S: cut(S, 'dtool', 'EP.von_mangoldt_q(K=NL4')),
    ('G-EVALUATOR-FIXTURE', 'the log`s fixture LINE and refusal LINE, and the tool`s registered bar',
     lambda S: 'at all 148 banked zeros, largest 4.90e-10' in line_with(S['c2log'], 'fixture:')
     and '### FIXTURE FAILED ; no count read' in line_with(S['c2log'], 'FIXTURE FAILED') and 'if not (wz < 1e-10 and wr < 1e-8):' in S['tool'],
     lambda S: put(S, 'c2log', S['c2log'].replace('### FIXTURE FAILED ; no count read', 'fixture passed'))),
    ('G-SIGMA-MAX-BOUND', 'the tool, the face and the log -- sigma_max 1.5 from the printed bound',
     lambda S: 'SIG_MAX = 1.5 ' in S['tool'] and '1.5497 < r(1) = 2' in S['face'] and 'SIG_MAX 1.50' in line_with(S['c2log'], 'start'),
     lambda S: put(S, 'tool', S['tool'].replace('SIG_MAX = 1.5 ', 'SIG_MAX = 1.2 '))),
    ('G-WHOLE-STRIP', 'the data directory -- NO edge bank: no contour was wound after the refusal',
     lambda S: not S['edges_exist'] and 'COUNT :' not in S['c2log'], lambda S: put(S, 'edges_exist', True)),
    ('G-BOXES-INTEGER', 'the log -- no box line, because none was computed',
     lambda S: 'boxes ' not in S['c2log'] and 'NOT REACHED' in S['comp'], lambda S: put(S, 'c2log', S['c2log'] + 'boxes 450' + NL)),
    ('G-COUNT-VS-MAIN', 'the data directory -- NO results bank; the count was not read',
     lambda S: not S['c2_exists'] and 'N(150)' not in S['c2log'], lambda S: put(S, 'c2_exists', True)),
    ('G-LINE-VS-BANK', 'the log -- no M-column line, three lines in all',
     lambda S: 'M column' not in S['c2log'] and len(S['c2log'].strip().split(NL)) == 3,
     lambda S: put(S, 'c2log', S['c2log'] + 'M column : x' + NL)),
    ('G-MISSING-LOCATED', 'the log -- nothing located', lambda S: 'locating' not in S['c2log'] and 'zero [' not in S['c2log'],
     lambda S: put(S, 'c2log', S['c2log'] + 'locating : 3 boxes' + NL)),
    ('G-ROUTE-A-CONFIRMS', 'the banked diagnosis and the fixture LINE -- route A`s agreement and the two zeros over the bar, by name',
     lambda S: 'over 1e-10: 2 of 148' in S['diag'] and '149.342745974862' in S['diag'] and '149.718007585845' in S['diag']
     and '4.90e-11' in line_with(S['c2log'], 'fixture:'),
     lambda S: put(S, 'diag', S['diag'].replace('over 1e-10: 2 of 148', 'over 1e-10: 0 of 148'))),
    ('G-POSITIVE-CONTROL', 'the diagnosis -- Newton from the worst banked ordinate reaches the zero',
     lambda S: 'e-38' in line_with(S['diag'], 'Newton from the worst'),
     lambda S: put(S, 'diag', S['diag'].replace('e-38', 'e-05'))),
    ('G-REGGATE-REEXAMINED', 'the components bank -- the verdict LINE',
     lambda S: 'NOT REACHED' in line_with(S['comp'], 'REGISTERED GATE RE-EXAMINED'),
     lambda S: put(S, 'comp', S['comp'].replace('REGISTERED GATE RE-EXAMINED', 'registered gate'))),
    ('G-C3-GATED', 'the kernel tool`s refusal, and NO cells bank',
     lambda S: 'COMPONENT 2 DID NOT CLOSE THE BANK -- COMPONENT 3 IS NOT RUN.' in S['ktool'] and len(S['cells']) == 0 and not S['c2_exists'],
     lambda S: put(S, 'cells', [dict(a=5.0)])),
    ('G-C3-GRID-RULE', 'the kernel tool`s text -- the rule at T = 150, as the face reads it',
     lambda S: 'while 2 * math.pi * (nv - 1) / L <= TQ + 200.0:' in S['ktool'] and 'TQ = 150.0' in S['ktool'],
     lambda S: cut(S, 'ktool', 'TQ = 150.0')),
    ('G-C3-TAIL-TESTED', 'the kernel tool`s text -- the tail bounded at the places side`s u-range',
     lambda S: 'np.linspace(TQ, float(CH.AT.UMAX)' in S['ktool'] and 'tail_inside=tr <= B' in S['ktool'],
     lambda S: cut(S, 'ktool', 'tail_inside=tr <= B')),
    ('G-C3-STOP-HONOURED', 'the components bank -- Component 3 NOT RUN, said', lambda S: 'COMPONENT 3 -- NOT RUN' in S['comp'],
     lambda S: cut(S, 'comp', 'COMPONENT 3 -- NOT RUN')),
    ('G-ANNOT-UNVERIFIED', 'the three components files -- NO b505 note, under (R116)(1)',
     lambda S: all(MARKA not in S['annfiles'][n] for n in ANNF) and 'NOT EXECUTED, under (R116)(1)' in S['comp'],
     lambda S: put(S, 'annfiles', dict(S['annfiles'], **{'b502_components.txt': S['annfiles']['b502_components.txt'] + MARKA}))),
    ('G-ANNOT-B502-FERRY', 'b502`s ferry -- untouched, no b505 note', lambda S: MARKA not in S['annfiles']['b502_ferry.txt']
     and S['annfiles']['b502_ferry.txt'].rstrip(NL).endswith('FERRY END (part 2 of 2)'),
     lambda S: put(S, 'annfiles', dict(S['annfiles'], **{'b502_ferry.txt': S['annfiles']['b502_ferry.txt'] + MARKA}))),
    ('G-ANNOT-PREFIX', 'the four files against their committed blobs, and no annotation record',
     lambda S: S['ann_unchanged'] and not S['ann'], lambda S: put(S, 'ann_unchanged', False)),
    ('G-N1-SCORED', 'the desk against Component 1', lambda S: 'n1' in (S['sc'] or {}) and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == (S['c1'].get('c1_frac') == '2/3' and S['c1'].get('c2_frac') == '4/3' and S['c1'].get('exact_fail') == 0
                           and S['c1'].get('lam_agree') == 4095),
     lambda S: put(S, 'sc', dict(S['sc'], n1=not (S['sc'] or {}).get('n1')))),
    ('G-N2-SCORED', 'the desk against the missing count -- NOT SCORABLE', lambda S: (S['sc'] or {}).get('n2') == 'NOT SCORABLE'
     and desk_word(S, 'N2') == 'NOT SCORABLE' and not S['c2_exists'], lambda S: put(S, 'sc', dict(S['sc'], n2=True))),
    ('G-N3-SCORED', 'the desk against the missing cells -- NOT SCORABLE', lambda S: (S['sc'] or {}).get('n3') == 'NOT SCORABLE'
     and desk_word(S, 'N3') == 'NOT SCORABLE' and len(S['cells']) == 0, lambda S: put(S, 'sc', dict(S['sc'], n3=False))),
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, three NOT SCORABLE',
     lambda S: "THE SEAT`S OWN EXPECTATIONS" in S['face'] and 'REGISTERED 3 ; HELD 0 ; REFUTED 0 ; NOT SCORABLE 3' in S['desk']
     and all(desk_word(S, t) == 'NOT SCORABLE' for t in ('S1', 'S2', 'S3')),
     lambda S: put(S, 'desk', S['desk'].replace('**(S1)** ### **NOT SCORABLE', '**(S1)** ### **HELD'))),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record',
     lambda S: 'where the deposit left it' in seg(S['ot'], '### b505 —', 99999).replace(NL, ' '),
     lambda S: put(S, 'ot', S['ot'].replace('### b505 —', '### b505 -'))),
    ...
    ('G-FOUR-LISTS-OPEN', 'the trail`s own text -- THIS act`s record',
     lambda S: 'the four lists stay OPEN' in seg(S['ot'], '### b505 —', 99999).replace(NL, ' '),
     lambda S: put(S, 'ot', S['ot'].replace('stay OPEN', 'are closed'))),
    ('G-LANE-SHUT', 'the trail`s own record -- the lane shut, said',
     lambda S: 'The numerical lane shuts at this act' in seg(S['ot'], '### b505 —', 99999).replace(NL, ' '),
    ...
    ('G-TRAIL-APPEND-ONLY', 'the trail`s own text', lambda S: S['ot'].count('### b505 —') == 1,
     lambda S: put(S, 'ot', S['ot'] + NL + '### b505 — a second record')),
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger',
     lambda S: S['corr'].count('| 354 |') == 1 and S['corr'].count('(b505, under (R115))') == 1,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 354 | a duplicate row |')),
    ('G-WRITELIST-KINDS', 'every b505 commit in three repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b505')" in S['suite']
                and "data/b505_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b505_components.txt' in gits(ROOT, 'show'")),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b505')
              and 'data/b505_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b505 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b505_checks_postpush.txt' if pushed else 'b505_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b505_exercise.json'), 'w', encoding='utf-8', newline=NL),
