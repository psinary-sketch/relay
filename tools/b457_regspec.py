# -*- coding: utf-8 -*-
"""b457_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b457_registration_2026-09-14.txt')
SPEC = os.path.join(ROOT, 'data', 'b457_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 0, 0, 'files', '### section (W): RouteProfile.lean lies outside every repository.'),
    ('kernel builds', 1, 1, 'builds', '### reading (1): the print`s prerequisite, in the scratch clone, part of the one run.'),
    ('repositories cloned', 1, 1, 'repositories', '### reading (1): a local clone of D:\SIDE-kernel, no remote.'),
    ('network remotes contacted', 0, 0, 'remotes', '### reading (1): the pins` ls-remote reads and the ritual`s pushes excepted.'),
    ('scratch trees left at the lane`s close', 0, 0, 'trees', '### reading (1): D:\b457-tag-run removed.'),
    ('branches merged or fetched', 0, 0, 'branches', '### section (Z): the push branch excepted.'),
    ('detached checkouts made', 1, 1, 'checkouts', '### reading (1): the tag`s commit, in the scratch clone.'),
    ...
    ('keystones edited or annotated', 1, 1, 'keystones', '### (W): THE_RESIDUE_OF_RH.'),
    ('state terms changed on a line the author`s ruling did not name', 0, 0, 'terms', '### (K) BAR 5.'),
    ('dispositions taken beyond those ruled', 0, 0, 'dispositions', '### (R66)-(R68).'),
    ('exit codes used as a verdict', 0, 0, 'verdicts', '### (K) BAR 2.'),
    ('waves opened', 0, 0, 'waves', '### (R66), (K) BAR 3.'),
    ('sentences graded by the gate', 0, 0, 'sentences', '### reading (3): priced, not built.'),
    ('dispositions recommended', 0, 0, 'dispositions', '### (K) BAR 4.'),
    ('ERRATA entries written', 0, 0, 'entries', '### section (W).'),
    ('live surfaces narrowed', 0, 0, 'surfaces', '### section (Z).'),
    ('live notes inserted under (R67)', 1, 0, 'notes', '### reading (1): README.md, only if every terminal is STANDARD THREE.'),
    ...
    ('prior banks edited, b456`s among them', 0, 0, 'files', '### section (Z).'),
    ('annotations written by this act', 1, 1, 'annotations', '### reading (2): one currency annotation.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### section (W): mirror-refresh-2026-09-14-b457.zip.'),
    ...
    ('readings of the order declared in advance on this face', 3, 3, 'readings', '### section (A): (1) to (3).'),
    ('pre-face reads declared', 5, 5, 'reads', '### section (A): (P1) to (P5).'),
    ...
    ('author rulings ratified and entered', 3, 3, 'rulings', '### section (A): (R66), (R67), (R68).'),
    ('author rulings executed', 2, 2, 'rulings', '### readings (1) and (2): (R67), (R68).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b457_addendum.txt.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (K) BAR 10.'),
    ('lanes opened', 1, 1, 'lanes', '### section (A): the KERNEL lane, for one lean run, closing at its end.'),
    ...
    ('expectations registered', 2, 2, 'expectations', '### section (J): (N1), (N2).'),
    ...
    ('corpus documents written other than by append', 2, 1, 'documents', '### (W): the cells of THE_RESIDUE_OF_RH; README`s conditional note.'),
    ('existing lines of the corpus changed', 4, 4, 'lines', '### reading (2): THE_RESIDUE_OF_RH.md:127-130.'),
    ('statements of a run reported as its output', 0, 0, 'statements', '### reading (1).'),
    ('lines removed from any document', 0, 0, 'lines', '### reading (1): by the predicate on the face.'),
    ('lean runs', 1, 1, 'runs', '### reading (1): (R67).'),
...
def main(argv):
    print('=' * 100)
    print('b457_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b457_registration_2026-09-14.txt -- b457, "
                             "THE PROFILES PRINTED AT THE TAG, AND THE SENTENCE GATE PRICED"),
            "clauses": clauses}
