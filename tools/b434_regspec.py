# -*- coding: utf-8 -*-
"""b434_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b434_registration_2026-09-12.txt')
SPEC = os.path.join(ROOT, 'data', 'b434_satisfiable.json')
...
CLAUSES = [
    ...
    ('author rulings ratified by this paste', 0, 0, 'rulings', '### section (A): none; (R41)-(R45) were ratified at b433.'),
    ('rulings extended beyond their own words', 0, 0, 'rulings', '### section (A).'),
    ('lane documents edited', 0, 0, 'documents', '### section (Z).'),
    ('lanes opened', 0, 0, 'lanes', '### (K) BAR 8.'),
    ('addresses resolved or documents fetched', 0, 0, 'reads', '### section (Z): nothing is fetched by this act.'),
    ('candidate zeros, seeds or arrangements constructed', 0, 0, 'candidates', '### section (Z).'),
    ('corpus .lean files edited', 0, 0, 'files', '### (K) BAR 4.'),
    ...
    ('new `relay` act-tool files', 7, 7, 'files', '### (W): six act tools and the fold generator.'),
...
def main(argv):
    print('=' * 100)
    print('b434_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b434_registration_2026-09-12.txt -- b434, THE EXTERNAL GRADING READ"),
            "clauses": clauses}
