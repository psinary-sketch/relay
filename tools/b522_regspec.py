# -*- coding: utf-8 -*-
"""b522_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b521's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the
...
REG = os.path.join(ROOT, 'data', 'b522_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b522_satisfiable.json')
...
CLAUSES = [
    ...
    ('chains run, or channel values computed', 1, 1, 'runs', '### (B)(0)-(1): the chain on variant (B) at p = 7, Q0 alone, every width of a = 15..60 computed, the IN widths read.'),
    ('lean runs', 0, 0, 'runs', '### (Z).'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z).'),
    ('lanes opened', 1, 1, 'lanes', '### (R131)(4): the numerical lane for this act alone.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b522.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R131).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b522_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b522_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b522_extract.txt'),
    ...
    spec = {"registration": ("data/b522_registration_2026-09-25.txt -- b522, "
