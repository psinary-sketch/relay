# -*- coding: utf-8 -*-
"""b304_kernel.py -- THE SHADOW'S BUILD, ITS PROFILE, AND THE BYTE COMPARISONS.
...
### ### **AND ITS ONE-SHOT LIMIT IS INHERITED TOO:** ### once the profile has moved, the
### precondition "the working tree profile is identical to `HEAD`" is false and the tool stops.
...
ALLP = K.ALLP
PROFILE = K.PROFILE
MODULE = os.path.join(SGS, 'Core', 'IndexRangeShadow.lean')
MODNAME = 'IndexRangeShadow'
NS = 'B304'
OUT = os.path.join(ROOT, 'data', 'b304_kernel_run.txt')
BOM = K.BOM
...
def main():
    ...
    K.rec = rec
    rec('=' * 100)
    rec('b304 -- THE SHADOW BUILD AND ITS PROFILE. ### EVERY NUMBER FROM THIS FILE.')
    rec('=' * 100)
