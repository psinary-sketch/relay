# -*- coding: utf-8 -*-
"""b505_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b505_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b505_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 0, 0, 'files', '### (W): the probe `b505_printaxioms.lean` is written by the launched process into relay`s data directory, which is not a repository of Lean, and is not committed; no `.lean` file of any kernel is written.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): none; the build read is b498`s and is over.'),
    ('chains run, or channel values computed', 3, 3, 'runs', '### (B)(1)-(3): the decomposition, the argument-principle count, and the derived kernel on the completed bank (the last refusing unless the bank closes).'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### (R115)(5): the numerical lane opens for this act alone and shuts at its close.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b505.zip, under (R69).'),
    ...
    ('new instruments built', 1, 1, 'instruments', '### (B)(2): the Chowla-Selberg evaluator and the argument-principle count, in b505_complete.py, fixtured against route A before any count is read.'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b505_extract.txt`.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R115) as amended a second time -- the Epstein margins UNVERIFIED; the control of record Q0; the kernel tested only on a complete bank; one function`s zeros with its own places side; the lane for b505; premises read from their files.'),
    ('author rulings recorded as entries by this act', 0, 0, 'rulings', '### (Z): (R115) is ratified in the ferry and entered by the trail record.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b505_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('new `relay` act-tool files named on this face', 9, 9, 'files', '### (W): the stem glob (R91) permits, so the bound is on the STEM.'),
...
def main(argv):
    print('=' * 100)
    print('b505_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b505_extract.txt'),
    ...
    spec = {"registration": ("data/b505_registration_2026-09-23.txt -- b505, "
                             "THE EPSTEIN CONTROL FOR Q0, ITS BANK COUNTED BELOW 150"),
            "clauses": clauses}
