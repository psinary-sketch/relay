# -*- coding: utf-8 -*-
"""b424_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b424_registration_2026-09-11.txt')
SPEC = os.path.join(ROOT, 'data', 'b424_satisfiable.json')
...
CLAUSES = [
    ...
    ('rows of `FACES_LEDGER.md` written', 0, 0, 'rows', '### (K) BAR 6: a block names the row.'),
    ...
    ('candidates in the population', 16, 16, 'candidates', '### (A) reading (2): four and twelve.'),
    ('candidates neither failed at a quoted step nor held', 0, 0, 'candidates', '### (K) BAR 3.'),
    ('candidates read twice or computed on', 0, 0, 'candidates', '### (A) reading (3), the cap.'),
    ('sites attempted', 1, 1, 'sites', '### (K) BAR 7.'),
    ('seventh sites entered', 0, 0, 'sites', '### (K) BAR 7.'),
    ('bytes of row U1`s line changed', 0, 0, 'bytes', '### (K) BAR 6.'),
    ('witness fields written with no candidate held', 0, 0, 'fields', '### (K) BAR 5.'),
    ('ledger blocks appended', 1, 1, 'blocks', '### (W) KIND 6, the exhausted branch.'),
...
def main(argv):
    print('=' * 100)
    print('b424_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b424_registration_2026-09-11.txt -- b424, SORTIE LEG 2, THE WITNESS ARC AT SITE (i)"),
            "clauses": clauses}
