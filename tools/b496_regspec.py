# -*- coding: utf-8 -*-
"""b496_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b496_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b496_satisfiable.json')
...
CLAUSES = [
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b496.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b496_extract.txt`.'),
    ('readings taken silently', 0, 0, 'readings', '### section (A).'),
    ('survey anchor misses', 0, 0, 'misses', '### section (A): 0.'),
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### section (C): the four readings taken to write this face truthfully are DECLARED THERE -- the roster count, the two artefact shapes, the grade-word counts per ledger, and the kernel table`s sparse pin column -- each as a prediction the components re-derive; and section (C2) declares the four index queries and their four misses.'),
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R107) -- THE KEYSTONE REFRESH PROGRAMME OPENED AS ACTS IN FOUR CHECKPOINTS, K0 running now because it reads and builds nothing, K1-K3 held until the (R104) acts through h2-as-one-Prop have closed; and the table K0 emits is an INSTRUMENT OUTPUT regenerated at every close, not a tracking document. ### RE-POINTED FROM b495`s (R106).'),
    ('author rulings recorded as entries by this act', 0, 0, 'rulings', '### (Z): (R107) is ratified in the ferry and entered by the trail record.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b496_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written at all', 1, 1, 'documents', '### (W): OPEN_TRAILS.md ALONE -- one appended record. ### THIS ACT WRITES NO REGISTRY ROW, NO SPIRAL_MAP NOTE, NO FINDINGS SECTION AND NO ERRATA LINE; its only other emissions are relay/data instrument files. ### THE CLAUSE WAS CARRIED FROM b495 READING 3/3, WHERE IT WAS TRUE, AND IT AGREED WITH ITSELF WHILE CONTRADICTING THIS ACT`S FACE -- a carried demand and a carried cap are satisfiable together no matter what the act does. ### The face is unchanged; the INSTRUMENT was wrong and is repaired, and BOTH RUNS ARE BANKED.'),
    ...
    ('existing `relay` tools edited in place', 1, 1, 'tools', '### (W): ONE -- the CLOSING SUITE gains a call to the shared generator, which (R107) requires in its own words: *"the generator added to the closing suite so every later close regenerates it and diffs against the prior run"*. ### corr_row.py is CARRIED as b488 guarded it and is NOT edited. ### RAISED FROM b495`s 0: an act whose ruling says ADD TO THE SUITE edits a tool, and a cap of 0 would have made its own order unsatisfiable.'),
...
def main(argv):
    print('=' * 100)
    print('b496_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b496_extract.txt'),
    ...
    spec = {"registration": ("data/b496_registration_2026-09-23.txt -- b496, "
