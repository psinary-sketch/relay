# -*- coding: utf-8 -*-
"""b341_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b341_registration_2026-09-06.txt')
SPEC = os.path.join(ROOT, 'data', 'b341_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ("PLACE-papers files written", 1, 1, "files", "### section (G): ERRATA.md, one appended entry, only on a naming verdict."),
    ("ERRATA entries edited", 0, 0, "entries", "### section (D): appended, never edited."),
    ("owner files edited (bench, keystone, FINDINGS)", 0, 0, "files", "### section (A)."),
    ("faces ledger rows or blocks written", 0, 0, "blocks", "### section (G): no row's status moves."),
    ("correspondence rows appended", 1, 1, "rows", "### section (G)."),
    ("findings sections edited", 0, 0, "sections", "none."),
    ("PDFs committed to any repo", 0, 0, "files", "### section (C): scratchpad only."),
...
def main(argv):
    print('=' * 100)
    print('b341_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": "data/b341_registration_2026-09-06.txt -- b341, THE TWO COEFFICIENTS", "clauses": clauses}
