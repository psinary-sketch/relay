# -*- coding: utf-8 -*-
"""b515_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b514's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b515_registration_2026-09-24.txt')
OT = os.path.join(PP, 'OPEN_TRAILS.md')
PRIOR_RELAY = '28f255ac'      # ### b514's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
KER_TIP = '0a9391e'           # ### the kernel's tip, untouched by this act
PRIOR_PP = 'ec42cad'           # ### b514's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b515 —'
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b515_ferry.txt')),
        scan=read(os.path.join(D, 'b515_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b515_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b515_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b515_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b515_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b514_closing.txt')),
        addendum=read(os.path.join(D, 'b515_addendum.txt')),
        comp=read(os.path.join(D, 'b515_components.txt')),
        desk=read(os.path.join(D, 'b515_desk_notes.txt')),
        report=read(os.path.join(D, 'b515_report.txt')),
        cells=sorted(jsonl(os.path.join(D, 'b515_cells.jsonl')), key=lambda c: c['a']),
        ladder=sorted(c['a'] for c in jsonl(os.path.join(D, 'b511_cells.jsonl')) if c.get('family') == 'bspline'),
        res=json.loads(read(os.path.join(D, 'b515_results.json')) or '{}'),
        fx=json.loads(read(os.path.join(D, 'b515_fixture.json')) or '[]'),
        mt=json.loads(read(os.path.join(D, 'b515_matched.json')) or '[]'),
        sc=json.loads(read(os.path.join(D, 'b515_scores.json')) or '{}'),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b515_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b515.zip'))),
        ...
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b515 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b515_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (os.path.join(D, x) for x in ('b515_fixture.json', 'b515_matched.json', 'b515_cells.jsonl'))),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b515 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[01234]_|^b334_', f)]
...
def hhat(a, g0, z):
    ...
    return (g0 * g0 - z * z) * csinc(z * s / 2.0) ** 6
...
def recount(S, o):
    ...
    up = set(round(c['a'], 9) for c in C[60:])
    ...
    return dict(verified=len(V), m_neg=[c['a'] for c in V if c[o]['m'] < -c[o]['B']],
                ...
                h2_neg_upper=[a for a in neg if round(a, 9) in up],
...
def corr_row(S):
    return next((l for l in S['corr'].split(NL) if l.startswith('| 364 |')), '')


import numpy as np  # noqa: E402

DELTA = 0.9532604747946607 - 0.5
HARD5 = 'no Q0 cell is negative on a closing bank: the witness construction h2_sign_imp_rh names is not exhibited at this rho by this window'

ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R124) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b515' in S['ferry'],
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b514`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 363 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 363 |', '| 3630 |'))),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b515' in S['ferry'] and 'ACT b515' in S['face'] and not glob.glob(os.path.join(D, 'b516_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b515')),
    ...
    ('G-R124-ENTERED', 'the banked ferry AND the trail', lambda S: 'RULING (R124) END' in S['ferry'] and S['ot'].count('**(R124) ratified') == 1,
     lambda S: put(S, 'ot', S['ot'].replace('**(R124) ratified', '(R124) noted'))),
    ('G-WORKORDER-FILED', 'the trail`s own record -- the work-order by name, once, not attempted',
     lambda S: trail(S).count('`W-ORD-WEIL-CONVERSE`') == 1 and 'priced at the fold, not attempted' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace('`W-ORD-WEIL-CONVERSE`', 'a work-order'))),
    ('G-FIXTURE-BAR', 'the fixture bank -- the largest difference recomputed from its two columns, both objects, against 1e-10',
     lambda S: len(S['fx']) == 2 and all(max(abs(a - b) for a, b in zip(f['closed'], f['numeric'])) <= 1e-10 for f in S['fx']),
    ...
    ('G-CLASSK-STATED', 'the fixture bank AND the components -- generating h named, k even, h C^2 with a live probe',
     lambda S: all(f['generating_h'].startswith('h(u) = phi``(u) + ') and f['order'] == 6 and f['even'] < 1e-12 * f['scale']
                   and f['knot_jumps']['D3'] > 1.0 and max(f['knot_jumps'][d] for d in ('D0', 'D1', 'D2')) < 1e-4 for f in S['fx'])
     and S['comp'].count('classK (READING (2)): k = weilTest h h with h = h(u) = phi') == 2,
    ...
    ('G-ZERO-CONTROL', 'the control bank -- three ladder widths, two objects; h-hat(gamma_0) zero, the slope recomputed here',
     ...
     and all(rel(m['slope'], (-2.0 * m['gamma0'] * csinc(m['gamma0'] * math.log(m['a']) / 6.0) ** 6).real)
    ...
    ('G-SIGNS-RECOUNTED', 'the results` sign lists against the cells, recounted',
     lambda S: all(S['res'][o][k] == recount(S, o)[k] for o in ('q', 'xi')
                   for k in ('m_neg', 'm_pos', 'h2_neg', 'h2_pos', 'h2_neg_upper', 'pair_neg', 'pair_pos')),
    ...
    ('G-READING-WORDS', 'the results` reading against the recount AND the trail -- (R122)(2)`s words',
     lambda S: (S['res'].get('reading') == HARD5 if not recount(S, 'q')['h2_neg'] else WIT in S['res'].get('reading', ''))
    ...
    ('G-N2-SCORED', 'the desk against the recount', lambda S: 'n2' in S['sc'] and desk_word(S, 'N2') == word_of(S['sc']['n2'])
     and S['sc']['n2'] == (len(recount(S, 'q')['h2_neg_upper']) >= 1), lambda S: put(S, 'sc', dict(S['sc'], n2=not S['sc'].get('n2')))),
    ('G-N3-SCORED', 'the desk against the recount', lambda S: 'n3' in S['sc'] and desk_word(S, 'N3') == word_of(S['sc']['n3'])
     and S['sc']['n3'] == (recount(S, 'xi')['verified'] > 0 and len(recount(S, 'xi')['h2_pos']) == recount(S, 'xi')['verified']),
    ...
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from the banks',
     ...
     and desk_word(S, 'S1') == word_of(bool(recount(S, 'q')['pair_neg']) and bool(recount(S, 'q')['pair_pos']))
     and desk_word(S, 'S2') == word_of(recount(S, 'q')['verified'] > 0 and len(recount(S, 'q')['h2_pos']) == recount(S, 'q')['verified'])
     and desk_word(S, 'S3') == word_of(bool(S['mt']) and all(abs(complex(*m['at_gamma0'])) < 1e-12 * m['line_peak'] for m in S['mt'])),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b515 -'))),
    ...
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 364 once, six cells as it landed',
     lambda S: S['corr'].count('| 364 |') == 1 and S['corr'].count('(b515, under (R124))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 364 | a duplicate row |')),
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b514`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b514_window.py'])),
    ('G-WRITELIST-KINDS', 'every b515 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b515')" in S['suite']
                and "data/b515_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b515_components.txt' in gits(ROOT, 'show'")),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b515')
              and 'data/b515_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b515 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b515_checks_postpush.txt' if pushed else 'b515_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b515_exercise.json'), 'w', encoding='utf-8', newline=NL),
