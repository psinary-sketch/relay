# -*- coding: utf-8 -*-
"""b520_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b518's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b520_registration_2026-09-24.txt')
OT = os.path.join(PP, 'OPEN_TRAILS.md')
PRIOR_RELAY = '60a287a8'      # ### b519's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
KER_TIP = 'ffc09da'           # ### the kernel's tip, untouched by this act
PRIOR_PP = '10f9125'           # ### b519's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b520 —'
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b520_ferry.txt')),
        scan=read(os.path.join(D, 'b520_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b520_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b520_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b520_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b520_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b519_closing.txt')),
        addendum=read(os.path.join(D, 'b520_addendum.txt')),
        comp=read(os.path.join(D, 'b520_components.txt')),
        desk=read(os.path.join(D, 'b520_desk_notes.txt')),
        report=read(os.path.join(D, 'b520_report.txt')),
        rows=sorted(jsonl(os.path.join(D, 'b520_cells.jsonl')), key=lambda c: c['a']),
        reach=json.loads(read(os.path.join(D, 'b520_reach.json')) or '{}'),
        tool=read(os.path.join(T, 'b520_widen.py')),
        res=json.loads(read(os.path.join(D, 'b520_results.json')) or '{}'),
        sc=json.loads(read(os.path.join(D, 'b520_scores.json')) or '{}'),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b520_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b520.zip'))),
        chain_clean=(gits(ROOT, 'status', '--porcelain', '--', 'tools/b321_window.py', 'tools/e16/carto_atlas.py',
                          ...
                          'tools/b504_chain.py', 'tools/b506_kernel.py', 'tools/b511_families.py', 'tools/b503_zero.py', 'tools/b514_window.py', 'tools/b515_window.py', 'tools/b518_window.py', 'tools/b519_window.py') == ''),
        ker_clean=(gits(KER, 'rev-parse', '--short=7', 'HEAD') == KER_TIP and gits(KER, 'status', '--porcelain', '--untracked-files=no') == ''),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b520 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b520_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (os.path.join(D, x) for x in ('b520_reach.json', 'b520_cells.jsonl'))),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b520 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[0-9]_|^b334_', f)]
...
def cells_of(S, o):
    return [c for c in S['rows'] if o in c]


def objs(S):
    return [(c, o) for c in S['rows'] for o in ('q', 'xi') if o in c]


def recount(S, o):
    C = cells_of(S, o)
    V = [c for c in C if abs(c[o]['r']) <= c[o]['B']]
    cross = [c['a'] for c in C if c[o]['ratio'] is not None and c[o]['ratio'] <= -1.0]
    return dict(cells=len(C), verified=len(V), h2_neg=[c['a'] for c in V if c[o]['h2'] < -c[o]['B']],
                h2_pos=[c['a'] for c in V if c[o]['h2'] > c[o]['B']], pair_neg=[c['a'] for c in C if c[o]['pair'] < 0],
                cross=(min(cross) if cross else None))


def reach_recount(S, o):
    """### the first grid width whose E_tail exceeds B' (or, for Q0, whose a^2 exceeds LAMQ's top) ends the reach."""
    last = None
    for s in S['reach']['scan']:
        e = s.get(o)
        if e is None:
            break
        if e['Etail'] > e['Eu'] + e['Ek'] + e['Eround'] or (o == 'q' and s['a'] ** 2 > S['reach']['lamq_top']):
            return last, s['a']
        last = s['a']
    return last, None
...
def corr_row(S):
    return next((l for l in S['corr'].split(NL) if l.startswith('| 369 |')), '')


ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R129) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b520' in S['ferry'],
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b519`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 368 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 368 |', '| 3680 |'))),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b520' in S['ferry'] and 'ACT b520' in S['face'] and not glob.glob(os.path.join(D, 'b521_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b520')),
    ...
    ('G-R129-ENTERED', 'the banked ferry AND the trail -- the reading in its words',
     lambda S: 'RULING (R129) END' in S['ferry'] and S['ot'].count('**(R129) ratified') == 1
     and 'sign from a zero on the line; dominance from width, not from design' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace('**(R129) ratified', '(R129) noted'))),
    ('G-WINDOW-IS-B', 'the tool`s own text -- b519`s cell IMPORTED at variant (B), nothing copied',
     lambda S: "import b519_window as B19" in S['tool'] and "B19.cell(a, 'B')" in S['tool'] and 'def cell(' not in S['tool'],
     lambda S: put(S, 'tool', S['tool'].replace("B19.cell(a, 'B')", "B19.cell(a, 'A')"))),
    ('G-GRID-AS-READ', 'the reach scan`s widths against READING (2)`s grid, in order, stopping only when both reaches end',
     lambda S: [s['a'] for s in S['reach']['scan']] == GRID[:len(S['reach']['scan'])] and bool(S['reach']['scan']),
     lambda S: put(S, 'reach', dict(S['reach'], scan=[dict(s, a=s['a'] + 0.5) for s in S['reach']['scan']]))),
    ('G-REACH-RECOMPUTED', 'each object`s reach and stop recomputed from the scan`s error terms',
     lambda S: all((S['reach']['reach'][o], (S['reach']['stop'][o] or {}).get('a')) == reach_recount(S, o) for o in ('q', 'xi')),
     lambda S: put(S, 'reach', dict(S['reach'], reach=dict(S['reach']['reach'], q=60.0)))),
    ('G-PAST-REACH-UNBANKED', 'the banked cells -- no object carried at or past its stop width',
     lambda S: all(all(c['a'] < S['reach']['stop'][o]['a'] for c in cells_of(S, o)) for o in ('q', 'xi') if S['reach']['stop'][o]),
     lambda S: put(S, 'rows', list(S['rows']) + [dict(a=99.0, q=dict(S['rows'][0].get('xi') or {}))])),
    ('G-NV-PRINTED', 'every banked row carries nv, NONE, with its note', lambda S: bool(S['rows']) and all('nv' in c and c['nv'] is None and 'closed-form' in c['nv_note'] for c in S['rows']),
     lambda S: put(S, 'rows', [dict(c, nv=8193) for c in S['rows']])),
    ('G-SPLIT-SUMS', 'every banked cell -- the zero side equals the pair, the other off-line pairs and the on-line part',
     lambda S: all(rel(c[o]['Z'], c[o]['on_rest'] + c[o]['pair'] + c[o]['others']) for c, o in objs(S)) and bool(objs(S)),
     lambda S: put(S, 'rows', [dict(c, xi=dict(c['xi'], others=c['xi']['others'] + 0.1 * abs(c['xi']['Z']) + 1.0)) if 'xi' in c else c for c in S['rows']])),
    ('G-RATIO-RECOMPUTED', 'every banked cell -- pair / (Z - pair), signed, recomputed',
     lambda S: all(rel(c[o]['ratio'], c[o]['pair'] / (c[o]['Z'] - c[o]['pair']), 1e-7) for c, o in objs(S)) and bool(objs(S)),
     lambda S: put(S, 'rows', [dict(c, xi=dict(c['xi'], ratio=0.1)) if 'xi' in c else c for c in S['rows']])),
    ('G-VERIFIED-RECOUNTED', 'every banked cell`s flag, and the results` counts',
     ...
     and all(S['res'][o]['verified'] == recount(S, o)['verified'] and S['res'][o]['cells'] == recount(S, o)['cells'] for o in ('q', 'xi')),
     lambda S: put(S, 'res', dict(S['res'], xi=dict(S['res']['xi'], verified=-1)))),
    ('G-BOUND-FORM', 'every banked cell -- B the sum of the four terms, the shortfall not in it',
     lambda S: all(rel(c[o]['B'], c[o]['Eu'] + c[o]['Ek'] + c[o]['Etail'] + c[o]['Eround']) for c, o in objs(S)) and bool(objs(S)),
     lambda S: put(S, 'rows', [dict(c, xi=dict(c['xi'], B=c['xi']['B'] * 1.5 + 1e-9)) if 'xi' in c else c for c in S['rows']])),
    ('G-SHORTFALL-PRINTED', 'every banked cell and every scanned width carries its ESTIMATE; the report prints it',
     lambda S: all(c[o].get('shortfall') is not None for c, o in objs(S)) and all('shortfall' in s[o] for s in S['reach']['scan'] for o in ('q', 'xi') if o in s)
     and 'shortfall' in S['report'], lambda S: put(S, 'report', S['report'].replace('shortfall', 'x'))),
    ('G-GROWTH-RECOMPUTED', 'every banked cell`s realized growth recomputed from the plateau`s closed form at i delta',
     lambda S: all(rel(c[o]['growth'], (math.sinh(DELTA * (math.log(c['a']) - 0.125 * math.log(c['a']))) / (DELTA * (math.log(c['a']) - 0.125 * math.log(c['a'])))
                                        * (math.sinh(DELTA * (0.125 * math.log(c['a']) / 5.0)) / (DELTA * (0.125 * math.log(c['a']) / 5.0))) ** 5), 1e-9)
                   and rel(c[o]['growth_bound'], math.exp(DELTA * math.log(c['a']))) for c, o in objs(S)) and bool(objs(S)),
     lambda S: put(S, 'rows', [dict(c, xi=dict(c['xi'], growth=c['xi']['growth'] * 10)) if 'xi' in c else c for c in S['rows']])),
    ('G-SIGNS-RECOUNTED', 'the results` sign lists against the banked cells, recounted',
     lambda S: all(S['res'][o][k] == recount(S, o)[k] for o in ('q', 'xi') for k in ('h2_neg', 'h2_pos', 'pair_neg')),
     lambda S: put(S, 'res', dict(S['res'], xi=dict(S['res']['xi'], pair_neg=[5.0])))),
    ('G-CROSSING-RECOMPUTED', 'the narrowest read width with ratio <= -1, recomputed, per object',
     lambda S: all(S['res'][o]['cross_minus_one'] == recount(S, o)['cross'] for o in ('q', 'xi')),
    ...
    ('G-READING-WORDS', 'the reading against the recount AND the trail -- (R127)(2)`s words',
     lambda S: ((S['res']['reading'].startswith(HARD5)) if not recount(S, 'q')['h2_neg'] else WIT5 in S['res']['reading'])
    ...
    ('G-N1-SCORED', 'the desk against the recount', lambda S: 'n1' in S['sc'] and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == (recount(S, 'q')['cross'] is not None and 20.0 <= recount(S, 'q')['cross'] <= 45.0),
    ...
    ('G-N2-SCORED', 'the desk against the recount', lambda S: 'n2' in S['sc'] and desk_word(S, 'N2') == word_of(S['sc']['n2'])
     and S['sc']['n2'] == (recount(S, 'q')['cross'] is not None
                           and next(c['q']['growth_fraction'] for c in cells_of(S, 'q') if c['a'] == recount(S, 'q')['cross']) >= 0.5),
    ...
    ('G-N3-SCORED', 'the desk against the recount', lambda S: 'n3' in S['sc'] and desk_word(S, 'N3') == word_of(S['sc']['n3'])
     and S['sc']['n3'] == (len(recount(S, 'xi')['h2_pos']) == recount(S, 'xi')['verified'])
     and (recount(S, 'xi')['verified'] > 0 or 'VACUOUS' in S['desk']),
     lambda S: put(S, 'sc', dict(S['sc'], n3=not S['sc'].get('n3')))),
    ('G-N4-SCORED', 'the desk against the reach recomputed', lambda S: 'n4' in S['sc'] and desk_word(S, 'N4') == word_of(S['sc']['n4'])
     and S['sc']['n4'] == ((reach_recount(S, 'q')[0] or 0) >= 45.0), lambda S: put(S, 'sc', dict(S['sc'], n4=not S['sc'].get('n4')))),
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from the banks',
     ...
     and desk_word(S, 'S1') == word_of(recount(S, 'q')['cross'] is not None)
     and desk_word(S, 'S3') == word_of(reach_recount(S, 'q')[0] == 60.0),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b520 -'))),
    ...
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 369 once, six cells as it landed',
     lambda S: S['corr'].count('| 369 |') == 1 and S['corr'].count('(b520, under (R129))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 369 | a duplicate row |')),
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b519`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b519_window.py'])),
    ('G-WRITELIST-KINDS', 'every b520 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b520')" in S['suite']
                and "data/b520_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b520_components.txt' in gits(ROOT, 'show'")),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b520')
              and 'data/b520_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b520 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b520_checks_postpush.txt' if pushed else 'b520_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b520_exercise.json'), 'w', encoding='utf-8', newline=NL),
