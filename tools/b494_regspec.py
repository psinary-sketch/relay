# -*- coding: utf-8 -*-
"""b494_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b494_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b494_satisfiable.json')
...
CLAUSES = [
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b494.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b494_extract.txt`.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R105) -- the reconciliation act carries a SECOND table, the frame check; the two tables are filed to FINDINGS as ONE section and no other document is created.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b494_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written at all', 2, 2, 'documents', '### (W): FINDINGS.md (ONE appended section carrying both tables, as (R105) directs) and OPEN_TRAILS.md (one appended record).'),
...
def main(argv):
    print('=' * 100)
    print('b494_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b494_extract.txt'),
    ...
    spec = {"registration": ("data/b494_registration_2026-09-23.txt -- b494, "
