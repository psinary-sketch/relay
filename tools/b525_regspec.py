# -*- coding: utf-8 -*-
"""b525_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b516's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the numerical
### ### clauses are zero because this fold runs no chain; no lane opens and no instrument is edited.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b525_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b525_satisfiable.json')
...
CLAUSES = [
    ...
    ('lanes opened', 0, 0, 'lanes', '### (A): no lane opened by this act.'),
    ...
    ('rules struck, amended, widened or re-ruled', 0, 0, 'rules', '### section (Z): (R134)(2)`s rule is filed, not minted by the seat.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b525.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R135).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b525_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b525_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b525_extract.txt'),
    ...
    spec = {"registration": ("data/b525_registration_2026-09-25.txt -- b525, "
                             "THE FOLD AT SPAN NINE"),
            "clauses": clauses}
