# -*- coding: utf-8 -*-
"""b544_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b541's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): no kernel file;
### ### one ERRATA entry filed with one bullet; one FINDINGS line inserted; three PLACE-papers documents written.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b544_registration_2026-09-26.txt')
SPEC = os.path.join(ROOT, 'data', 'b544_satisfiable.json')
...
CLAUSES = [
    ...
    ('kernel builds', 0, 0, 'builds', '### (Z): the axiom probes of READING (3) are not builds.'),
    ...
    ('branches merged or fetched', 0, 0, 'branches', '### READING (8): two branch refs deleted by name, none merged or fetched.'),
    ...
    ('ERRATA entries written', 1, 1, 'entries', '### READING (7): E-2026-09-25-6 filed as drafted.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R154).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b544_addendum.txt, under the stem glob.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('PLACE-papers documents written other than by appending', 2, 2, 'documents', '### READING (7): ERRATA`s bullet; FINDINGS`s line beneath :1109.'),
    ...
    ('existing `relay` tools edited in place', 1, 1, 'tools', '### READING (6): terminal_table.py, one column, under (R154)(4).'),
...
def main(argv):
    print('=' * 100)
    print('b544_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b544_extract.txt'),
    ...
    spec = {"registration": ("data/b544_registration_2026-09-26.txt -- b544, "
                             "CP-2 AND CP-3, UNDER (R154)"),
            "clauses": clauses}
