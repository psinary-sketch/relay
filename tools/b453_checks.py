# -*- coding: utf-8 -*-
"""b453_checks.py -- THE CONTROL SUITE FOR b453. ### **EVERY ARM DECLARED ON THE LOCKED FACE, RUN.**
...
FACE = os.path.join(D, 'b453_registration_2026-09-14.txt')
PREPUSH = os.path.join(D, 'b453_checks.txt')
POSTPUSH = os.path.join(D, 'b453_checks_postpush.txt')
NL = chr(10)
L = []
...
ACT_START = min(os.path.getmtime(os.path.join(D, f))
                for f in ('b453_ferry.txt', 'b453_ferry_scan.txt'))
...
EXTR = read(os.path.join(D, 'b453_extract.txt'))
RGATE = read(os.path.join(D, 'b453_reg_gate.txt'))
TERM = read(os.path.join(D, 'b453_reg_termscan.txt'))
SPECRUN = read(os.path.join(D, 'b453_regspec_run.txt'))
AUDIT = read(os.path.join(D, 'audit_b453_reg_satisfiable.txt'))
LOCKG = read(os.path.join(D, 'b453_lockgate_notes.txt'))
assert LOCKG, 'the lock-gate record is missing; no arm may read it as empty'
SCAN = read(os.path.join(D, 'b453_ferry_scan.txt'))
FERRY = read(os.path.join(D, 'b453_ferry.txt'))
CENS = read(os.path.join(D, 'b453_census_stepzero.txt'))
FCENS = read(os.path.join(D, 'b453_faces_census_stepzero.txt'))
PINS = read(os.path.join(D, 'b453_pins_stepzero.txt'))
ADD = os.path.join(D, 'b453_addendum.txt')
SRC_COMP = read(os.path.join(T, 'b453_components.py'))
SRC_DESK = read(os.path.join(T, 'b453_desk_bank.py'))
SRC_EXT = read(os.path.join(T, 'b453_extract.py'))
SRC_CHK = read(os.path.join(T, 'b453_checks.py'))
...
DIGREL = 'phase2/method/THE_FINDINGS_AS_THEY_STAND.md'
...
MIRROR = read(os.path.join(D, 'b453_mirror.txt'))
FOLDRUN = read(os.path.join(D, 'b453_fold_run.txt'))
SRC_ALL = {n: read(os.path.join(T, 'b453_%s.py' % n)) for n in ('extract', 'regspec', 'reg_gate', 'components', 'checks', 'desk_bank')}


def appended_by_act(repo, rel, act):
    """### append-only against the tree before this act: HEAD (pre-commit) or the parent of this act`s commit (post-commit)."""
    new = read(os.path.join(repo, rel)).replace(chr(13) + NL, NL)
    if not unchanged(repo, rel):
        old = git(repo, 'show', 'HEAD:%s' % rel).replace(chr(13) + NL, NL)
    else:
        shas = [ln.split(' ', 1)[0] for ln in git(repo, 'log', '--format=%H %s', '-10', '--', rel).splitlines() if act in ln]
        if not shas:
            return False
        old = git(repo, 'show', '%s^:%s' % (shas[0], rel)).replace(chr(13) + NL, NL)
    return bool(old) and new.startswith(old.rstrip(NL)) and len(new) > len(old.rstrip(NL))
...
def main(argv):
    ...
    rec('=' * 100)
    rec('b453 -- THE CONTROL SUITE. ### **%s READING.**' % ('POST-PUSH' if post else 'PRE-PUSH'))
    rec('=' * 100)
    relay_new = sorted(set(touched(ROOT, r'b453')) | set(committed_by_act(ROOT, r'b453')))
    ...
    seal = subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', 'data/b453_registration_2026-09-14.txt'], cwd=ROOT,
                          capture_output=True, text=True, encoding='utf-8', errors='replace').stdout or ''
    CODE = pycode_of(SRC_COMP + NL + SRC_DESK)
    prior_pushed = 0 == subprocess.run(['git', '-C', ROOT, 'merge-base', '--is-ancestor', '4cd4068', 'origin/main'], capture_output=True).returncode
    fnd = read(os.path.join(PP, 'FINDINGS.md'))
    sec = fnd[fnd.find(MARK_F):] if MARK_F in fnd else ''
    dig = read(os.path.join(PP, DIGREL))
    rows = FJ.get('rows') or []
    reverified = all(read(os.path.join(D, r['bank'])).count(g['string']) >= 1 for r in rows for g in r['strings']) and len(rows) == 8
    quoted = all(('`%s`' % g['string'].replace('`', "'").replace('|', '/')) in sec for r in rows for g in r['strings'])
    heading_ok = bool(re.search(r'(?m)^## THE RESIDUE AND RECONCILIATION ARC, b445–b452 — THE FOLD\s*$', sec))
    PRIORFILE = r"b4(?!53)\d\d\w*?_[\w.-]+\.(?:txt|json|md|py)"
    WRITE = r"write_bytes\(|\.write\(|dump_json\(|dump\(|run_clock\.write|open\([^)]*'(?:w|wb|a)'"
    CONST = r"\s*(?:OUT|BANKOUT|PREPUSH|POSTPUSH|FOLDJ|COMP|RUN|SPEC)\s*="
    ...
    def mirror_tagged():
        return (verdict_line(MIRROR, '### VERDICT:', 'CLEAN ON ALL THREE CLAUSES') and 'mirror-refresh-2026-09-14-b453.zip' in MIRROR
    ...
    ARMS = [
        ...
        ('G-PRIOR-CLOSED-PUSHED', 'row 301 on the face; 4cd4068 on origin/main', 'row 301' in F and prior_pushed),
        ('G-ADDENDUM-SLOT-DECLARED', 'slot named', 'b453_addendum.txt' in FACET),
        ('G-ADDENDUM-SLOT-CONTENT-BOUNDED', 'slot empty or a quotation', (not os.path.exists(ADD)) or not read(ADD).strip()),
        ('G-NO-EXPECTATIONS', 'none registered, none scored', fold('NONE, BY THE ORDER: A FOLD RESTATES AND MINTS NOTHING.') in F and '(N1)' not in COMP),
        ('F-QUOTE', 'every verified string quoted in the section', bool(sec) and quoted),
        ('F-NOGRADE', '17 strings re-verified in their own banks', reverified and FJ.get('verified') == FJ.get('strings') == 17),
        ('F-ADDITIVE', 'FINDINGS.md appended only', appended_by_act(PP, 'FINDINGS.md', 'b453') and fnd.count(MARK_F) == 1),
        ('F-SPAN-BY-TOOL', 'the tool reads the fold b445-b452, filed by b453', (SPAN.get('last_fold') or {}).get('lo') == 445 and (SPAN.get('last_fold') or {}).get('hi') == 452
         and SPAN.get('filed_by') == 453 and FJ.get('span_tool') == 9),
        ('F-HEADING-TOOL-READABLE', 'the heading in the form the tool reads', heading_ok and (SPAN.get('last_fold') or {}).get('title') == 'THE RESIDUE AND RECONCILIATION ARC'),
        ('F-COLUMNS', 'columns 0/3/5 and four borderlines named', FJ.get('columns') == {'MODEL': 3, 'RECORD': 5} and '**0**' in sec and 'b446, b447, b448 and b449' in sec),
        ('F-ONE-STATEMENT', 'one statement in the section equals the json`s', bool(FJ.get('one')) and FJ['one'] in sec),
        ('F-CARRIED-FAILURE', 'b452`s arm failure carried, unweakened', '`data/b452_dump.json`' in sec and 'the arm was not weakened' in sec),
        ('F-ERRORS-ENTERED', 'the navigator`s and the seat`s entries', '### Errors entered as their owners’' in sec and 'third incident this window' in sec
         and 'entered as the navigator’s and not re-derived' in sec and '**The seat’s:**' in sec),
        ('F-RULINGS-STATUS', 'four rulings with status', all(('| `(R6%d)`' % k) in sec for k in (1, 2, 3, 4)) and 'directed to b454, not executed' in sec),
        ('F-SPAN-HEADING-FIRED', 'fired twice, not repaired; span tool unmoved', 'W-ORD-SPAN-HEADING fired twice and is still unrepaired' in sec and unchanged(ROOT, 'tools/b363_span.py')
         and 'tools/b363_span.py' not in committed_by_act(ROOT, r'.')),
        ('F-DIGEST-REFRESHED', 'one digest block quoting the one statement', dig.count(MARK_D) == 1 and FJ.get('one', '')[:120] in dig),
        ('F-DIGEST-ADDITIVE', 'the digest appended only', appended_by_act(PP, DIGREL, 'b453')),
        ('G-NOGRADE-MOVED', 'no grade moved; ledgers untouched', fold('NO GRADE MOVED') in F and unchanged(PP, 'FACES_LEDGER.md') and unchanged(PP, 'REGISTRY.md')),
        ...
        ('G-NOPRIORBANK', 'no prior bank tracked file touched', 0 == len([ln for ln in git(ROOT, 'status', '--porcelain', '--untracked-files=no').splitlines() if 'data/' in ln and 'b453' not in ln])),
        ('G-FOUR-LISTS-OPEN', 'lists open', fold('NO LIST OF THE FOUR CLOSED') in F and 'The four lists are open.' in sec),
        ('G-CORPUS-SCOPE', 'PLACE-papers: FINDINGS, the digest, the trail', all(f in ('FINDINGS.md', DIGREL, 'OPEN_TRAILS.md') for f in pp_touched + pp_committed)),
        ...
        ('G-PUSH-SIDE-B439-PREDICATE', 'b439 predicate', "if 'b453' not in subj" in SRC_CHK and "'--is-ancestor', 'HEAD', 'origin/main'" in SRC_CHK),
        ...
        ('G-MIRROR-TAGGED-BUILD', 'closing build under -DateTag 2026-09-14-b453, clean, at PLACE-papers HEAD', 'DEFERRED' if not post else mirror_tagged()),
