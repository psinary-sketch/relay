# -*- coding: utf-8 -*-
"""b538_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b536's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): two NEW kernel files,
### ### no tag, no ERRATA, no README/REGISTRY; PLACE-papers written by two appends.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b538_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b538_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): RegisterDepth.lean and AxiomCheckRegisterDepth.lean, both NEW.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): lake env lean on the new file, READING (7).'),
    ('chains run, or channel values computed', 0, 0, 'runs', '### (Z): no number computed.'),
    ('lanes opened', 1, 1, 'lanes', '### (R148): the kernel lane for this act.'),
    ('disproof lanes opened', 0, 0, 'lanes', '### section (Z): (R51) stands.'),
    ('network remotes contacted', 0, 0, 'remotes', '### (Z): the pins, the ritual`s pushes and the ls-remote read-backs excepted.'),
    ...
    ('grades conferred by statement-read on this act`s compiled model theorems', 11, 0, 'grades', '### READING (8): at most the theorems of READING (4); a halted one takes none.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R148).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b538_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b538_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b538_extract.txt'),
    ...
    spec = {"registration": ("data/b538_registration_2026-09-25.txt -- b538, "
                             "W-ORD-REGISTER-DEPTH, UNDER (R148)"),
            "clauses": clauses}
