# W-K200 Stage B: moments (exact summation, dps 2100) + the blocked K=200 factorizations,
# both objects, checkpointed; run repeatedly until complete.
...
def get_moments(tag, apath):
    mpath = os.path.join(T, f"k200_moments_{tag}.txt")
...
def main():
    mp.mp.dps = DPS
    t0 = time.time()
    for tag, apath in (("zeta", "k200_zeros.txt"), ("ctrl", "k200_smooth.txt")):
        jpath = os.path.join(T, f"k200_jacobi_{tag}.txt")
        if os.path.exists(jpath):
            print(f"[{tag}] already banked"); continue
        s = get_moments(tag, apath)
        Gf = lambda i, j: s[i + j + 1]
        ck = os.path.join(T, f"k200_chol_{tag}.ckpt")
        R = bc.blocked_cholesky(Gf, K + 1, ck, block=16, max_blocks=6)
        if R is None:
        ...
        os.remove(ck)
        print(f"[{tag}] Jacobi K=200 banked ({time.time()-t0:.0f} s)")
