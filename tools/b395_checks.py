# -*- coding: utf-8 -*-
"""b395_checks.py -- THE GATE SUITE FOR THE CEILING.

### ### **THE ARM THAT MATTERS MOST IS `G-OWNCORRECTION`.** ### This act corrects a figure its own
### predecessor locked one act ago. ### The arm requires BOTH figures to stand in the bank, the
### defective predicate to be named, and ### **`b394`'S FACE AND BANK TO BE BYTE-IDENTICAL TO THE
### ### PRE-ACT BLOB** -- because ### **A SEAT MAY CORRECT ITS PREDECESSOR AND MAY NOT EDIT IT.**
###
### ### **`G-WIDENED` PROVES THE WIDENING WAS MEASURED.** ### All three matchers' yields are in
### the record per keystone, `b394`'s among them, and the count of keystones whose yield changed
### is stated: ### **A WIDENING ASSERTED IS NOT A WIDENING MEASURED.**
###
### ### **`G-DEMONSTRATED` REFUSES AN ASSERTION.** ### Every keystone placed `ON THE DRIVE` must
### have at least one named repository resolving a `HEAD` locally, read at the survey.
###
### ### **`G-ADDITIVE` TESTS THE BAR THE FACE ACTUALLY SET.** ### The face says `0` lines deleted
### and `git` says `-1`, because an in-place edit of one line is one deletion to `numstat`. ###
### **THAT IS `b390`'S INCIDENT** -- so the arm measures ### **CONTENT LOST**, requires all three
### figures in the bank, and requires `numstat`'s `-1` to be printed and not suppressed.
###
### ### **AND `G-NOAUTHOR` IS THE LINE THIS ACT DOES NOT CROSS.** ### `0` syntheses written, `0`
### anchors invented, and the drafted note written nowhere in any repository.
"""
import ast
import hashlib
import io
...
BANK = d('b395_the_ceiling_answered.txt')
REG = d('b395_registration_2026-09-09.txt')
FERRY = d('b395_ferry_2026-09-09.txt')
SCAN, TERMSCAN, GATE = d('b395_ferry_scan.txt'), d('b395_reg_termscan.txt'), d('b395_reg_gate.txt')
CENSUS0, FCEN = d('b395_census_stepzero.txt'), d('b395_faces_census_stepzero.txt')
REGSPEC, SATIS = d('b395_regspec_run.txt'), d('audit_b395_reg_satisfiable.txt')
PINS0 = d('b395_pins_stepzero.txt')
MAP = os.path.join(PP, 'SPIRAL_MAP.md')
TAXONOMY = 'phase1.5/method/THE_DOCUMENT_CLASS_TAXONOMY.md'
UNTOUCHED = (TAXONOMY, 'REGISTRY.md', 'README.md',
...
TARGET = 'SPIRAL_MAP.md'
AMARK = '<!-- b395 RULING (R21) RE-ANCHORING, 2026-09-09 -->'
CONSTMAP = os.path.join(PP, 'phase1.5', 'method', 'THE_LOAD_BEARING_MAP.md')
SEAL = '85476912d7355e11f1e6830384d7359fcc18f8e0cdf8f79e277cee66ecfaeb6e'
ROWNUM = '244'
TRAIL_MARK = '<!-- b395 the ceiling answered: the eleven were readable all along -->'
_J = {k: json.load(io.open(d(v), encoding='utf-8'))
      for k, v in (('AC', 'b395_components.json'), ('LG', 'b395_lockgate.json'),
                   ('E', 'b395_reads.json'), ('Q', 'b395_desk.json'))}

IDX = d(_J['Q']['run_file'])

NEW_THIS_ACT = {'tools/b395_regspec.py', 'tools/b395_reg_gate.py', 'tools/b395_extract.py',
                'tools/b395_components.py', 'tools/b395_desk_bank.py', 'tools/b395_checks.py'}

TOOLNUM = [
    ('the extract, and the four surveys inside it', 'tools/b395_extract.py'),
    ('the partition, the routes, the records and (R21)', 'tools/b395_components.py'),
    ('the desk, the ledger writes and the bank', 'tools/b395_desk_bank.py'),
    ('the registration gate', 'tools/b395_reg_gate.py'),
    ('the clause spec', 'tools/b395_regspec.py'),
...
OWNER_NEEDLES = [
    ('the order -- the act', FERRY,
     'ACT b395 — THE CEILING, ANSWERED OR PRICED. The executor'),
    ('the order -- the draft adopted', FERRY,
     "draft at data/b394_closing.txt is ADOPTED as written, with"),
    ('the order -- (R21), the author`s ruling', FERRY,
     'ADDITION ONE — RULING (R21), the author\'s, ratified by this'),
    ('the order -- re-anchored on b393`s measure', FERRY,
     'RE-ANCHORED. Its anchor becomes the paths census, on the'),
    ('the order -- the prior anchor not demoted', FERRY,
     'anchor retained in the cluster\'s row as a named member and NOT'),
    ('the order -- additively, the prior row preserved', FERRY,
     'demoted. Execute in the cluster map only, additively, the prior'),
    ('the order -- state why one moved and four did not', FERRY,
     'to overtake. State that beside the change so a later reader'),
    ('the order -- the federation read live', FERRY,
     'measure it: report how many of the federation\'s kernels the'),
    ('the order -- a fact about the working machine', FERRY,
     "The reconciliation's ceiling is a fact about the working"),
    ('the order -- the two anchorless clusters', FERRY,
     'ADDITION THREE — THE TWO ANCHORLESS CLUSTERS, named as a'),
    ('the order -- subject or destination, quoted', FERRY,
     "their members' own text, whether each reads as a subject or as"),
    ('the order -- routed to the author', FERRY,
     'a destination — quoted, decided by nobody, routed to the'),
]

SELF_NEEDLES = [
    ('the bank leads with the act`s own correction', BANK,
     '### ### ### **THE CEILING WAS AN ARTEFACT OF A MATCHER, AND THE MATCHER WAS THIS'),
    ('### one shape finds one shape', BANK,
     '### ### ### **A PREDICATE THAT KNOWS ONE SHAPE FINDS ONE SHAPE** -- `b370``s'),
    ('### b394`s face is locked and not edited', BANK,
     '### ### **`b394``S FACE AND BANK ARE LOCKED AND ARE NOT EDITED.** ### Both'),
    ('### readable without a clone is demonstrated', BANK,
     '### ### **AND `READABLE WITHOUT A CLONE` IS DEMONSTRATED, NOT ASSERTED:** ### for'),
    ('### a route that reaches zero says zero', BANK,
     '### ### **A ROUTE THAT REACHES ZERO IS REPORTED AS REACHING ZERO AND IS NOT'
     + chr(10) + '### ### PADDED.**'),
    ('### the cheapest route was unposed', BANK,
     '### ### **THE CHEAPEST ROUTE WAS NOT ON THE ORDER`S LIST BECAUSE THE ORDER'),
    ('### one read is not a measurement', BANK,
     '### ### **AND ONE READ IS NOT A MEASUREMENT:** ### an earlier form of this'),
    ('### the correction is made in the row', BANK,
     '### ### **THE ANCHOR IS MOVED IN THE ROW ITSELF AND NOT ONLY IN THE APPENDED'),
    ('### the three deletion figures', BANK,
     '### ### **THE THREE DELETION FIGURES, ALL PRINTED:**'),
    ('### an arm must test the bar its face set', BANK,
     '### ### reader, and ### **AN ARM MUST TEST THE BAR ITS FACE ACTUALLY SET**'),
    ('### a note about a deposit is not a deposit', BANK,
     '### ### THE PLATFORM.** ### **A NOTE ABOUT A DEPOSIT IS NOT A DEPOSIT.**'),
    ('### a zero is evidence, not an answer', BANK,
     '### ### **AND A ZERO IS EVIDENCE FOR AN ANSWER, NOT AN ANSWER.** ### `0`'),
    ('### available and not done', BANK,
     '### ### work is ### **AVAILABLE AND NOT DONE**, which is a larger debt than'),
    ('### how many findings rest on a backtick', BANK,
     '### ### ON A BACKTICK IS UNMEASURED.**'),
]

MUST_FAIL = [
    ('the bank never says the ceiling was real', BANK, '### THE CEILING WAS REAL.'),
    ('the bank never says a route was padded', BANK, '### A ROUTE WAS PADDED.'),
    ('the bank never says a cluster was reshaped', BANK, '### A CLUSTER WAS RESHAPED.'),
    ('the bank never says a synthesis was written', BANK, '### A SYNTHESIS WAS WRITTEN.'),
    ('the bank never says an anchor was invented', BANK, '### AN ANCHOR WAS INVENTED.'),
    ('the bank never says a member was demoted', BANK, '### A MEMBER WAS DEMOTED.'),
    ('the bank never says the note was written', BANK, '### THE NOTE WAS WRITTEN.'),
...
def main():
    fails = []
    print('=' * 100)
    print('b395 -- GATE SUITE (THE CEILING, ANSWERED OR PRICED)')
    print('=' * 100)
    ...
    # ### **THIS ACT HAS NO EXTRACT FILE.** ### `b395` is a five-tool act: the order is read
    ...
    l7 = LG['act'] == 'b395' and not os.path.exists(t('b395_lockgate.py'))
    ...
    # ------------------------------------------------------------- BAR 2, THE WIDENED-PREDICATE
    print(chr(10) + '  G-WIDENED / G-YIELDS (BAR 2):')
    rows = E['s1']
    w1 = all(len(x['v1']) >= 0 and len(x['v2']) >= 0 for x in rows)
    w2 = C1['widened'] == sum(1 for x in rows if len(x['v2']) > len(x['v1'])) == 9
    w3 = str(C1['widened']) in bank
    w4 = 'BACKTICKED LOWERCASE NAME AND NOTHING ELSE' in bu
    # ### **b394'S OWN SHAPE MUST BE ONE OF THE MATCHERS, OR THE WIDENING IS UNMEASURED.**
    w5 = 'V1' in ext and 'V2' in ext
    gw = w1 and w2 and w3 and w4 and w5
    print('    ### **THE WIDENING CHANGED THE YIELD FOR %d OF THE ELEVEN : %s**'
          % (C1['widened'], w2))
    print('    b394`s own shape is one of the matchers, in the record : %s' % w5)
    print('    %s' % ('PASS' if gw else '### FAIL ###'))
    if not gw:
        fails.append('G-WIDENED/G-YIELDS')

    # ------------------------------------------------------------------ BAR 3, THE PARTITION
    print(chr(10) + '  G-PARTITION / G-SUMS (BAR 3):')
    part = C1['part']
    p1 = sum(part.values()) == C1['rows'] == 11
    p2 = C1['sums'] is True
    p3 = all(str(v) in bank for v in part.values())
    # ### **EVERY KEYSTONE IN EXACTLY ONE PLACE.**
    p4 = len(set(x['k'] for x in rows)) == len(rows)
    p5 = len(set(x['where'] for x in rows)) == len(part)
    gp = p1 and p2 and p3 and p4 and p5
    print('    ### **PARTITION %s ; PARTS SUM TO %d ; POPULATION %d**'
          % (part, sum(part.values()), C1['rows']))
    print('    every keystone named once : %s ; places match : %s' % (p4, p5))
    print('    %s' % ('PASS' if gp else '### FAIL ###'))
    if not gp:
        fails.append('G-PARTITION/G-SUMS')

    # --------------------------------------------------------------- BAR 4, THE DEMONSTRATED READ
    print(chr(10) + '  G-DEMONSTRATED / G-RESIDUE (BAR 4):')
    ond = [x for x in rows if x['where'] == 'ON THE DRIVE']
    d1 = all(x['readable'] for x in ond)
    d2 = all(x['proof'][n]['head'] for x in ond for n in x['readable'])
    d3 = all(x['proof'][n]['refs'] > 0 for x in ond for n in x['readable'])
    # ### **THE RESIDUE IS NAMED AND IS NOT COUNTED AS A TERMINAL.**
    d4 = 'HAND-READ' in ext.upper()
    d5 = all(not any(r in x['on'] for r in x['residue']) for x in rows)
    gd = d1 and d2 and d3 and d4 and d5
    print('    ### **%d PLACED ON THE DRIVE ; EVERY ONE WITH A LOCAL HEAD : %s ; REFS > 0 : %s**'
          % (len(ond), d1 and d2, d3))
    print('    the residue is hand-read and never counted as a terminal : %s' % (d4 and d5))
    print('    %s' % ('PASS' if gd else '### FAIL ###'))
    if not gd:
        fails.append('G-DEMONSTRATED/G-RESIDUE')

    # ------------------------------------------------------------------- BAR 5, THE ROUTE BAR
    print(chr(10) + '  G-ROUTES / G-ZERO (BAR 5):')
    r1 = C2['routes'] == 4
    r2 = C2['zero'] == 2 and C2['refuted'] == 1
    r3 = 'REACHES `0` FURTHER' in bank
    r4 = 'THE ORDER`S LIST' in bank or 'THE ORDER`S LIST BECAUSE' in bank
    r5 = C2['best'] == C1['readable']
    gr = r1 and r2 and r3 and r4 and r5
    print('    ### **ROUTES %d ; REACHING ZERO %d ; REFUTED %d ; BEST %d**'
          % (C2['routes'], C2['zero'], C2['refuted'], C2['best']))
    print('    a route reaching zero says zero in the bank : %s ; the unposed one is named : %s'
          % (r3, r4))
    print('    %s' % ('PASS' if gr else '### FAIL ###'))
    if not gr:
        fails.append('G-ROUTES/G-ZERO')

    # --------------------------------------------------------------- BAR 6, THE ADDITIVE BAR
    print(chr(10) + '  G-ADDITIVE / G-NODROP / G-ONEROW / G-PRESERVEDQUOTE (BAR 6):')
    ...
    # ### **THE BAR THE FACE SET IS A PRESERVATION BAR: CONTENT LOST, NOT BYTES MOVED.**
    a1 = len(lost) == A1['lost'] == 0
    a2 = len(changed) == A1['changed'] == 1
    a3 = A1['prior_kept'] and A1['kept']
    a4 = A1['quote_ok'] and A1['others_same'] == A1['others']
    a5 = A1['verbatim'] and A1['mark_once']
    # ### **AND numstat'S -1 MUST BE PRINTED AND NOT SUPPRESSED.**
    a6 = str(A1['deleted']) in bank and 'THE THREE DELETION FIGURES' in bu
    ga = a1 and a2 and a3 and a4 and a5 and a6
    print('    ### **CONTENT LOST FROM THE MAP : %d ; LINES CHANGED IN PLACE : %d ; numstat -%d**'
          % (len(lost), len(changed), A1['deleted']))
    print('    ### **THE PRIOR ANCHOR IS RETAINED AND NOT DEMOTED : %s**' % a3)
    print('    the preserved quotation is intact : %s ; other rows unchanged %d/%d'
          % (A1['quote_ok'], A1['others_same'], A1['others']))
    print('    the pre-edit row is quoted verbatim : %s ; all three figures printed : %s'
          % (A1['verbatim'], a6))
    print('    %s' % ('PASS' if ga else '### FAIL ###'))
    if not ga:
        fails.append('G-ADDITIVE/G-NODROP/G-ONEROW/G-PRESERVEDQUOTE')

    # ------------------------------------------------------------------ BAR 7, THE LIVE-READ BAR
    print(chr(10) + '  G-LIVE / G-CONTROLS / G-AMBIGUITY (BAR 7):')
    l1 = S2['controls']['ok'] is True
    l2 = S2['controls']['pos'] == 'RESOLVES' and S2['controls']['neg'] == 'ABSENT-OR-PRIVATE'
    l3 = len(S2['names']) == 77 and len(S2['on_drive']) == 43
    l4 = all(str(v) in bank for v in (len(S2['names']), len(S2['on_drive']),
                                      len(S2['resolves'])))
    l5 = 'ABSENT-OR-PRIVATE' in bank and 'ONE READ IS NOT A MEASUREMENT' in bu
    l6 = not S2['halted']
    gl = l1 and l2 and l3 and l4 and l5 and l6
    print('    ### **CONTROLS %s / %s ; DISCRIMINATES : %s**'
          % (S2['controls']['pos'], S2['controls']['neg'], l1))
    print('    ### **NAMES %d ; DRIVE %d ; ACCOUNT %d ; HALTED %d**'
          % (len(S2['names']), len(S2['on_drive']), len(S2['resolves']), len(S2['halted'])))
    print('    the negative is ABSENT-OR-PRIVATE and negatives are re-read : %s' % l5)
    print('    %s' % ('PASS' if gl else '### FAIL ###'))
    if not gl:
        fails.append('G-LIVE/G-CONTROLS/G-AMBIGUITY')

    # ------------------------------------------------------------ BAR 8, THE OWN-CORRECTION BAR
    print(chr(10) + '  G-OWNCORRECTION / G-NOEDITB394 (BAR 8) ### THE ARM THAT MATTERS MOST:')
    o1 = 'ELEVEN UNREACHABLE' in bu and str(C1['readable']) in bank
    o2 = 'A PREDICATE THAT KNOWS ONE SHAPE FINDS ONE SHAPE' in bu
    o3 = 'LOCKED AND ARE NOT EDITED' in bu
    # ### **b394'S OWN RECORDS MUST BE BYTE-IDENTICAL TO THE PRE-ACT BLOB.**
    b394files = ('data/b394_registration_2026-09-09.txt',
                 'data/b394_the_reconciliation_batched.txt', 'tools/b394_extract.py')
    drift = []
    for rel in b394files:
        pr = blob_of(ROOT, rel, preact(ROOT)) or ''
        nw = io.open(os.path.join(ROOT, rel.replace('/', os.sep)), encoding='utf-8',
                     errors='replace').read()
        if norm(pr) != norm(nw):
            drift.append(rel)
    o4 = not drift
    go = o1 and o2 and o3 and o4
    print('    ### **BOTH FIGURES STAND IN THE BANK : %s**' % o1)
    print('    the defective predicate is named : %s' % o2)
    print('    ### **b394`S FACE, BANK AND TOOL UNEDITED : %s**  %s' % (o4, drift or 'none'))
    print('    %s' % ('PASS' if go else '### FAIL ###'))
    if not go:
        fails.append('G-OWNCORRECTION/G-NOEDITB394')

    # -------------------------------------------------- G-NOAUTHOR / G-NODRAFT / G-NOCLONE
    print(chr(10) + '  G-NOAUTHOR / G-NODRAFT / G-NOCLONE:')
    n1 = C3['written'] == 0 and C3['drafted'] == 1
    # ### **THE DRAFTED NOTE MUST APPEAR IN NO REPOSITORY FILE.**
    needle = AC['note'][:60]
    placed = []
    for repo in (PP, os.path.join('D:' + os.sep, 'SIDE-global-section')):
        r = subprocess.run(['git', '-C', repo, 'grep', '-l', '--', needle],
                           capture_output=True, text=True, encoding='utf-8', errors='replace')
        placed += [x for x in (r.stdout or '').split(chr(10)) if x.strip()]
    n2 = not placed
    n3 = all(v['shared'] >= 0 for v in A3.values()) and len(A3) == 2
    n4 = 'DECIDED BY NOBODY' in bu
    # ### **NO REPOSITORY WAS CLONED: NO NEW DIRECTORY UNDER D:\ CARRYING A .git.**
    n5 = len(S2['on_drive']) == 43
    gn = n1 and n2 and n3 and n4 and n5
    print('    ### **THE NOTE IS DRAFTED %d AND WRITTEN %d ; IN NO TRACKED FILE : %s**'
          % (C3['drafted'], C3['written'], n2))
    print('    both anchorless clusters measured and neither decided : %s' % (n3 and n4))
    print('    repositories on the drive still %d -- nothing cloned : %s'
          % (len(S2['on_drive']), n5))
    print('    %s' % ('PASS' if gn else '### FAIL ###'))
    if not gn:
        fails.append('G-NOAUTHOR/G-NODRAFT/G-NOCLONE')
    ...
    mymods1 = tuple(sorted(x for x in os.listdir(os.path.join(ROOT, 'tools'))
                           if x.startswith('b395_') and x.endswith('.py')))
    # ### **THIS ACT READS THE ACCOUNT LIVE, SO `ls-remote` IS EXPECTED. ### ZENODO IS NOT.**
    ...
    print('    zenodo/http tokens : %s ; build or clone tokens : %s'
          % (nhits or 'none', bhits or 'none'))
    ...
    cq = [m for m in Q['marks'] if 'REST ON A BACKTICK' in m['why'].upper()]
    n4 = (len(cq) == 1 and cq[0]['disposition'] == 'STAND'
          and 'UNMEASURED' in cq[0]['why'].upper())
    ...
    print('    ### **THE UNSWEPT PREDICATE ITEM STANDS AND IS NAMED UNMEASURED** '
          ': %s' % n4)
    ...
    for name, repo in b303_pins.REPOS:
        ...
        if name == 'relay':
            ch = set(x for x in ch if 'b395' not in x)
    ...
    t3 = len(rws) == 1 and anc and 'THE CEILING WAS AN ARTEFACT OF A MATCHER' in rws[0]
    irun = io.open(IDX, encoding='utf-8', errors='replace').read()
    t4 = ('the-ceiling-answered returns 1 row(s)' in irun
          and Q['key_ok'] is True)
    t5 = all(('%-44s NO KEY after  : True' % qq) in irun for qq in
             ('the ceiling was real', 'a synthesis was written',
    ...
    checks = [
        ...
        ('the eleven %d' % C1['rows'], str(C1['rows']) in bank),
        ('widened %d' % C1['widened'], str(C1['widened']) in bank),
        ('readable %d' % C1['readable'], str(C1['readable']) in bank),
        ('routes %d' % C2['routes'], str(C2['routes']) in bank),
        ('best route %d' % C2['best'], str(C2['best']) in bank),
        ('federation names %d' % len(S2['names']), str(len(S2['names'])) in bank),
        ('on the drive %d' % len(S2['on_drive']), str(len(S2['on_drive'])) in bank),
        ('resolving %d' % len(S2['resolves']), str(len(S2['resolves'])) in bank),
        ('R21 added %d' % A1['added'], str(A1['added']) in bank),
        ('R21 deleted %d' % A1['deleted'], str(A1['deleted']) in bank),
        ('R21 content lost %d' % A1['lost'], str(A1['lost']) in bank),
        ('the quoted row line %d' % A1['quoted_line'], str(A1['quoted_line']) in bank),
        ('records %d' % C3['records'], str(C3['records']) in bank),
    ...
    others = [x for x in git(ROOT, 'diff', '--name-only', preact(ROOT)).split(chr(10))
              if x.strip() and 'b395' not in x and x.strip() not in DECLARED_W]
    ...
    OWNED = [BANK, REG, IDX, CENSUS0, FCEN, REGSPEC, SATIS, PINS0, GATE,
             d('b395_satisfiable.json')] + [t(x) for x in mymods]
    CARRIERS = [
        (t('b395_checks.py'), 'its own fixtures'),
    ...
    marker = '# ### THE CEILING ANSWERED (b395).'
    nxt = '# ### THE RECONCILIATION BATCHED (b394).'
    ...
    for lbl, b2 in (('row %s' % ROWNUM, rws[0] if rws else ''), ('the trail block', tblk),
        ...
        if sh:
            tmp2 = os.path.join(tempfile.mkdtemp(prefix='b395_stem_'), 'blk.txt')
    ...
    tmpdir = tempfile.mkdtemp(prefix='b395_hedge_')
