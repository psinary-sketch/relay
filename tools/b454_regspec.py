# -*- coding: utf-8 -*-
"""b454_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b454_registration_2026-09-14.txt')
SPEC = os.path.join(ROOT, 'data', 'b454_satisfiable.json')
...
CLAUSES = [
    ...
    ('relations graded and recorded in the trail', 1, 0, 'relations', '### reading (2): ENUMERA`s nearest terminal against (E1).'),
    ...
    ('keystones edited or annotated', 10, 0, 'keystones', '### section (W): by path, each only by (1a) and appended blocks.'),
    ('branch lines whose state terms are edited', 8, 0, 'lines', '### reading (1a): the table on the face.'),
    ('words edited on a branch line other than the named state terms', 0, 0, 'words', '### (K) BAR 4.'),
    ('findings added without a holding pre-b450 ledger line', 0, 0, 'findings', '### (K) BAR 2.'),
    ('registry rows written or edited', 3, 0, 'rows', '### reading (1d): 1.5a-7, 1.5a-8, 1.5h-9.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### section (W): mirror-refresh-2026-09-14-b454.zip.'),
    ...
    ('work-orders repaired', 1, 1, 'work-orders', '### reading (3): W-ORD-SPAN-HEADING.'),
    ...
    ('readings of the order declared in advance on this face', 3, 3, 'readings', '### section (A): (1), (2) and (3).'),
    ...
    ('author rulings executed', 2, 2, 'rulings', '### the order: (R63) under (R64).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b454_addendum.txt.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (K) BAR 10.'),
    ('lanes opened', 0, 0, 'lanes', '### section (A): both instrument lanes parked; the order permits the one named repair.'),
    ('disproof lanes opened', 0, 0, 'lanes', '### section (Z): (R51) stands.'),
    ('new instruments built', 0, 0, 'instruments', '### section (Z).'),
    ('instrument files edited', 1, 1, 'files', '### reading (3): tools/b363_span.py.'),
    ...
    ('repairs made', 1, 1, 'repairs', '### reading (3): the work-order`s own repair.'),
    ('expectations registered', 3, 3, 'expectations', '### section (J): (N1), (N2), (N3).'),
    ('registry cells edited in place', 1, 0, 'cells', '### reading (1d): the Version cell at REGISTRY.md:141.'),
    ...
    ('existing `relay` tools edited in place', 2, 2, 'tools', '### (W): banked_index.py and b363_span.py.'),
    ('corpus documents written other than by append', 4, 0, 'documents', '### (W): three keystones by state terms, REGISTRY by one cell.'),
...
def main(argv):
    print('=' * 100)
    print('b454_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b454_registration_2026-09-14.txt -- b454, "
                             "(R63) EXECUTED UNDER (R64), AND THE SPAN-HEADING WORK-ORDER REPAIRED"),
            "clauses": clauses}
