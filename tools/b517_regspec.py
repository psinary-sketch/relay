# -*- coding: utf-8 -*-
"""b517_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b513's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the kernel
### ### clauses count the two new kernel files; the grades clause counts the two theorems.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b517_registration_2026-09-24.txt')
SPEC = os.path.join(ROOT, 'data', 'b517_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): GrowthBound.lean and AxiomCheckGrowth.lean in the kernel, both NEW.'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### (R126)(3): the kernel lane for this act alone.'),
    ...
    ('grades conferred on the two theorems, by the order', 2, 2, 'grades', '### READING (6): DERIVES, each of its model statement.'),
    ...
    ('theorems proved', 2, 2, 'theorems', '### READING (1): paperFT_growth and paperFT_growth_at.'),
    ...
    ('claims about RH, h2 or any zero beyond quoting a verified source', 0, 0, 'claims', '### section (A): a bound on a transform is not a claim about any zero.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b517.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R126).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b517_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b517_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b517_extract.txt'),
    ...
    spec = {"registration": ("data/b517_registration_2026-09-24.txt -- b517, "
