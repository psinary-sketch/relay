# -*- coding: utf-8 -*-
"""b490_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b490_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b490_satisfiable.json')
...
CLAUSES = [
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b490.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b490_extract.txt`.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R100) -- the conjecture`s wording is the seat`s repair, graded MEASURED on the ladder`s chart; the three fitted forms are RETIRED. ### Ratified and entered by the trail record.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b490_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written at all', 1, 1, 'documents', '### (W): OPEN_TRAILS.md alone -- one appended record, carrying the TESTED-AT-b334 citation. NO FINDINGS SECTION: this act folds nothing.'),
...
def main(argv):
    print('=' * 100)
    print('b490_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b490_extract.txt'),
    ...
    spec = {"registration": ("data/b490_registration_2026-09-23.txt -- b490, "
