# -*- coding: utf-8 -*-
"""b348_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b348_registration_2026-09-07.txt')
SPEC = os.path.join(ROOT, 'data', 'b348_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ("PLACE-papers files written", 1, 1, "files", "### section (H): FINDINGS.md, one APPENDED section."),
    ("TECHNE files written", 1, 1, "files", "### section (H): the minted species' module."),
    ...
    ("findings sections EDITED", 0, 0, "sections", "### section (C): the fold is PURELY ADDITIVE, checked mechanically."),
    ("keystones created or edited", 0, 0, "files", "none."),
    ("grades moved", 0, 0, "grades", "### section (C): F-NOGRADE, checked mechanically against each act's own bank."),
    ("acts re-verdicted", 0, 0, "acts", "section (A)."),
    ("bars moved", 0, 0, "bars", "section (A)."),
    ("new mathematics", 0, 0, "statements", "section (A)."),
    ("measurements made about the objects", 0, 0, "measurements", "### a fold measures nothing."),
    ("frames built", 0, 0, "frames", "section (A)."),
    ("research proposals OPENED", 0, 0, "proposals", "### section (D): the partition is NAMED; opening it is another act."),
...
def main(argv):
    print('=' * 100)
    print('b348_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": "data/b348_registration_2026-09-07.txt -- b348, THE LI CONTROL, RE-RUN", "clauses": clauses}
