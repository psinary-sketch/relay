# -*- coding: utf-8 -*-
"""b528_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b522's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the
### ### numerical clauses re-read for this act's per-width reach at p = 7; no prior bank is touched.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b528_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b528_satisfiable.json')
...
CLAUSES = [
    ...
    ('chains run, or channel values computed', 1, 1, 'runs', '### (B)(1)-(2): the chain on the kernel`s plateau, Q0 and xi, every width of a = 15..60 computed, the IN widths read.'),
    ('lean runs', 0, 0, 'runs', '### (Z).'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z).'),
    ('lanes opened', 1, 1, 'lanes', '### (R138)(4): the numerical lane for this act alone.'),
    ...
    ('work-orders filed', 0, 0, 'work-orders', '### section (A): W-ORD-SMOOTH-TAIL is filed by (R138)(2), the author`s; the seat files none.'),
    ...
    ('expectations scored on a population other than the one named', 0, 0, 'expectations', '### READINGS (3) and (8).'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b528.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R138).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b528_addendum.txt, under the stem glob.'),
    ...
    ('bars declared', 'BARS', 'BARS', 'bars', '### MEASURED: this face has no (K) section; no new fixture is run (READING (1)).'),
...
def main(argv):
    print('=' * 100)
    print('b528_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b528_extract.txt'),
    ...
    spec = {"registration": ("data/b528_registration_2026-09-25.txt -- b528, "
                             "VARIANT (B) AT p = 7, THE REACH READ PER WIDTH"),
            "clauses": clauses}
