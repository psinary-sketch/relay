# -*- coding: utf-8 -*-
"""b488_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b488_registration_2026-09-22.txt')
SPEC = os.path.join(ROOT, 'data', 'b488_satisfiable.json')
...
CLAUSES = [
    ...
    ('chains run, or channel values computed', 0, 0, 'runs', '### (Z): every number is quoted from b486`s bank; no chain is run.'),
    ('lean runs', 0, 0, 'runs', '### (Z): none.'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z): every repository this act reads is already on disk.'),
    ('lanes opened', 0, 0, 'lanes', '### (Z): nothing is compiled and no lane is opened.'),
    ...
    ('prior acts` banks edited', 0, 0, 'files', '### (W): NO prior act`s bank is written. b486`s bank is READ and quoted, and nothing else.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b488.zip, under (R69).'),
    ...
    ('readings of the order declared in advance on this face', 8, 8, 'readings', '### section (A): (1) to (8), eight.'),
    ('pre-face reads declared', 4, 4, 'reads', '### section (A): (P1), (P2), (P3), (P4).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R98) -- a fold`s home is FINDINGS, and corr_row.py gains the appender`s guard.'),
    ('author rulings recorded as entries by this act', 0, 0, 'rulings', '### (Z): (R98) is ratified in the ferry and entered by the trail record.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b488_addendum.txt, under the stem glob (R91) permits.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('expectations registered', 3, 3, 'expectations', '### sections (N) and (S): the navigator`s (N1) and (N2), and the seat`s (S1), (S2), (S3).'),
    ...
    ('PLACE-papers documents written at all', 2, 2, 'documents', '### (W): FINDINGS.md (one appended section) and OPEN_TRAILS.md (one appended record).'),
    ...
    ('bars declared', 0, 0, 'bars', '### ### **THIS FACE DECLARES NO NUMBERED BARS.** ### Its constraints are in section (Z) THE NOTHINGS, which is not a (K) block. ### THE CLAUSE WAS CARRIED FROM b487 READING 8/8 AND CONTRADICTED THIS ACT`S OWN SEALED FACE, WHICH HAS NO (K) SECTION AT ALL. ### Repaired; both runs banked.'),
    ...
    ('new `relay` act-tool files named on this face', 8, 8, 'files', '### (W): the stem glob (R91) permits, so the bound is on the STEM and not on a list of names.'),
    ...
    ('existing `relay` tools edited in place', 1, 1, 'tools', '### (W) and (R98): EXACTLY ONE -- tools/corr_row.py gains the guard the ruling directs. ### THE CLAUSE WAS CARRIED FROM b487 READING 0/0, WHERE IT WAS TRUE, AND IT CONTRADICTED THIS ACT`S OWN SEALED FACE, WHICH NAMES THE EDIT IN (W). ### The face is unchanged; the INSTRUMENT was wrong and is repaired, and both runs are banked.'),
...
def main(argv):
    print('=' * 100)
    print('b488_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b488_registration_2026-09-22.txt -- b488, "
