# -*- coding: utf-8 -*-
"""b310_kernel.py -- THE SHADOW'S BUILD, ITS PROFILE, AND THE BYTE COMPARISONS.
...
ALLP = K.ALLP
PROFILE = K.PROFILE
MODULE = os.path.join(SGS, 'Core', 'SmearCollapseShadow.lean')
MODNAME = 'SmearCollapseShadow'
NS = 'B310'
OUT = os.path.join(ROOT, 'data', 'b310_kernel_run.txt')
BOM = K.BOM
...
def main():
    ...
    K.rec = rec
    rec('=' * 100)
    rec('b310 -- THE SHADOW BUILD AND ITS PROFILE. ### EVERY NUMBER FROM THIS FILE.')
    rec('=' * 100)
    ...
    # ### ### **THE PRECONDITION IS NORMALISED, AND THIS IS b309's `(D6)` MET A SECOND TIME.**
    # ### b309 found that `core.autocrlf` rewrites the working profile to CRLF on checkout while
    # ### the committed blob stays LF, so a RAW byte comparison fails on a file `git status` calls
    # ### clean. ### **b309 REPAIRED ITS GATE AND LEFT THE SAME COMPARISON IN THE KERNEL TOOL**,
    # ### where it did not fire because that tool ran BEFORE the branch switch. ### At b310 it runs
    # ### AFTER one, and it fired.
    # ### ### **THE REPAIR IS THE SAME AND ITS NARROWING IS THE SAME: ### ONLY THE LINE-ENDING
    # ### ### FORM IS IGNORED, WHICH IS GIT'S DOING AND NOT THE ACT'S.** ### The BOM test below
    # ### still reads RAW bytes, because `normalise` keeps a byte-order mark inside the first line
    # ### rather than removing it -- so the b298 check is untouched.
    ...
    rec('  banked profile on disk   : %d prints, %d zero-axiom, %d otherwise, %d bytes'
        % (n0, z0, o0, len(banked)))
    rec('  banked profile at HEAD   : %s   ### compared NORMALISED (b309 D6, met again)'
        % ('IDENTICAL' if head == banked else '### DIFFERS FROM DISK ###'))
    rec('  raw bytes on disk / at HEAD : %d / %s   ### they differ by line endings alone'
        % (len(banked_raw), len(head_raw) if head_raw is not None else 'absent'))
    rec('  BOM on the banked file   : %s   ### read from RAW bytes, not the normalised ones'
        % banked_raw.startswith(BOM))
