# -*- coding: utf-8 -*-
"""b444_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b444_registration_2026-09-12.txt')
SPEC = os.path.join(ROOT, 'data', 'b444_satisfiable.json')
...
CLAUSES = [
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b444_addendum.txt.'),
    ...
    ('span figures printed', 3, 3, 'figures', '### (K) BAR 1: tool, tool rule, order.'),
    ('fold sections appended', 1, 1, 'sections', '### reading (1).'),
    ('verdict strings written without an exact bank match', 0, 0, 'strings', '### (K) BAR 3.'),
    ('prior-fold clauses re-folded as this arc`s', 0, 0, 'clauses', '### (K) BAR 2.'),
    ('work-orders filed', 1, 1, 'orders', '### reading (1): the span tool.'),
    ('digest blocks appended', 1, 1, 'blocks', '### (S5).'),
    ('cells computed', 0, 0, 'cells', '### reading (2): a read.'),
    ('instruments run', 0, 0, 'runs', '### reading (2).'),
    ('normalizations pooled', 0, 0, 'tables', '### (K) BAR 7.'),
    ('transforms chosen after a residual was computed', 0, 0, 'transforms', '### (K) BAR 5.'),
    ('residuals called the object`s without a decision', 0, 0, 'claims', '### (K) BAR 6.'),
    ('span tool edits', 0, 0, 'edits', '### (K) BAR 8.'),
...
def main(argv):
    print('=' * 100)
    print('b444_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b444_registration_2026-09-12.txt -- b444, "
                             "THE FOLD, AND THE CHANNELS DECORRELATED"),
            "clauses": clauses}
