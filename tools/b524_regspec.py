# -*- coding: utf-8 -*-
"""b524_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b517's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the kernel
### ### clauses count the two new kernel files; the grades clause counts (i), (ii), (iii); the Prop takes none.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b524_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b524_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): TwoPropertyWindow.lean and AxiomCheckWindow.lean in the kernel, both NEW.'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### (R133)(5): the kernel lane for this act alone.'),
    ...
    ('grades conferred on (i), (ii) and (iii), by statement-read', 3, 3, 'grades', '### READING (6): each read against the ferry`s words; the Prop takes none.'),
    ('grades moved on any other row of the corpus, or grade names minted', 0, 0, 'grades', '### section (Z).'),
    ('kernel tags made', 0, 0, 'tags', '### section (Z).'),
    ('vendored modules edited', 0, 0, 'modules', '### READING (2).'),
    ('props stated and proved', 0, 0, 'props', '### READING (3): f_pair_hypothesis is stated, not proved.'),
    ...
    ('claims about RH, h2 or any zero beyond quoting a verified source', 0, 0, 'claims', '### section (A): a window and its transform are not a claim about any zero.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b524.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R133).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b524_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b524_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b524_extract.txt'),
    ...
    spec = {"registration": ("data/b524_registration_2026-09-25.txt -- b524, "
                             "(d), THE GROWTH BOUND, COMPILED"),
            "clauses": clauses}
