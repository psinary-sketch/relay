# -*- coding: utf-8 -*-
"""b458_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b458_registration_2026-09-21.txt')
SPEC = os.path.join(ROOT, 'data', 'b458_satisfiable.json')
...
CLAUSES = [
    ...
    ('kernel builds', 0, 0, 'builds', '### section (Z): no build of any kind.'),
    ('lean runs', 0, 0, 'runs', '### section (Z).'),
    ('repositories cloned', 0, 0, 'repositories', '### section (Z): no clone made.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-2026-09-21-b458.zip, under (R69).'),
    ...
    ('work-orders repaired', 0, 0, 'work-orders', '### section (Z): (R69) closes one by ruling, which is not a repair.'),
    ('instrument files edited', 0, 0, 'files', '### section (Z): not mirror_build.ps1.'),
    ...
    ('readings of the order declared in advance on this face', 2, 2, 'readings', '### section (A): (1) and (2).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### section (A): (R69).'),
    ('author rulings recorded as entries by this act', 5, 5, 'rulings', '### reading (2): (R65)-(R69).'),
    ('rulings reconstructed where the record holds no text', 0, 0, 'rulings', '### (K) BAR 2.'),
    ('sentences composed to fill a ruling out', 0, 0, 'sentences', '### (K) BAR 2.'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b458_addendum.txt.'),
    ...
    ('prose passages of the corpus rewritten', 0, 0, 'passages', '### section (Z): append only.'),
...
def main(argv):
    print('=' * 100)
    print('b458_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b458_registration_2026-09-21.txt -- b458, "
                             "RULINGS (R65) THROUGH (R68) ENTERED AS RECORDS"),
            "clauses": clauses}
