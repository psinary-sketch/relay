# -*- coding: utf-8 -*-
"""b521_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b520's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the
### ### numerical clauses re-read for this act's tightened tail and re-run pass; two prior banks take one append each under (R130)(1).
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b521_registration_2026-09-24.txt')
SPEC = os.path.join(ROOT, 'data', 'b521_satisfiable.json')
...
CLAUSES = [
    ...
    ('chains run, or channel values computed', 1, 1, 'runs', '### (B)(0)-(2): the chain on variant (B) at order p, Q0 alone, a = 15..60 within reach.'),
    ('lean runs', 0, 0, 'runs', '### (Z).'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z).'),
    ('lanes opened', 1, 1, 'lanes', '### (R130)(4): the numerical lane for this act alone.'),
    ...
    ('candidate zeros, seeds, families or arrangements constructed', 0, 0, 'candidates', '### (Z): no zero added or moved; the synthetic fixture zero is a fixture, not a candidate; the window is the ferry`s.'),
    ...
    ('prior acts` banks edited other than by one proved append', 0, 0, 'files', '### (W) and (Z).'),
    ('prior acts` banks appended under (R130)(1)', 2, 2, 'files', '### READING (0): b518_components.txt, b519_components.txt.'),
    ...
    ('expectations scored on a population other than the one named', 0, 0, 'expectations', '### READINGS (3), (7) and (10).'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b521.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R130).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b521_addendum.txt, under the stem glob.'),
    ...
    ('bars declared', 'BARS', 'BARS', 'bars', '### MEASURED: this face has no (K) section; its fixture bar is in READING (4) with its floor.'),
...
def main(argv):
    print('=' * 100)
    print('b521_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b521_extract.txt'),
    ...
    spec = {"registration": ("data/b521_registration_2026-09-24.txt -- b521, "
                             "Q0`S TAIL TIGHTENED, THEN THE WIDTH PASS RE-RUN"),
            "clauses": clauses}
