# -*- coding: utf-8 -*-
"""b453_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b453_registration_2026-09-14.txt')
SPEC = os.path.join(ROOT, 'data', 'b453_satisfiable.json')
...
CLAUSES = [
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### section (W): mirror-refresh-2026-09-14-b453.zip.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b453_addendum.txt.'),
    ...
    ('rules, work-orders or ledgers minted', 0, 0, 'mints', '### (K) BAR 3.'),
    ('repairs made', 0, 0, 'repairs', '### (K) BAR 3: W-ORD-SPAN-HEADING fired, not repaired.'),
    ('expectations registered', 0, 0, 'expectations', '### section (J): none, by the order.'),
    ...
    ('rulings executed that the author directed to a later act', 0, 0, 'rulings', '### section (A): (R63) carried to b454.'),
    ...
    ('bars declared', 6, 6, 'bars', '### section (K).'),
    ...
    ('corpus documents written other than by append', 0, 0, 'documents', '### (K) BAR 2.'),
...
def main(argv):
    print('=' * 100)
    print('b453_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b453_registration_2026-09-14.txt -- b453, "
