# -*- coding: utf-8 -*-
"""b503_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b503_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b503_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 0, 0, 'files', '### (W): the probe `b503_printaxioms.lean` is written by the launched process into relay`s data directory, which is not a repository of Lean, and is not committed; no `.lean` file of any kernel is written.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): none; the build read is b498`s and is over.'),
    ('chains run, or channel values computed', 1, 1, 'runs', '### (B)(2): the zero side, and the consistent A, re-formed by the exact transform at all 119 cells.'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### (R113)(2): the numerical lane reopens for this act alone and shuts at its close.'),
    ...
    ('candidate zeros, seeds, families or arrangements constructed', 0, 0, 'candidates', '### (Z): no new cells; the 119 are b501`s and b502`s.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b503.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b503_extract.txt`.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R113) as amended -- the unverified rule governs, 33 cells marked by kind; the lane reopens for b503; m = Z entered as an expectation.'),
    ('author rulings recorded as entries by this act', 0, 0, 'rulings', '### (Z): (R113) is ratified in the ferry and entered by the trail record.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b503_addendum.txt, under the stem glob (R91) permits.'),
...
def main(argv):
    print('=' * 100)
    print('b503_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b503_extract.txt'),
    ...
    spec = {"registration": ("data/b503_registration_2026-09-23.txt -- b503, "
                             "THE TRANSFORM MADE ALIAS-FREE, THE TAIL RE-READ, AND m(a) ATTRIBUTED ZERO BY ZERO"),
            "clauses": clauses}
