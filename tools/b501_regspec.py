# -*- coding: utf-8 -*-
"""b501_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b501_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b501_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 0, 0, 'files', '### (W): the probe `b501_printaxioms.lean` is written by the launched process into relay`s data directory, which is not a repository of Lean, and is not committed; no `.lean` file of any kernel is written.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): none; the build read is b498`s and is over.'),
    ('chains run, or channel values computed', 1, 1, 'runs', '### (B)(2)-(3): ONE bound computation over the thirty-five cells and its two controls, in this act`s own process.'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### (R112): the numerical lane opens for b501 and b502 only.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b501.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b501_extract.txt`.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b501_addendum.txt, under the stem glob (R91) permits.'),
...
def main(argv):
    print('=' * 100)
    print('b501_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b501_extract.txt'),
    ...
    spec = {"registration": ("data/b501_registration_2026-09-23.txt -- b501, "
                             "THE PLACES-SIDE QUADRATURE BOUND, BUILT AND TESTED"),
            "clauses": clauses}
