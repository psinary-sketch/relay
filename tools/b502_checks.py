# -*- coding: utf-8 -*-
"""b502_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE.
...
FACE = os.path.join(D, 'b502_registration_2026-09-23.txt')
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b502_ferry.txt')),
        scan=read(os.path.join(D, 'b502_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b502_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b502_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b502_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b502_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b501_closing.txt')),
        addendum=read(os.path.join(D, 'b502_addendum.txt')),
        comp=read(os.path.join(D, 'b502_components.txt')),
        desk=read(os.path.join(D, 'b502_desk_notes.txt')),
        span=read(os.path.join(D, 'b502_span_notes2.txt')) + read(os.path.join(D, 'b502_span_notes.txt')),
        res=json.loads(read(os.path.join(D, 'b502_results.json')) or '{}'),
        sc=json.loads(read(os.path.join(D, 'b502_scores.json')) or '{}'),
        plan=json.loads(read(os.path.join(D, 'b502_plan.json')) or '[]'),
        cells=[json.loads(l) for l in read(os.path.join(D, 'b502_cells.jsonl')).split(NL) if l.strip()],
        old=[json.loads(l) for l in read(os.path.join(D, 'b501_cells.jsonl')).split(NL) if l.strip()],
        epc=json.loads(read(os.path.join(D, 'b502_epcheck.json')) or '{}'),
        tool=read(os.path.join(T, 'b502_ladder.py')),
        face_mtime=os.path.getmtime(FACE),
        cells_first_mtime=min([os.path.getmtime(os.path.join(D, 'b502_plan.json'))] or [0]),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b502_checks.py')),
        artefacts_tracked=gits(ROOT, 'ls-files', 'data/anthropic-zeta23'),
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b502.zip'))),
        chain_clean=(gits(ROOT, 'status', '--porcelain', '--', 'tools/b321_window.py', 'tools/e16/carto_atlas.py',
                          'tools/b318_square.py', 'tools/b317_smear.py', 'tools/b325_epstein.py',
                          'tools/b501_bound.py', 'tools/b437_components.py') == ''),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b502 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b502_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b502 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[01]_', f)]
...
def lsq(xs, ys, g):
    G = [g(x) for x in xs]
    c = sum(a * b for a, b in zip(G, ys)) / sum(a * a for a in G)
    return c, math.sqrt(sum((y - c * gg) ** 2 for y, gg in zip(ys, G)) / len(ys))


FORMS4 = {'(a) c': lambda a: 1.0, '(b) c/a': lambda a: 1.0 / a,
...
ARMS = [
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b501`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 350 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 350 |', '| 3500 |'))),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b502' in S['ferry'] and 'ACT b502' in S['face']
     and not os.path.exists(os.path.join(D, 'b503_registration_2026-09-23.txt')),
     lambda S: cut(S, 'face', 'ACT b502')),
    ...
    ('G-PRICES-ON-FACE', 'the face`s (C2) block and the measured total',
     lambda S: '(C2) THE PRICES, UNDER (R112)' in S['face'] and (res(S, 'seconds') or 0) > 0,
     lambda S: cut(S, 'face', '(C2) THE PRICES, UNDER (R112)')),
    ('G-FITS-REGISTERED-FIRST', 'file times -- the sealed face older than the first bank of the run',
     lambda S: S['face_mtime'] < S['cells_first_mtime'] and '(d) `m(a) = c / a^2`' in S['face'],
     lambda S: put(S, 'cells_first_mtime', 0.0)),
    ('G-GENERATOR-EXACT', 'the plan against prime powers counted afresh, and a = sqrt n exactly at every boundary',
     lambda S: sum(c['kind'] == 'boundary' for c in S['plan']) == pp_count()
     and all(c['a'] == math.sqrt(c['n0']) for c in S['plan'] if c['kind'] == 'boundary'),
     lambda S: put(S, 'plan', [dict(c, a=round(c['a'], 6)) for c in S['plan']])),
    ('G-ROUNDED-BESIDE', 'every new cell -- the rounded a banked beside the exact',
     lambda S: all(c.get('rounded') == round(c['a'], 6) for c in S['cells']),
     lambda S: put(S, 'cells', [dict(c, rounded=None) for c in S['cells']])),
    ('G-NEW-CELL-COUNT', 'the results bank against the cells and the plan',
     lambda S: res(S, 'n_new') == len(S['cells']) == len(S['plan']) == 84,
     lambda S: put(S, 'res', dict(S['res'], n_new=83))),
    ('G-CHAIN-EVERY-CELL', 'every new cell -- W, A, PR, the zero side and m banked, W = PR - A',
     lambda S: all(abs(c['W'] - (c['prime'] - c['arch'])) == 0 and c['m'] == -c['W'] and 'zero' in c for c in S['cells']),
     lambda S: put(S, 'cells', [dict(c, m=c['m'] + 1e-9) for c in S['cells']])),
    ('G-TERMS-SUM', 'every new cell -- the per-n terms sum to PR',
     lambda S: all(abs(sum(c['terms'].values()) - c['prime']) < 1e-12 for c in S['cells']),
     lambda S: put(S, 'cells', [dict(c, terms={}) for c in S['cells']])),
    ('G-FLOOR-EVERY-CELL', 'every new cell -- B = E_v + E_u, and the bound`s base equals the chain',
     lambda S: all(c['B'] == c['E_v'] + c['E_u'] and c['bound_base_agrees'] for c in S['cells']),
     lambda S: put(S, 'cells', [dict(c, bound_base_agrees=False) for c in S['cells']])),
    ('G-OLD-FLOORS-B501', 'the old thirty-five -- their floors are b501`s, unchanged',
     lambda S: len(S['old']) == 35 and all(isinstance(o['B'], float) for o in S['old']) and 'b501_cells.jsonl' in S['tool'],
     lambda S: cut(S, 'tool', 'b501_cells.jsonl')),
    ('G-EXTREMA-LISTED', 'the bank against the extrema recomputed from both sets of cells',
     lambda S: [(e['t'], e['a']) for e in res(S, 'extrema') or []] == [
         (('MAX' if y[1] > y[0] and y[1] > y[2] else 'MIN'), x) for x, y in
         [(v[k][0], (v[k - 1][1], v[k][1], v[k + 1][1])) for v in [sorted([(o['a'], o['m']) for o in S['old']] + [(c['a'], c['m']) for c in S['cells']])]
          for k in range(1, len(v) - 1)] if (y[1] > y[0] and y[1] > y[2]) or (y[1] < y[0] and y[1] < y[2])],
     lambda S: put(S, 'res', dict(S['res'], extrema=[]))),
    ('G-SIGN-AGAINST-FLOOR', 'every cell, old and new, against its floor',
     lambda S: res(S, 'not_above') == sum(c['m'] <= c['B'] for c in S['old'] + S['cells']),
     lambda S: put(S, 'res', dict(S['res'], not_above=5))),
    ('G-TEN-FLOORS-COUNT', 'every cell, old and new -- m within ten floors of zero',
     lambda S: res(S, 'within_ten') == sum(c['m'] < 10 * c['B'] for c in S['old'] + S['cells']),
     lambda S: put(S, 'res', dict(S['res'], within_ten=7))),
    ('G-FOUR-FITS', 'the bank`s fits recomputed on both sets',
     lambda S: all(abs(lsq([o['a'] for o in S['old']], [o['m'] for o in S['old']], g)[1] - res(S, 'fits')[k]['rms_old']) < 1e-12
                   and abs(lsq([c['a'] for c in S['cells']], [c['m'] for c in S['cells']], g)[1] - res(S, 'fits')[k]['rms_new']) < 1e-12
                   for k, g in FORMS4.items()),
     lambda S: put(S, 'res', dict(S['res'], fits={k: dict(v, rms_new=v['rms_new'] * 2) for k, v in res(S, 'fits').items()}))),
    ('G-FIT-LOSS-NAMED', 'the components bank -- the loss printed with the fits',
     lambda S: 'LOSS: RMS ABSOLUTE RESIDUAL' in S['comp'], lambda S: cut(S, 'comp', 'LOSS: RMS ABSOLUTE RESIDUAL')),
    ('G-WINNERS-PRINTED', 'the bank`s winners against its own residuals',
     lambda S: res(S, 'win_new') == min(res(S, 'fits'), key=lambda k: res(S, 'fits')[k]['rms_new'])
     and res(S, 'win_old') == min(res(S, 'fits'), key=lambda k: res(S, 'fits')[k]['rms_old']),
     lambda S: put(S, 'res', dict(S['res'], win_new='(a) c'))),
    ('G-EPSTEIN-PRICED-UNDER-HOUR', 'the face`s price and the measured Epstein seconds',
     lambda S: 'UNDER THE ONE HOUR (R112) SETS, SO IT RUNS' in S['face'] and sum(c['s_epstein'] for c in S['cells']) < 3600,
     lambda S: put(S, 'cells', [dict(c, s_epstein=100.0) for c in S['cells']])),
    ('G-EPSTEIN-ROUTE-REPRODUCES', 'the route`s positive control against b477',
     lambda S: S['epc'].get('reproduces') is True and S['epc'].get('max_diff', 1) <= 1e-12,
     lambda S: put(S, 'epc', dict(S['epc'], max_diff=1e-3, reproduces=False))),
    ('G-EPSTEIN-EVERY-CELL', 'every new cell -- the Epstein margin banked as A_Q - finite',
     lambda S: all(c['m_Q'] == c['ep_arch'] - c['ep_finite'] for c in S['cells']),
     lambda S: put(S, 'cells', [dict(c, m_Q=0.0) for c in S['cells']])),
    ('G-EPSTEIN-NEGATIVES', 'the bank against every cell`s Epstein margin',
     lambda S: res(S, 'epneg') == [c['a'] for c in S['cells'] if c['m_Q'] < 0] and 'NEGATIVE : NONE' in S['comp'],
     lambda S: put(S, 'res', dict(S['res'], epneg=[5.0]))),
    ('G-N1-SCORED', 'the desk against the cells',
     lambda S: 'n1' in (S['sc'] or {}) and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == all(c['m'] > c['B'] for c in S['cells']),
     lambda S: put(S, 'sc', dict(S['sc'], n1=not (S['sc'] or {}).get('n1')))),
    ('G-N2-SCORED', 'the desk against the winners',
     lambda S: 'n2' in (S['sc'] or {}) and desk_word(S, 'N2') == word_of(S['sc']['n2'])
     and S['sc']['n2'] == (res(S, 'win_new') != '(a) c'),
     lambda S: put(S, 'sc', dict(S['sc'], n2=not (S['sc'] or {}).get('n2')))),
    ('G-N3-SCORED', 'the desk -- and out of range said',
     lambda S: 'n3' in (S['sc'] or {}) and desk_word(S, 'N3') == word_of(S['sc']['n3']) and 'out of range' in S['desk'],
    ...
    ('G-NOGRADE-CONFERRED', 'the face', lambda S: 'THIS ACT' in S['face'] and 'CONFERS NO GRADE' in S['face'],
     lambda S: cut(S, 'face', 'CONFERS NO GRADE')),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record',
     lambda S: 'where the deposit left it' in seg(S['ot'], '### b502 —', 99999),
     lambda S: put(S, 'ot', S['ot'].replace('### b502 —', '### b502 -'))),
    ...
    ('G-FOUR-LISTS-OPEN', 'the trail`s own text -- THIS act`s record',
     lambda S: 'the four lists stay OPEN' in seg(S['ot'], '### b502 —', 99999).replace(NL, ' '),
     lambda S: put(S, 'ot', S['ot'].replace('stay OPEN', 'are closed'))),
    ('G-LANE-SHUT', 'the trail`s own record -- the lane shut at this close, said',
     lambda S: 'THE NUMERICAL LANE IS SHUT' in seg(S['ot'], '### b502 —', 99999),
     lambda S: put(S, 'ot', S['ot'].replace('THE NUMERICAL LANE IS SHUT', 'the lane stays open'))),
    ...
    ('G-TRAIL-APPEND-ONLY', 'the trail`s own text', lambda S: S['ot'].count('### b502 —') == 1,
     lambda S: put(S, 'ot', S['ot'] + NL + '### b502 — a second record')),
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger',
     lambda S: S['corr'].count('| 351 |') == 1 and S['corr'].count('(b502, under (R112)') == 1,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 351 | a duplicate row |')),
    ('G-WRITELIST-KINDS', 'every b502 commit in three repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b502')" in S['suite']
                and "data/b502_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b502_components.txt' in gits(ROOT, 'show'")),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b502')
              and 'data/b502_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b502 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b502_checks_postpush.txt' if pushed else 'b502_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b502_exercise.json'), 'w', encoding='utf-8', newline=NL),
