# -*- coding: utf-8 -*-
"""b532_kernel.py -- COMPONENTS 1-4 IN SIDE-explicit-formula. ### `python tools/b532_kernel.py install | compile | statement | read | profile`

### `install`   -- the sealed draft (scratchpad) copied into the kernel as the NEW files SIDEExplicitFormula/H2Bridge.lean and
###                AxiomCheckBridge.lean (no prior bytes); the draft`s sha256 printed and banked. Run ONCE, after the seal.
### `compile`   -- `lake env lean -o <olean> SIDEExplicitFormula/H2Bridge.lean`; EVERY RUN IS AN ATTEMPT (READING (4)),
...
### `profile`   -- `lake env lean AxiomCheckBridge.lean`: each theorem MATCHED ON THE WHOLE STRING; the `#check` and `#print`.
...
SCR = os.environ.get('B532_DRAFT', '')
MOD = os.path.join('SIDEExplicitFormula', 'H2Bridge.lean')
CHK = 'AxiomCheckBridge.lean'
OLEAN = os.path.join('.lake', 'build', 'lib', 'lean', 'SIDEExplicitFormula', 'H2Bridge.olean')
...
def install():
    ...
    out = {}
    for name, dst in (('H2Bridge.lean', MOD), (CHK, CHK)):
    ...
    io.open(os.path.join(D, 'b532_install.json'), 'w', encoding='utf-8', newline=NL).write(json.dumps(out, indent=1) + NL)
...
def compile_():
    ...
    att = os.path.join(D, 'b532_attempts.json')
    hist = json.loads(io.open(att, encoding='utf-8').read()) if os.path.exists(att) else []
    n = len(hist) + 1
    srcp = os.path.join(D, 'b532_attempt%d.lean' % n)
    shutil.copyfile(os.path.join(KER, MOD), srcp)
    r, secs = lake(['-o', OLEAN, MOD], 'b532_compile_log')
...
def statement():
    d = decls(src())
    io.open(os.path.join(D, 'b532_statement.txt'), 'w', encoding='utf-8', newline=NL).write((NL + NL).join(x['head'] for x in d) + NL)
    io.open(os.path.join(D, 'b532_definitions.txt'), 'w', encoding='utf-8', newline=NL).write((NL + NL).join(x['whole'] for x in d) + NL)
...
def read():
    ...
    io.open(os.path.join(D, 'b532_read.json'), 'w', encoding='utf-8', newline=NL).write(json.dumps(res, indent=1, ensure_ascii=False) + NL)
    for x in res['decls']:
...
def profile():
    ...
    io.open(os.path.join(D, 'b532_profile.json'), 'w', encoding='utf-8', newline=NL).write(json.dumps(res, indent=1, ensure_ascii=False) + NL)
