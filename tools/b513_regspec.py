# -*- coding: utf-8 -*-
"""b513_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b512's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the kernel
### ### clauses count the two new kernel files; the grades clause counts the chain's three links.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b513_registration_2026-09-24.txt')
SPEC = os.path.join(ROOT, 'data', 'b513_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): RHChain.lean and AxiomCheckChain.lean in the kernel, both NEW.'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### (R122)(1): the kernel lane for this act alone.'),
    ...
    ('grades conferred on the chain`s links, by the order', 3, 3, 'grades', '### READING (5): DERIVES, each of its model statement.'),
    ...
    ('theorems proved', 2, 2, 'theorems', '### READING (2): rh_imp_h2_sign and rh_imp_cell_form.'),
    ...
    ('claims about RH, h2 or any zero beyond quoting a verified source', 0, 0, 'claims', '### section (A): an implication with RH as hypothesis is not a claim that RH holds.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b513.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R122).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b513_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b513_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b513_extract.txt'),
    ...
    spec = {"registration": ("data/b513_registration_2026-09-24.txt -- b513, "
                             "RH -> h2_sign COMPILED"),
            "clauses": clauses}
