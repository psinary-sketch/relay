# -*- coding: utf-8 -*-
"""b528_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b528's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b528_registration_2026-09-25.txt')
OT = os.path.join(PP, 'OPEN_TRAILS.md')
PRIOR_RELAY = '1a83e199'      # ### b527's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
KER_TIP = '334f0ed'           # ### the kernel's tip, untouched by this act
PRIOR_PP = 'f3475be'           # ### b527's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b528 —'
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b528_ferry.txt')),
        scan=read(os.path.join(D, 'b528_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b528_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b528_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b528_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b528_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b527_closing.txt')),
        addendum=read(os.path.join(D, 'b528_addendum.txt')),
        comp=read(os.path.join(D, 'b528_components.txt')),
        desk=read(os.path.join(D, 'b528_desk_notes.txt')),
        report=read(os.path.join(D, 'b528_report.txt')),
        rows=sorted(jsonl(os.path.join(D, 'b528_cells_q.jsonl')), key=lambda c: c['a']),
        xrows=sorted(jsonl(os.path.join(D, 'b528_cells_xi.jsonl')), key=lambda c: c['a']),
        check=json.loads(read(os.path.join(D, 'b528_check.json')) or '{}'),
        fixture=json.loads(read(os.path.join(D, 'b528_fixture.json')) or '[]'),
        b27val=json.loads(read(os.path.join(D, 'b527_values.json')) or '{}'),
        preseal=read(os.path.join(D, 'b528_preseal_pricing.txt')),
        tool=read(os.path.join(T, 'b528_smooth.py')),
        res=json.loads(read(os.path.join(D, 'b528_results.json')) or '{}'),
        sc=json.loads(read(os.path.join(D, 'b528_scores.json')) or '{}'),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b528_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b528.zip'))),
        chain_clean=(gits(ROOT, 'status', '--porcelain', '--', 'tools/b321_window.py', 'tools/e16/carto_atlas.py',
                          ...
                          'tools/b504_chain.py', 'tools/b506_kernel.py', 'tools/b511_families.py', 'tools/b503_zero.py', 'tools/b514_window.py', 'tools/b515_window.py', 'tools/b518_window.py', 'tools/b519_window.py', 'tools/b520_widen.py', 'tools/b521_tail.py', 'tools/b522_reach.py', 'tools/b523_caveats.py', 'tools/b527_values.py') == ''),
        ker_clean=(gits(KER, 'rev-parse', '--short=7', 'HEAD') == KER_TIP and gits(KER, 'status', '--porcelain', '--untracked-files=no') == ''),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b528 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b528_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (os.path.join(D, x) for x in ('b528_check.json', 'b528_fixture.json', 'b528_cells_q.jsonl', 'b528_cells_xi.jsonl'))),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b528 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[0-9]_|^b52[0-7]_|^b334_', f)]
...
FIX_PAIRS = [(15.0, 'q'), (15.0, 'xi'), (34.0, 'q'), (34.0, 'xi'), (60.0, 'q'), (60.0, 'xi')]
# ### the cell arms` positive controls inject one defective cell INSIDE the reach; if no width of either object is read
# ### the arm is VACUOUS on the live source and the suite says so.
VACUOUS_ARMS = ('G-SPLIT-SUMS', 'G-RATIO-RECOMPUTED', 'G-VERIFIED-RECOUNTED', 'G-SIGNS-RECOUNTED')
...
def allc(S):
    return list(S['rows']) + list(S['xrows'])


def inn(S):
    return [c for c in allc(S) if c['within']]


def bad_cell(S, **kw):
    c = dict(a=30.0, obj='q', gamma0=16.290216, h2=1.0, B=1.0, B_full=1.0, r=0.0, Z=10.0, Z_on=1.0, on_rest=1.0, pair=1.0, others=1.0,
             rest=9.0, ratio=0.1, Eu=0.1, Ek=0.1, Eround=0.1, Eft=0.1, Bprime=0.4, tail_est=0.1, tail_full_est=0.1, within=True,
             verified=False, neg=False, neg_full=False, pos=False, tail_share=0.9, growth=99.0, growth_bound=1.0, tail_status='majorant')
    c.update(kw)
    return put(S, 'rows', list(S['rows']) + [c])


def st(x):
    """### Mathlib`s `Real.smoothTransition`, written here from its definition -- independent of every act tool."""
    if x <= 0.0:
        return 0.0
    if x >= 1.0:
        return 1.0
    e1, e2 = math.exp(-1.0 / x), math.exp(-1.0 / (1.0 - x))
    return e1 / (e1 + e2)


def phi_ind(u, a):
    L = math.log(a)
    return st((L - abs(u)) / (0.25 * L))


def growth_ind(a, n=8001):
    L = math.log(a)
    h = 2.0 * L / (n - 1)
    num = den = 0.0
    for i in range(n):
...
def growth_of(a):
    if a not in GROWTH:
        GROWTH[a] = growth_ind(a)
    return GROWTH[a]


def recount(S, obj):
    C = [c for c in (S['rows'] if obj == 'q' else S['xrows']) if c['within']]
    V = [c for c in C if abs(c['r']) <= c['B']]
    neg = [c for c in V if c['h2'] < -c['B']]
    cross = [c for c in C if c['ratio'] is not None and c['ratio'] <= -1.0]
    return dict(within=[c['a'] for c in C], verified=len(V), neg=[c['a'] for c in neg], pos=[c['a'] for c in V if c['h2'] > c['B']],
                cross=(cross[0]['a'] if cross else None), G_cross=(cross[0]['growth'] if cross else None))


def corr_row(S):
    return next((l for l in S['corr'].split(NL) if l.startswith('| 377 |')), '')


def sc_n1(S):
    X = [c for c in S['xrows'] if c['within'] and abs(c['r']) <= c['B']]
    return bool(X) and all(c['h2'] > c['B'] for c in X)


def sc_n3(S):
    R = recount(S, 'q')
    return R['cross'] is not None and abs(R['G_cross'] / G_BSPLINE - 1.0) <= 0.2


def fixture_ok(S):
    F = S['fixture']
    if sorted((f['a'], f['obj']) for f in F) != sorted(FIX_PAIRS):
        return False
    for f in F:
        g0 = 16.290216 if f['obj'] == 'q' else 14.1347
        for x in f['rows']:
            if x['bar'] > 0:
                if not (x['diff'] <= x['bar'] and x['meets'] is True):
                    return False
            elif not (x['meets'] is None and x['z'] == str(g0)):
                return False
    return True
...
ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R138) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b528' in S['ferry'],
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b527`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 376 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 376 |', '| 3760 |'))),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b528' in S['ferry'] and 'ACT b528' in S['face'] and not glob.glob(os.path.join(D, 'b529_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b528')),
    ('G-PEEK-DECLARED', 'the face`s (A) and (C) blocks, the pre-seal bank`s time, and the component banks` times against the lock',
     lambda S: 'A PRE-SEAL PRICE, DECLARED' in flat(S['face']) and 'the declared pre-seal price above' in flat(S['face'])
     and 'tail ESTIMATE (150..1500)' in S['preseal'] and S['after_lock'],
     lambda S: put(S, 'after_lock', False)),
    ('G-R138-ENTERED', 'the banked ferry AND the trail -- (R138) ratified, W-ORD-SMOOTH-TAIL named as the author filed it',
     lambda S: 'RULING (R138) END' in S['ferry'] and S['ot'].count('**(R138) ratified') == 1 and 'W-ORD-SMOOTH-TAIL' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace('**(R138) ratified', '(R138) noted'))),
    ('G-PHI-CHECKED', 'Component 0 (i) -- the forty kernel values IMPORTED from b527`s bank, and phi re-derived here from Mathlib`s definition',
     lambda S: S['check'].get('n') == 40 and S['check']['agree'] is True and S['check']['max_chain'] <= 1e-14
     and [r['kernel'] for r in S['check']['rows']] == [r['kernel'] for r in S['b27val']['rows']]
     and all(abs(phi_ind(r['u'], S['check']['a']) - r['kernel']) <= 1e-14 for r in S['check']['rows']),
     lambda S: put(S, 'check', dict(S['check'], rows=[dict(r, kernel=r['kernel'] + 1e-6) for r in S['check']['rows']]))),
    ('G-FIXTURE-BAR', 'Component 0 (ii) -- six width-object pairs, every scored z within its bar, the unscored z exactly gamma_0 (defect D1)',
     lambda S: fixture_ok(S),
     lambda S: put(S, 'fixture', [dict(f, rows=[dict(x, diff=2 * x['bar'] + 1.0) for x in f['rows']]) for f in S['fixture']])),
    ('G-GRID-COMPLETE', 'both cell banks against READING (5)`s grid, every width, in order',
     lambda S: [c['a'] for c in S['rows']] == GRID and [c['a'] for c in S['xrows']] == GRID,
     lambda S: put(S, 'xrows', S['xrows'][:-1])),
    ('G-TAIL-SAID-ESTIMATE', 'every banked cell`s tail status, and the report`s own words',
     lambda S: all(c.get('tail_status') == 'ESTIMATE' for c in allc(S)) and 'every tail an ESTIMATE' in S['report']
     and sum(1 for l in S['report'].split(NL) if l.rstrip().endswith('ESTIMATE')) == len(allc(S)),
     lambda S: bad_cell(S)),
    ('G-REACH-RECOMPUTED', 'every cell`s IN / OUT and B` from its error terms; the results` IN and OUT lists',
     lambda S: all(c['within'] == (c['tail_est'] <= c['Bprime']) and rel(c['Bprime'], c['Eu'] + c['Ek'] + c['Eround'] + c['Eft']) for c in allc(S))
     and all(S['res'][o]['within'] == recount(S, o)['within'] for o in ('q', 'xi')),
     lambda S: bad_cell(S, tail_est=9.0)),
    ('G-OUT-BANKED-ONLY', 'the report`s lines and the results` sign lists -- an OUT width is printed NOT READ and in no sign list',
     lambda S: S['report'].count('NOT READ (banked') == sum(1 for c in allc(S) if not c['within'])
     and all(set(S['res'][o][k]) <= set(S['res'][o]['within']) for o in ('q', 'xi') for k in ('neg', 'pos', 'undecided', 'neg_full')),
     lambda S: put(S, 'report', S['report'].replace('NOT READ (banked', 'read', 1) if 'NOT READ (banked' in S['report'] else S['report'] + ' NOT READ (banked')),
    ('G-SPLIT-SUMS', 'every IN cell -- the zero side equals the pair, the other off-line pairs and the on-line part',
     lambda S: all(rel(c['Z'], c['on_rest'] + c['pair'] + c['others']) for c in inn(S)), lambda S: bad_cell(S)),
    ('G-RATIO-RECOMPUTED', 'every IN cell -- pair / (Z - pair), signed, recomputed',
     lambda S: all(rel(c['ratio'], c['pair'] / (c['Z'] - c['pair']), 1e-7) for c in inn(S)), lambda S: bad_cell(S)),
    ('G-VERIFIED-RECOUNTED', 'every cell`s flag, and the results` counts over the IN widths',
     lambda S: all(c['verified'] == (abs(c['r']) <= c['B']) for c in allc(S))
     and all(S['res'][o]['verified'] == recount(S, o)['verified'] for o in ('q', 'xi')),
     lambda S: bad_cell(S, verified=True, r=5.0)),
    ('G-BOUND-FORM', 'every cell -- B = B` + the tail ESTIMATE, the survival bound likewise, the tail`s share recomputed',
     lambda S: all(rel(c['B'], c['Bprime'] + c['tail_est']) and rel(c['B_full'], c['Bprime'] + c['tail_full_est'])
                   and rel(c['tail_share'], c['tail_est'] / c['B']) for c in allc(S)),
     lambda S: bad_cell(S)),
    ('G-GROWTH-BY-INTEGRAL', 'every cell`s G re-integrated here from phi written from Mathlib`s definition, and e^{delta L}',
     lambda S: all(rel(c['growth'], growth_of(c['a']), 1e-7) and rel(c['growth_bound'], math.exp(DELTA * math.log(c['a']))) for c in allc(S)),
     lambda S: bad_cell(S)),
    ('G-SIGNS-RECOUNTED', 'the results` sign lists and crossings against the IN cells, recounted',
     lambda S: all(S['res'][o]['neg'] == recount(S, o)['neg'] and S['res'][o]['pos'] == recount(S, o)['pos']
                   and S['res'][o]['cross'] == recount(S, o)['cross'] for o in ('q', 'xi')),
     lambda S: put(S, 'res', dict(S['res'], q=dict(S['res']['q'], neg=[5.0])))),
    ('G-FORM-TAGGED', 'the form of words against the recount AND the trail -- window and tail status named',
     lambda S: 'the kernel`s plateau' in S['res']['form'] and 'ESTIMATE' in S['res']['form'] and 'W-ORD-SMOOTH-TAIL' in S['res']['form']
     and (('no Q0 cell read' in S['res']['form']) == (not recount(S, 'q')['within'])) and flat(S['res']['form']) in trail(S),
     lambda S: put(S, 'res', dict(S['res'], form='xi positive; Q0 negative'))),
    ('G-N1-SCORED', 'the desk against the xi cells recounted', lambda S: 'n1' in S['sc'] and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == sc_n1(S), lambda S: put(S, 'sc', dict(S['sc'], n1=not S['sc'].get('n1')))),
    ('G-N2-SCORED', 'the desk against the Q0 cells recounted', lambda S: 'n2' in S['sc'] and desk_word(S, 'N2') == word_of(S['sc']['n2'])
     and S['sc']['n2'] == bool(recount(S, 'q')['neg']), lambda S: put(S, 'sc', dict(S['sc'], n2=not S['sc'].get('n2')))),
    ('G-N3-SCORED', 'the desk against the Q0 cells recounted', lambda S: 'n3' in S['sc'] and desk_word(S, 'N3') == word_of(S['sc']['n3'])
     and S['sc']['n3'] == sc_n3(S), lambda S: put(S, 'sc', dict(S['sc'], n3=not S['sc'].get('n3')))),
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from the banks',
     ...
     and desk_word(S, 'S1') == word_of(not recount(S, 'q')['within'])
     and desk_word(S, 'S2') == word_of(len(S['xrows']) == 46 and recount(S, 'xi')['verified'] == 46 and len(recount(S, 'xi')['within']) == 46)
     and desk_word(S, 'S3') == word_of(fixture_ok(S) and all(x['meets'] is True for f in S['fixture'] for x in f['rows'])),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b528 -'))),
    ...
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 377 once, six cells as it landed',
     lambda S: S['corr'].count('| 377 |') == 1 and S['corr'].count('(b528, under (R138))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 377 | a duplicate row |')),
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b527`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b527_values.py'])),
    ('G-WRITELIST-KINDS', 'every b528 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b528')" in S['suite']
                and "data/b528_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b528_components.txt' in gits(ROOT, 'show'")),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b528')
              and 'data/b528_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b528 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b528_checks_postpush.txt' if pushed else 'b528_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b528_exercise.json'), 'w', encoding='utf-8', newline=NL),
