# -*- coding: utf-8 -*-
"""b456_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b456_registration_2026-09-14.txt')
SPEC = os.path.join(ROOT, 'data', 'b456_satisfiable.json')
...
CLAUSES = [
    ...
    ('keystones edited or annotated', 2, 2, 'keystones', '### (W): INVARIANCE_BARRIERS and THE_RESIDUE_OF_RH.'),
    ('state terms changed on a line the author`s ruling did not name', 0, 0, 'terms', '### (K) BAR 5.'),
    ('dispositions taken beyond the ruled (i) and (iii)', 0, 0, 'dispositions', '### (R65), (K) BAR 4.'),
    ('dispositions recommended', 0, 0, 'dispositions', '### (K) BAR 4.'),
    ('ERRATA entries written', 1, 1, 'entries', '### reading (1).'),
    ('live surfaces narrowed', 0, 0, 'surfaces', '### section (Z).'),
    ('live notes inserted under (R20)', 2, 2, 'notes', '### reading (2): REGISTRY and README, one sentence.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### section (W): mirror-refresh-2026-09-14-b456.zip.'),
    ...
    ('pre-face reads declared', 7, 7, 'reads', '### section (A): (P1) to (P7).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### section (A): (R65).'),
    ('author rulings executed', 0, 0, 'rulings', '### section (A).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b456_addendum.txt.'),
    ...
    ('corpus documents written other than by append', 3, 0, 'documents', '### (W): the note inserted in REGISTRY and README; the :145 term.'),
    ('existing lines of the corpus changed', 1, 0, 'lines', '### reading (4): THE_RESIDUE_OF_RH.md:145 only.'),
    ('statements of a run reported as its output', 0, 0, 'statements', '### (K) BAR 3.'),
...
def main(argv):
    print('=' * 100)
    print('b456_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b456_registration_2026-09-14.txt -- b456, "
                             "THE ERRATA ENTRY, THE LIVE NOTE, AND THE ROUTED ANNOTATIONS COMPLETED"),
            "clauses": clauses}
