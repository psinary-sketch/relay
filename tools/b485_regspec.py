# -*- coding: utf-8 -*-
"""b485_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b485_registration_2026-09-22.txt')
SPEC = os.path.join(ROOT, 'data', 'b485_satisfiable.json')
...
CLAUSES = [
    ...
    ('registry rows written or edited', 0, 0, 'rows', '### (W): the DAY 1 DEPOSIT HISTORY rows are ANNOTATED by appending, never rewritten; no d1-1 or roster row is edited.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b485.zip, under (R69).'),
    ...
    ('readings of the order declared in advance on this face', 7, 7, 'readings', '### section (A): (1) to (6) plus (5b).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R94) brings the two manifests into the record at the author`s printed hashes.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b485_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written other than by appending', 1, 1, 'documents', '### (W): REGISTRY.md -- the gate row at :523 gains a dated clause IN PLACE if the gate discharges, with the original line preserved verbatim; the row annotations are appends.'),
    ('PLACE-papers documents written at all', 3, 3, 'documents', '### (W): OPEN_TRAILS.md appended, ERRATA.md appended, REGISTRY.md annotated. NOT meta/ZENODO_METADATA.md, which its own banner freezes.'),
...
def main(argv):
    print('=' * 100)
    print('b485_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b485_registration_2026-09-22.txt -- b485, "
                             "THE TWO RECORDS READ AND THE CIRCULATION GATE`S PASS RUN"),
            "clauses": clauses}
