# -*- coding: utf-8 -*-
"""b489_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b489_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b489_satisfiable.json')
...
CLAUSES = [
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b489.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b489_extract.txt`.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R99) -- of the three drafts, s2 applied as drafted, s14 with its surround restored, s15 NOT applied. ### Ratified and entered by the trail record only; this act applies nothing.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b489_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written at all', 1, 1, 'documents', '### (W): OPEN_TRAILS.md alone -- one appended record, carrying the conjecture`s sentence. NO FINDINGS SECTION: this act folds nothing.'),
...
def main(argv):
    print('=' * 100)
    print('b489_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b489_extract.txt'),
    ...
    spec = {"registration": ("data/b489_registration_2026-09-23.txt -- b489, "
