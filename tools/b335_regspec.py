# -*- coding: utf-8 -*-
"""b335_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b335_registration_2026-09-06.txt')
SPEC = os.path.join(ROOT, 'data', 'b335_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ("relay files written (new)", 1, 1, "files", "### section (G): tools/FERRY_STANDING.md."),
    ("owner instrument files edited", 1, 1, "files", "### section (G): tools/ferry_scan.py, by the order's words, additions only."),
    ("PLACE-papers files written", 1, 1, "files", "### section (G): protocols/EXECUTOR_RULES.md, Rule 6 appended."),
    ("correspondence rows appended", 1, 1, "rows", "### section (G)."),
    ("ledger rows written", 0, 0, "rows", "none."),
    ("findings sections edited", 0, 0, "sections", "none."),
    ("grades conferred by this act", 0, 0, "grades", "section (A): filings only."),
    ("proofs attempted", 0, 0, "proofs", "section (A)."),
    ("new mathematics", 0, 0, "statements", "section (A)."),
    ("claims about the quantifier", 0, 0, "claims", "section (H); must-fail fixture."),
    ...
    ("struck-clause record edits", 0, 0, "edits", "### section (G): the record referenced, not restated, not edited."),
...
def main(argv):
    print('=' * 100)
    print('b335_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": "data/b335_registration_2026-09-06.txt -- b335, THE STANDING CLAUSES, FILED", "clauses": clauses}
