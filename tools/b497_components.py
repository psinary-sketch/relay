# -*- coding: utf-8 -*-
"""b497_components.py -- COMPONENTS 1 AND 2. ### **THE LOG READ WHOLE, AND THE THREE PROFILES.**
...
FAILED = re.compile(r'(?:error:|✗|failed).{0,200}', re.I)
DURATION = re.compile(r'\((\d+(?:\.\d+)?)s\)')
AXIOM = re.compile(r"^'([A-Za-z_][A-Za-z0-9_.]*)'\s+"
...
def main():
    ...
    rec('=' * 104)
    rec('b497 COMPONENTS 1 AND 2 -- ### **THE LOG READ WHOLE, AND THE THREE PROFILES.**')
    rec('=' * 104)
    rec('')
    rec('### COMPONENT 1 -- THE LOG.')
    rec('-' * 104)
    rec('    file   : `relay/data/b495_ef_build.log`')
    rec('    bytes  : ### **%d** ; lines : ### **%d**' % (len(raw), len([x for x in lines if x])))
    rec('    sha256 : %s' % sha(raw))
    rec('    ### ### **COMMITTED UNCHANGED.** ### b495 declined to commit it because a running')
    rec('    ### process owned it; this act commits the bytes it read and hashes them here so a')
    rec('    ### later reader can prove the committed file is the file this act reported on.')
    rec('')
    ...
    rec('    ### ### **THIS IS THE CELL b495 OWED.** ### b495`s (N3) asked for more than six')
    rec('    ### distinct instants within the first ten module marks, and its (S2) asked whether')
    rec('    ### the `FromPNTPlus` ten were what `lake` built earliest. ### **BOTH NAMED THE LOG,')
    rec('    ### AND b495`S OWN ORDER FORBADE READING IT** -- so both were scored NOT SCORABLE and')
    rec('    ### routed here. ### They are paid below, on the whole file.')
    ...
    rec('    `lake`s summed reported durations : ### **%.1f s** across %d reported spans'
        % (sum(durs), len(durs)))
    gap = wall - sum(durs)
    rec('    ### ### **THE TWO NUMBERS MEASURE DIFFERENT THINGS, WHICH IS WHY THE ORDER ASKS FOR')
    rec('    ### BOTH** -- and ### **THE SUMMED DURATIONS ARE THE LARGER**, by %.1f s.' % -gap)
    rec('    ### ### **THE SEAT PREDICTED THIS GAP WITH THE SIGN BACKWARDS.** ### (S3) said wall')
    rec('    ### time would GREATLY EXCEED the summed durations, reasoning that the mathlib CLONE')
    rec('    ### is network time `lake` does not account. ### The clone is real and is in the log,')
    rec('    ### but it is swamped by a larger effect the prediction ignored: ### **`lake` BUILDS')
    rec('    ### IN PARALLEL.** ### %d module spans summing %.1f s finished inside %.1f s of wall'
        % (len(durs), sum(durs), wall))
    rec('    ### clock -- about ### **%.1fx** concurrency. ### **A SUM OF PER-JOB DURATIONS IS NOT'
        % (sum(durs) / wall if wall else 0))
    rec('    ### AN ELAPSED TIME**, and the seat compared them as though it were.')
    rec('')
    ...
    for f in fails[:8]:
        rec('      %s' % f[:110])
    rec('    ### ### **A NON-ZERO EXIT IS NOT A FAILED MODULE.** ### Both exits are')
    rec('    ### `0xC000013A`, which is ### **`STATUS_CONTROL_C_EXIT`** -- what a Windows process')
    rec('    ### reports when a console control event reaches it. ### **THE BUILD WAS KILLED, NOT')
    rec('    ### BROKEN**, and the log names no module that failed to compile. ### A reader who')
    rec('    ### saw the exit code and wrote *"the build failed"* would have said something')
    rec('    ### ### **THE LOG DOES NOT SUPPORT.**')
    rec('')
    ...
    io.open(os.path.join(D, 'b497_components.txt'), 'w', encoding='utf-8',
    ...
    json.dump(dict(bytes=len(raw), sha256=sha(raw), lines=len([x for x in lines if x]),
                   ...
                   all_standard_three=allstd, component3_run=allstd),
              io.open(os.path.join(D, 'b497_results.json'), 'w', encoding='utf-8', newline=NL),
              indent=1, ensure_ascii=False)
    print('  written: b497_components.txt, b497_results.json')
    return 0
