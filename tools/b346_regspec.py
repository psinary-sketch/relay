# -*- coding: utf-8 -*-
"""b346_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b346_registration_2026-09-06.txt')
SPEC = os.path.join(ROOT, 'data', 'b346_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ("PLACE-papers files written", 1, 1, "files", "### section (H): ERRATA.md, one appended block; the CAP is a ceiling and the block is written ONLY IF the verdict is RESOLVED."),
    ("ERRATA entries opened", 0, 0, "entries", "### section (H): a block against an existing entry is not an entry."),
    ("correspondence rows appended", 1, 1, "rows", "### section (H)."),
    ("findings sections edited", 0, 0, "sections", "none."),
    ("bars moved", 0, 0, "bars", "### section (A)."),
    ("acts re-verdicted", 0, 0, "acts", "### section (A): b339, b344, b313, b264 all stand as banked."),
    ("banked values called wrong", 0, 0, "values", "section (I)."),
    ("new frames built", 0, 0, "frames", "### section (A): the frames already in the record and no others."),
    ("new domains or cells evaluated", 0, 0, "cells", "section (D)."),
    ("cells added or moved after a value", 0, 0, "cells", "### section (D): b264's converged cells, fixed here."),
    ("axes moved", 0, 0, "axes", "### section (C): the floor's other two are named, not moved."),
    ...
    ("claims that a convention is correct", 0, 0, "claims", "### section (D): b313's clause governs."),
    ...
    ("TECHNE files written", 0, 0, "files", "### the author's two additions are b347's, not this act's."),
    ("deposited texts touched", 0, 0, "files", "none."),
    ("owner instrument files edited", 0, 0, "files", "### imported and copied, never edited."),
...
def main(argv):
    print('=' * 100)
    print('b346_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": "data/b346_registration_2026-09-06.txt -- b346, THE LI CONTROL, RE-RUN", "clauses": clauses}
