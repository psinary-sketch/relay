# -*- coding: utf-8 -*-
"""b533_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b532's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): two NEW kernel
### ### files; grades by statement-read on the compiled model theorems; the trail alone in PLACE-papers.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b533_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b533_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): PowerWindow.lean and AxiomCheckPower.lean, both NEW.'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### (R143): the kernel lane for this act.'),
    ...
    ('grades conferred by statement-read on this act`s compiled model theorems', 14, 0, 'grades', '### READING (10): at most the fourteen named; a halted lemma takes none.'),
    ...
    ('props stated and proved', 0, 0, 'props', '### READINGS (6), (8): nodes_distinct_nonreal and rh_strip_imp_rh stated, not proved.'),
    ...
    ('ERRATA entries written', 0, 0, 'entries', '### (R143)(1): E-2026-09-25-1 stays drafted.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b533.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R143).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b533_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b533_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b533_extract.txt'),
    ...
    spec = {"registration": ("data/b533_registration_2026-09-25.txt -- b533, "
                             "f4, THE POWER-WINDOW ROUTE, ACT ONE OF TWO"),
            "clauses": clauses}
