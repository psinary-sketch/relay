# -*- coding: utf-8 -*-
"""b491_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b491_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b491_satisfiable.json')
...
CLAUSES = [
    ...
    ('prior acts` banks edited', 0, 0, 'files', '### (W): NONE IS EDITED. b475`s log is COMMITTED UNCHANGED as evidence under (R101) -- added to the index, not written; not one byte of it is altered.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b491.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b491_extract.txt`.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R101) -- the b475 log is READ, its process measured dead; (R82) is decided on it; the NUMERICAL LANE opens for ONE run, which is b492 and not this act.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b491_addendum.txt, under the stem glob (R91) permits.'),
...
def main(argv):
    print('=' * 100)
    print('b491_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b491_extract.txt'),
    ...
    spec = {"registration": ("data/b491_registration_2026-09-23.txt -- b491, "
