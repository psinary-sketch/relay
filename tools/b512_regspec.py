# -*- coding: utf-8 -*-
"""b512_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b509's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the kernel
### ### clauses count the one module and the print file; the notes clause counts five banks.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b512_registration_2026-09-24.txt')
SPEC = os.path.join(ROOT, 'data', 'b512_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): Registers.lean in the kernel, and the print file in relay`s data directory.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): no `lake build`; `lake env lean` on one module.'),
    ('chains run, or channel values computed', 0, 0, 'runs', '### (Z): none.'),
    ('lanes opened', 1, 1, 'lanes', '### (R121)(5): the kernel lane for this act alone.'),
    ...
    ('grades moved on any row of the corpus, or grade names minted', 0, 0, 'grades', '### section (Z).'),
    ('kernel tags made', 0, 0, 'tags', '### section (Z).'),
    ('vendored modules edited', 0, 0, 'modules', '### READING (2).'),
    ('theorems proved beyond h2_sign_imp_cell', 0, 0, 'theorems', '### READING (5).'),
    ('work-orders filed', 1, 1, 'work-orders', '### (R121)(3): W-ORD-STRICT-BOUND.'),
    ...
    ('prior acts` banks edited', 5, 5, 'files', '### (W) and (R121)(1): ONE appended note each to five components files, prefix proved.'),
    ('keystones edited or annotated', 0, 0, 'keystones', '### (W): FINDINGS.md takes one appended note; it is a ledger, not a keystone.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b512.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R121).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b512_addendum.txt, under the stem glob.'),
    ...
    ('PLACE-papers documents written at all', 2, 2, 'documents', '### (W): OPEN_TRAILS.md and FINDINGS.md, ONE append each.'),
...
def main(argv):
    print('=' * 100)
    print('b512_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b512_extract.txt'),
    ...
    spec = {"registration": ("data/b512_registration_2026-09-24.txt -- b512, "
                             "THE REGISTER-EQUIVALENCE READ"),
            "clauses": clauses}
