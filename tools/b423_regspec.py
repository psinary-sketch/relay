# -*- coding: utf-8 -*-
"""b423_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b423_registration_2026-09-11.txt')
SPEC = os.path.join(ROOT, 'data', 'b423_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ('deposit actions', 0, 0, 'actions', '### (K) BAR 8.'),
    ('bytes written at Zenodo, in any branch', 0, 0, 'bytes', '### (K) BAR 8.'),
    ('platform calls of any kind', 0, 0, 'calls', '### (K) BAR 8; section (Z).'),
    ('kernel .lean files touched', 0, 0, 'files', '### (K) BAR 1.'),
    ('kernel builds', 0, 0, 'builds', '### (K) BAR 1.'),
    ('keystone bytes changed', 0, 0, 'bytes', '### (K) BAR 5.'),
    ...
    ('prior acts` banks edited', 0, 0, 'files', '### (K) BAR 2.'),
    ...
    ('claims about h2, in either direction', 0, 0, 'claims', '### (K) BAR 9.'),
    ('aggregations stated', 0, 0, 'statements', 'M-2 IS OWED AND STAYS OWED.'),
    ('verdicts on M-2', 0, 0, 'verdicts', 'carried from b310.'),
    ('counts adopted without measurement', 0, 0, 'counts', '### section (Z).'),
    ('arms trusted because a previous act trusted them', 0, 0, 'arms', '### (K) BAR 10.'),
    ...
    ('readings of hypothesis 1 scored', 2, 2, 'readings', '### (A) reading (1).'),
    ('readings scored without their deciding sentence', 0, 0, 'readings', '### (K) BAR 3.'),
    ('prior verdicts re-verdicted', 0, 0, 'verdicts', '### (K) BAR 2.'),
    ('consequences of the ruling ruled by the seat', 0, 0, 'consequences', '### (K) BAR 4.'),
    ('ferry paraphrases deciding over their source', 0, 0, 'paraphrases', '### (K) BAR 6.'),
    ('corpus bytes changed other than by append', 0, 0, 'bytes', '### (K) BAR 7.'),
    ...
    ('arms scanning source without stripping comments AND string literals', 0, 0, 'arms', '### (K) BAR 10.'),
    ...
    ('inherited arms not re-pointed at this act', 0, 0, 'arms', '### (K) BAR 10.'),
...
def main(argv):
    print('=' * 100)
    print('b423_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b423_registration_2026-09-11.txt -- b423, SORTIE LEG 1, THE (R37) QUESTION"),
            "clauses": clauses}
