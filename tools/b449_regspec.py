# -*- coding: utf-8 -*-
"""b449_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b449_registration_2026-09-13.txt')
SPEC = os.path.join(ROOT, 'data', 'b449_satisfiable.json')
...
CLAUSES = [
    ...
    ('mirror zips removed or rewritten', 0, 0, 'zips', '### section (W): the dc5e18e zip kept.'),
    ('mirror zips written outside a repository', 1, 1, 'zips', '### section (W): mirror-refresh-2026-09-13-b449.zip.'),
    ...
    ('work-orders filed', 1, 1, 'work-orders', '### reading (1)(c): W-ORD-MIRROR-ZIP-NAME.'),
    ...
    ('readings of the order declared in advance on this face', 3, 3, 'readings', '### section (A): (0) to (2).'),
    ('pre-face reads declared', 6, 6, 'reads', '### section (A): (P1) to (P6).'),
    ('readings taken silently', 0, 0, 'readings', '### section (A).'),
    ('survey anchor misses', 0, 0, 'misses', '### section (A).'),
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### section (A): no value of the object computed before the face.'),
    ('author rulings ratified and entered', 1, 1, 'rulings', '### section (A): (R62).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b449_addendum.txt.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (K) BAR 10.'),
    ('lanes opened', 1, 1, 'lanes', '### section (A): the instrument lane under (R62), one read.'),
    ...
    ('channels evaluated other than the prime channel', 0, 0, 'channels', '### (K) BAR 2.'),
    ('cells the integrand read runs at', 1, 1, 'cells', '### reading (2).'),
    ('levels the integrand read runs at', 4, 4, 'levels', '### reading (2).'),
    ('loom blocks appended', 1, 1, 'blocks', '### reading (1)(b).'),
    ('records of PLACE-papers edited rather than appended, the (R61) record among them', 0, 0, 'records', '### (K) BAR 4.'),
    ...
    ('errors entered as their owners`', 2, 2, 'errors', '### section (A): (E1), (E2).'),
    ...
    ('bars declared', 11, 11, 'bars', '### section (K).'),
...
def main(argv):
    print('=' * 100)
    print('b449_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b449_registration_2026-09-13.txt -- b449, "
                             "THE (R61) RECORD RATIFIED UNDER A FACE, AND THE OUTLIER'S INTEGRAND AT ITS AIM"),
            "clauses": clauses}
