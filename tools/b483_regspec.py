# -*- coding: utf-8 -*-
"""b483_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b483_registration_2026-09-22.txt')
SPEC = os.path.join(ROOT, 'data', 'b483_satisfiable.json')
...
CLAUSES = [
    ...
    ('Gram matrices formed from banked entries', 70, 70, 'matrices', '### section (B)(1): one nested Gram per size per family per object, 13+22 sizes times two objects.'),
    ('eigenvalue decompositions computed', 70, 70, 'decompositions', '### section (B)(1): one per Gram formed.'),
    ('Gram entries interpolated, extrapolated or invented', 0, 0, 'entries', '### (K) BAR 4: an entry the bank does not hold is ABSENT.'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### section (S): the numerical lane opens for this read and CLOSES AT THE ACT`S END.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b483.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 3, 3, 'rulings', '### the ferry: (R90) strikes the gate`s trigger, (R91) amends (R85)`s granularity, (R92) routes a repair to the NEXT act.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b483_addendum.txt, under the stem glob (R91) permits.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('expectations registered', 6, 6, 'expectations', '### section (J): (F1), (F2), (F3), (N1), (N2), (N3) -- b476`s six, unedited, read in (R87)`s orientation.'),
    ...
    ('prose passages of the corpus rewritten', 0, 0, 'passages', '### section (Z) and (K) BAR 2: append only, prefix proved.'),
    ('species summed', 0, 0, 'sums', '### section (Z).'),
    ('loom blocks appended', 0, 0, 'blocks', '### section (Z).'),
    ('PLACE-papers documents written other than by appending', 0, 0, 'documents', '### (W).'),
...
def main(argv):
    print('=' * 100)
    print('b483_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b483_registration_2026-09-22.txt -- b483, "
                             "THE b477 LOG READ AND THE SIX SCORED"),
            "clauses": clauses}
