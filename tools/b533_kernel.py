# -*- coding: utf-8 -*-
"""b533_kernel.py -- COMPONENTS 1-4 IN SIDE-explicit-formula. ### `python tools/b533_kernel.py install | compile | statement | read | profile`

### `install`   -- the sealed draft (scratchpad) copied into the kernel as the NEW files SIDEExplicitFormula/PowerWindow.lean and
###                AxiomCheckPower.lean (no prior bytes); the draft`s sha256 printed and banked. Run ONCE, after the seal.
### `compile`   -- `lake env lean -o <olean> SIDEExplicitFormula/PowerWindow.lean`; EVERY RUN IS AN ATTEMPT (READING (4)),
...
### `profile`   -- `lake env lean AxiomCheckPower.lean`: each theorem MATCHED ON THE WHOLE STRING; the `#check` and `#print`.
...
SCR = os.environ.get('B533_DRAFT', '')
MOD = os.path.join('SIDEExplicitFormula', 'PowerWindow.lean')
CHK = 'AxiomCheckPower.lean'
OLEAN = os.path.join('.lake', 'build', 'lib', 'lean', 'SIDEExplicitFormula', 'PowerWindow.olean')
...
def install():
    ...
    out = {}
    for name, dst in (('PowerWindow.lean', MOD), (CHK, CHK)):
    ...
    io.open(os.path.join(D, 'b533_install.json'), 'w', encoding='utf-8', newline=NL).write(json.dumps(out, indent=1) + NL)
...
def compile_():
    ...
    att = os.path.join(D, 'b533_attempts.json')
    hist = json.loads(io.open(att, encoding='utf-8').read()) if os.path.exists(att) else []
    n = len(hist) + 1
    srcp = os.path.join(D, 'b533_attempt%d.lean' % n)
    shutil.copyfile(os.path.join(KER, MOD), srcp)
    r, secs = lake(['-o', OLEAN, MOD], 'b533_compile_log')
    errs = [l for l in r.stdout.split(NL) if ': error' in l]
    srctext = io.open(srcp, encoding='utf-8').read().split(NL)
    starts = [(i + 1, re.match(r'^(?:theorem|def) (\S+)', l).group(1)) for i, l in enumerate(srctext) if re.match(r'^(?:theorem|def) \S+', l)]
    failed = set()
    for e in errs:
        m = re.search(r'PowerWindow\.lean:(\d+):', e)
        if m:
            ln = int(m.group(1))
    ...
    hist.append(dict(attempt=n, exit=r.returncode, seconds=round(secs, 1), errors=len(errs), first_errors=errs[:8],
                     sorry=('sorry' in r.stdout), source_sha=sha(srcp), failed_decls=sorted(failed)))
...
def statement():
    d = decls(src())
    io.open(os.path.join(D, 'b533_statement.txt'), 'w', encoding='utf-8', newline=NL).write((NL + NL).join(x['head'] for x in d) + NL)
    io.open(os.path.join(D, 'b533_definitions.txt'), 'w', encoding='utf-8', newline=NL).write((NL + NL).join(x['whole'] for x in d) + NL)
...
def read():
    ...
    io.open(os.path.join(D, 'b533_read.json'), 'w', encoding='utf-8', newline=NL).write(json.dumps(res, indent=1, ensure_ascii=False) + NL)
    for x in res['decls']:
