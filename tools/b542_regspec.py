# -*- coding: utf-8 -*-
"""b542_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b541's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): no kernel file;
### ### one ERRATA entry filed with one bullet inserted; three PLACE-papers documents written.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b542_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b542_satisfiable.json')
...
CLAUSES = [
    ...
    ('kernel builds', 0, 0, 'builds', '### (Z): the axiom probe of READING (10) is not a build.'),
    ...
    ('network remotes contacted', 0, 0, 'remotes', '### READING (1): no fetch; the pins, the population`s ls-remote reads, the ritual`s pushes and read-backs excepted.'),
    ('scratch trees left', 0, 0, 'trees', '### (Z).'),
    ('branches merged or fetched', 0, 0, 'branches', '### READING (11): two branch refs are deleted by name, none merged or fetched.'),
    ...
    ('ERRATA entries written', 1, 1, 'entries', '### READING (9): E-2026-09-25-5 filed as drafted.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R152).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b542_addendum.txt, under the stem glob.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('PLACE-papers documents written other than by appending', 1, 1, 'documents', '### READING (9): ERRATA, one bullet inserted.'),
    ('PLACE-papers documents written at all', 3, 3, 'documents', '### (W): ERRATA, FINDINGS, OPEN_TRAILS.'),
...
def main(argv):
    print('=' * 100)
    print('b542_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b542_extract.txt'),
    ...
    spec = {"registration": ("data/b542_registration_2026-09-25.txt -- b542, "
                             "E-5 FILED; THE INHERITANCE CENSUS, UNDER (R152)"),
            "clauses": clauses}
