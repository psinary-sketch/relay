# -*- coding: utf-8 -*-
"""b537_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b536's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): no kernel file,
### ### no tag, no grade; one zip outside a repository; one memory entry; PLACE-papers written by one append.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b537_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b537_satisfiable.json')
...
CLAUSES = [
    ...
    ('scratch trees left', 1, 1, 'trees', '### (C): b534`s zip unpacked at /tmp/tmp.g5JUpjTi0f, left for the OS; the builder`s stage dir is (W)`s.'),
    ...
    ('work-orders filed', 2, 2, 'work-orders', '### READING (6): W-ORD-SEAM-UPSTREAM and W-ORD-REGISTER-DEPTH, named not started.'),
    ...
    ('mirror zips removed or rewritten', 0, 0, 'zips', '### (Z): no PRIOR zip; this act`s own zip takes its head block by READING (1).'),
    ('mirror zips written outside a repository', 1, 1, 'zips', '### READING (1): mirror-refresh-2026-09-25-b537.zip.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R147).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b537_addendum.txt, under the stem glob.'),
    ...
    ('PLACE-papers documents written at all', 1, 1, 'documents', '### (W): OPEN_TRAILS.'),
...
def main(argv):
    print('=' * 100)
    print('b537_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b537_extract.txt'),
    ...
    spec = {"registration": ("data/b537_registration_2026-09-25.txt -- b537, "
                             "THE MIRROR EXPORT AND THE MEMORY REFRESH, UNDER (R147)"),
            "clauses": clauses}
