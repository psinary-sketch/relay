# -*- coding: utf-8 -*-
"""b499_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b499_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b499_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ('deposit actions', 0, 0, 'actions', '### (Z): nothing deposits. An edit of a published record`s title and description under (R110) is not a deposit action -- no version, no file.'),
    ('bytes written at Zenodo, in any branch', 6, 6, 'bytes', '### (R110) and (B)(2): TWO fields -- title and description -- on each of THREE records. Counted in FIELDS, not bytes: the cap is the fields the ruling permits. ### RAISED FROM 0 BY (R110).'),
    ('platform calls of any kind', 16, 16, 'calls', '### (B)(0)-(2): ONE token GET at (0); THREE deposition GETs at (1); per record edit + PUT + publish = NINE writes; THREE anonymous record GETs, each up to SIX tries, counted as ONE fetch-back each. A discard on a stop is covered by the same cap. ### RAISED FROM 0 BY (R110).'),
    ('.lean files written or edited in any repository', 0, 0, 'files', '### (W): none. SIDE-explicit-formula is written in its guard file only.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): none.'),
    ('chains run, or channel values computed', 0, 0, 'runs', '### (Z): none. Every value is read from the banked listing.'),
    ('lean runs', 0, 0, 'runs', '### (Z): none.'),
    ('repositories cloned', 1, 1, 'repositories', '### (B)(G): ONE scratch clone of relay, from the local path, in the session scratchpad, to exercise the guard; removed after.'),
    ('lanes opened', 0, 0, 'lanes', '### (Z): none; the kernel lane closed with b498.'),
    ('disproof lanes opened', 0, 0, 'lanes', '### section (Z): (R51) stands.'),
    ('network remotes contacted', 1, 1, 'remotes', '### (B): zenodo.org, and no other; the pins` ls-remote reads and the ritual`s pushes excepted.'),
    ('scratch trees left', 0, 0, 'trees', '### (B)(G): the scratch clone is removed after the exercise.'),
    ...
    ('addresses resolved or documents fetched', 7, 7, 'reads', '### (B)(0)-(2): the ONE token GET, the THREE deposition GETs and the THREE anonymous fetch-backs.'),
    ...
    ('prior acts` banks edited', 0, 0, 'files', '### (W): NONE. The listing is WRITTEN ONCE under a name of its own, not under a prior act`s stem, and is never afterwards altered.'),
    ...
    ('state terms changed on any line', 0, 0, 'terms', '### section (Z): every corpus write is an APPEND or an INSERTED NOTE with the prior text preserved verbatim; no state word is overwritten.'),
    ...
    ('ERRATA entries written', 1, 1, 'entries', '### (B)(3): ONE entry, E-2026-09-23-1, through errata_append.py, ON THREE MATCHES ONLY.'),
    ('live surfaces narrowed', 0, 0, 'surfaces', '### section (Z).'),
    ('live notes inserted', 0, 0, 'notes', '### section (Z): README is not written.'),
    ('registry rows written or edited', 0, 0, 'rows', '### (B)(3): NO row is written or edited; FIVE NOTE LINES are INSERTED beside named lines, each prior line preserved.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b499.zip, under (R69).'),
    ...
    ('counts or values the seat predicts on this face', 0, 0, 'predictions', '### section (G1): this face predicts no artifact count.'),
    ('statements imported', 0, 0, 'statements', '### (Z): nothing is imported; b495`s launcher and log are read as text.'),
    ...
    ('instrument files edited', 1, 1, 'files', '### (W) and (B)(G): the tracked guard .githooks/pre-push, ONE limb added, byte-identical in four repositories, on the author`s choice recorded in (A).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b499_extract.txt`.'),
    ('readings taken silently', 0, 0, 'readings', '### section (A).'),
    ('survey anchor misses', 0, 0, 'misses', '### section (A): 0.'),
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### section (C): SEVEN reads made before the seal are declared there, (i) to (vii); NO live Zenodo read was made before the seal.'),
    ('author rulings ratified and entered', 2, 2, 'rulings', '### the ferry: (R110) -- the seat may write title and description of the three (R109) records by the legacy edit / PUT / publish sequence, the token in the environment only, every write fetched back and compared, a mismatch stopping the act; (R111) -- texts [5] and [6] amended in the v2 file on the record`s statement-reads, the ERRATA entry recording both. ### RE-POINTED FROM b498`s (R109).'),
    ('author rulings recorded as entries by this act', 0, 0, 'rulings', '### (Z): (R110) and (R111) are ratified in the ferry and entered by the trail record.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b499_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written other than by appending', 2, 2, 'documents', '### (B)(3): REGISTRY.md and meta/ZENODO_METADATA.md take notes INSERTED beside their named lines, not appended at the end, because the order places each note beside its own line. ### No prior line is edited or removed, PROVED per file.'),
    ('PLACE-papers documents written at all', 4, 4, 'documents', '### (W): OPEN_TRAILS.md (one appended record), ERRATA.md (one appended entry), REGISTRY.md and meta/ZENODO_METADATA.md (inserted notes) -- the last three ON THREE MATCHES ONLY.'),
    ...
    ('lines removed from any document', 0, 0, 'lines', '### section (B)(3): no line of any written file before the write is absent from it after, PROVED per file.'),
    ...
    ('new `relay` act-tool files named on this face', 10, 10, 'files', '### (W): the stem glob (R91) permits, so the bound is on the STEM. ### RAISED FROM b498`s 9: guard, zenodo and record tools.'),
    ...
    ('existing `relay` tools edited in place', 0, 0, 'tools', '### (W): NONE of relay`s tools. The GUARD is not a tool under tools/ -- it is the tracked .githooks/pre-push, declared separately below.'),
...
def main(argv):
    print('=' * 100)
    print('b499_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b499_extract.txt'),
    ...
    spec = {"registration": ("data/b499_registration_2026-09-23.txt -- b499, "
                             "THE TEN METADATA EDITS APPLIED BY API AND FETCHED BACK"),
            "clauses": clauses}
