# -*- coding: utf-8 -*-
"""b526_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b525's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b526_registration_2026-09-25.txt')
...
PRIOR_RELAY = '4136b9ec'      # ### b525's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
KER_TIP = 'ffc09da'           # ### the kernel's tip, untouched by this act
PRIOR_PP = '4230299'           # ### b525's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b526 —'
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b526_ferry.txt')),
        scan=read(os.path.join(D, 'b526_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b526_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b526_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b526_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b526_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b525_closing.txt')),
        addendum=read(os.path.join(D, 'b526_addendum.txt')),
        comp=read(os.path.join(D, 'b526_components.txt')),
        desk=read(os.path.join(D, 'b526_desk_notes.txt')),
        report=read(os.path.join(D, 'b526_report.txt')),
        rows=sorted(jsonl(os.path.join(D, 'b526_xi7_cells.jsonl')), key=lambda c: c['a']),
        bump=json.loads(read(os.path.join(D, 'b526_bump.json')) or '{}'),
        notes=json.loads(read(os.path.join(D, 'b526_notes.json')) or '{}'),
        tool=read(os.path.join(T, 'b526_xi7.py')),
        ...
        ker_changed=set(x for x in gits(KER, 'diff', '--name-only', 'e21277b').split(NL) if x.strip())
                    | set(p[3:].strip() for p in git(KER, 'status', '--porcelain', '--untracked-files=no').split(NL) if p.strip()),
        res=json.loads(read(os.path.join(D, 'b526_results.json')) or '{}'),
        sc=json.loads(read(os.path.join(D, 'b526_scores.json')) or '{}'),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b526_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b526.zip'))),
        ...
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b526 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b526_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (os.path.join(D, x) for x in ('b526_notes.json', 'b526_bump.json', 'b526_xi7_cells.jsonl'))),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b526 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[0-9]_|^b52[0-5]_|^b334_', f)]
...
def nonunique_now():
    """### the control recomputed HERE, independently of the tool: base A and base B on a grid at R = 4/3."""
    import numpy as np
    R = 4.0 / 3.0
    ...
    def st(x):
        ...
        return a / (a + b)
    x = np.linspace(-1.3, 1.3, 2601)
    A, B = st((R - np.abs(x)) / (R - 1)), st(st((R - np.abs(x)) / (R - 1)))
    return bool(np.all(A[np.abs(x) <= 1] == 1) and np.all(B[np.abs(x) <= 1] == 1) and np.max(np.abs(A - B)) > 0.01)


def corr_row(S):
    return next((l for l in S['corr'].split(NL) if l.startswith('| 375 |')), '')


ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R136) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b526' in S['ferry'],
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b525`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 374 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 374 |', '| 3740 |'))),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b526' in S['ferry'] and 'ACT b526' in S['face'] and not glob.glob(os.path.join(D, 'b527_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b526')),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b526 -'))),
    ...
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 375 once, six cells as it landed',
     lambda S: S['corr'].count('| 375 |') == 1 and S['corr'].count('(b526, under (R136))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 375 | a duplicate row |')),
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b525`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b519_window.py'])),
    ('G-WRITELIST-KINDS', 'every b526 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b526')" in S['suite']
                and "data/b526_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b526_components.txt' in gits(ROOT, 'show'")),
    ...
    ('G-PEEK-DECLARED', 'the face`s (C) block AND the component banks` times against the lock',
     lambda S: 'NO CELL WAS COMPUTED AND NO TOOL OF THIS ACT RUN BEFORE THE SEAL' in flat(S['face']) and S['after_lock'],
     lambda S: put(S, 'after_lock', False)),
    ('G-R136-ENTERED', 'the banked ferry AND the trail', lambda S: 'RULING (R136) END' in S['ferry'] and S['ot'].count('**(R136) ratified') == 1,
     lambda S: put(S, 'ot', S['ot'].replace('**(R136) ratified', '(R136) noted'))),
    ('G-FOLD-NOTE-APPENDED', 'FINDINGS and the digest against their blobs at b525`s close -- prefixes, one note each, the ferry`s sentence',
     lambda S: S['ft_prefix'] and S['dg_prefix'] and S['ft'].count('**Appended at b526 under `(R136)`(1)') == 1
     and S['dg'].count('**Appended at b526 under `(R136)`(1)') == 1 and S['notes']['sentence'] in S['ft'] and S['notes']['sentence'] in S['dg']
     and S['notes']['sentence'] == ' '.join(S['ferry'][S['ferry'].index('"the numbers cited'):S['ferry'].index('the instances differ."') + 22].split()),
     lambda S: put(S, 'ft_prefix', False)),
    ('G-XI-WINDOW-IMPORTED', 'the tool`s own text -- b521`s PWindow at xi, order 7, imported, no window class of its own',
     lambda S: 'B21.PWindow(a, \'xi\', P)' in S['tool'] and NL + 'P = 7' + NL in S['tool'] and 'class ' not in S['tool'],
     lambda S: put(S, 'tool', S['tool'].replace(NL + 'P = 7' + NL, NL + 'P = 5' + NL))),
    ('G-GRID-COMPLETE', 'the cell bank -- every width a = 15..60, once, in order', lambda S: [c['a'] for c in S['rows']] == GRID,
     lambda S: put(S, 'rows', S['rows'][:-1])),
    ('G-REACH-RECOMPUTED', 'every cell`s IN flag recomputed from its error terms, and the results` count',
     lambda S: all(c['within'] == (c['Etail'] <= c['Eu'] + c['Ek'] + c['Eround']) for c in S['rows'])
     and S['res']['within'] == sum(1 for c in S['rows'] if c['within']),
     lambda S: put(S, 'rows', [dict(c, within=not c['within']) for c in S['rows']])),
    ('G-SPLIT-SUMS', 'every cell -- the zero side the pair plus the on-line rest',
     lambda S: bool(S['rows']) and all(rel(c['Z'], c['on_rest'] + c['pair'] + c['others']) for c in S['rows']),
     lambda S: put(S, 'rows', [dict(c, others=c['others'] + 1.0) for c in S['rows']])),
    ('G-RATIO-RECOMPUTED', 'every cell -- pair / (Z - pair), recomputed',
     lambda S: bool(S['rows']) and all(rel(c['ratio'], c['pair'] / (c['Z'] - c['pair']), 1e-7) for c in S['rows']),
     lambda S: put(S, 'rows', [dict(c, ratio=0.1) for c in S['rows']])),
    ('G-VERIFIED-RECOUNTED', 'every cell`s flag, and the results` counts',
     lambda S: all(c['verified'] == (abs(c['r']) <= c['B']) for c in S['rows'])
     and S['res']['verified'] == sum(1 for c in S['rows'] if c['within'] and abs(c['r']) <= c['B']),
     lambda S: put(S, 'res', dict(S['res'], verified=-1))),
    ('G-BOUND-FORM', 'every cell -- B the four terms, B` the three', lambda S: bool(S['rows']) and all(
        rel(c['B'], c['Eu'] + c['Ek'] + c['Etail'] + c['Eround']) and rel(c['Bprime'], c['Eu'] + c['Ek'] + c['Eround']) for c in S['rows']),
     lambda S: put(S, 'rows', [dict(c, B=c['B'] * 2 + 1e-9) for c in S['rows']])),
    ('G-SHORTFALL-PRINTED', 'every cell carries its ESTIMATE; the report prints the check',
     lambda S: all(c.get('shortfall') is not None for c in S['rows']) and 'shortfall ESTIMATE above B' in S['report'],
     lambda S: put(S, 'report', S['report'].replace('shortfall ESTIMATE above B', 'x'))),
    ('G-SIGNS-RECOUNTED', 'the results` sign lists recounted from the cells',
     lambda S: S['res']['positive'] == [c['a'] for c in S['rows'] if c['within'] and abs(c['r']) <= c['B'] and c['h2'] > c['B']]
     and S['res']['negative'] == [c['a'] for c in S['rows'] if c['within'] and abs(c['r']) <= c['B'] and c['h2'] < -c['B']],
     lambda S: put(S, 'res', dict(S['res'], negative=[34.0]))),
    ('G-HALT-PROVED', 'the bump bank`s Mathlib quotations re-read HERE in Mathlib`s source at the pin',
     lambda S: all(q['hits'] and all(t in read(os.path.join(ML, q['file'][len('Mathlib/'):])).split(NL)[n - 1] for n, t in q['hits'])
                   for q in S['bump']['quotes'])
     and any('Nonempty.some hb.out' in t for q in S['bump']['quotes'] for _n, t in q['hits'])
     and any(': Prop where' in t for q in S['bump']['quotes'] for _n, t in q['hits']),
     lambda S: put(S, 'bump', dict(S['bump'], quotes=[dict(q, hits=[[1, 'no such line']]) for q in S['bump']['quotes']]))),
    ('G-NONUNIQUE-CONTROL', 'the control recomputed HERE -- both bases pass the fields, they differ, the bad one fails',
     lambda S: nonunique_now() and S['bump']['both_pass'] is True and S['bump']['bad_rejected'] is True and S['bump']['max_pointwise_diff'] > 0,
     lambda S: put(S, 'bump', dict(S['bump'], both_pass=False))),
    ('G-NOTHING-READ-ON-KERNEL-PHI', 'the data directory and the report -- no Q0 or xi cell on the kernel`s phi banked; NOT READ said',
     lambda S: not [f for f in os.listdir(D) if f.startswith('b526_') and ('bump_cells' in f or 'kphi' in f)]
     and 'Q0 and xi on the kernel`s phi: NOT READ' in S['report'],
     lambda S: cut(S, 'report', 'Q0 and xi on the kernel`s phi: NOT READ')),
    ('G-FORM-REDERIVED', 'the results` form against Component 1 alone -- present iff xi positive at all 46',
     lambda S: (S['res']['form'] is not None) == (len(S['res']['positive']) == 46 == len(S['rows']))
     and (S['res']['form'] is None or ('ramp order 7' in S['res']['form'] and S['res']['form'] in trail(S))),
     lambda S: put(S, 'res', dict(S['res'], form=None))),
    ('G-N1-SCORED', 'the desk against the cells', lambda S: 'n1' in S['sc'] and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == (S['res']['verified'] >= 1 and len(S['res']['positive']) == S['res']['verified']),
     lambda S: put(S, 'sc', dict(S['sc'], n1=not S['sc'].get('n1')))),
    ('G-N2-N3-NOT-SCORABLE', 'the desk and the scores -- both NOT SCORABLE, the conflict named',
     lambda S: S['sc'].get('n2', 1) is None and S['sc'].get('n3', 1) is None and '**(N2)** ### **NOT SCORABLE.**' in S['desk']
     and '**(N3)** ### **NOT SCORABLE.**' in S['desk'] and 'Classical.choice' in S['desk'],
    ...
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from the banks',
     ...
     and desk_word(S, 'S1') == word_of(len(S['rows']) == 46 and all(c['within'] and abs(c['r']) <= c['B'] for c in S['rows']))
     and desk_word(S, 'S2') == word_of(len(S['res']['positive']) == 46)
     and desk_word(S, 'S3') == word_of(S['bump']['max_pointwise_diff'] > 0.01),
     lambda S: put(S, 'desk', S['desk'].replace('**(S1)** ### **', '**(S1)** ### **NOT'))),
    ('G-CORPUS-SCOPE', 'the PLACE-papers file list -- FINDINGS, the digest, the trail; no chain file edited; the kernel untouched',
     lambda S: sorted(S['tracked']) == sorted(['FINDINGS.md', 'OPEN_TRAILS.md', 'phase2/method/THE_FINDINGS_AS_THEY_STAND.md'])
     and S['chain_clean'] is True and S['ker_changed'] == set(),
     lambda S: put(S, 'ker_changed', {'Zeta23/Defs.lean'})),
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b526')
              and 'data/b526_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b526 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b526_checks_postpush.txt' if pushed else 'b526_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b526_exercise.json'), 'w', encoding='utf-8', newline=NL),
