# -*- coding: utf-8 -*-
"""b479_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b479_registration_2026-09-22.txt')
SPEC = os.path.join(ROOT, 'data', 'b479_satisfiable.json')
...
CLAUSES = [
    ...
    ('relations graded and recorded in this act`s record and the trail', 7, 7, 'relations', '### section (B)(2): one verdict per class, seven rows over eight facts.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b479.zip, under (R69).'),
    ...
    ('repositories hard-failing at step zero', 4, 0, 'repositories', '### section (A) READING (0): the FIRST run read 4, all ls-remote UNRESOLVED at once -- a transient network failure; the retry reads 0 of 4 and BOTH runs are banked.'),
    ('repositories ahead of origin at step zero', 0, 0, 'repositories', '### section (A): nothing pushed because nothing was ahead.'),
    ('readings of the order declared in advance on this face', 7, 7, 'readings', '### section (A): (0) to (6), seven.'),
    ('pre-face reads declared', 6, 6, 'reads', '### section (A): (P0) to (P5).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R96) re-issues this act under its own number and fixes the fold`s numbering and span rule.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b479_addendum.txt, under the stem glob (R91) permits.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('expectations registered', 3, 3, 'expectations', '### section (J): (N1), (N2), (N3) as b479`s own banked ferry registered them; the amendment says they stand.'),
...
def main(argv):
    print('=' * 100)
    print('b479_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b479_registration_2026-09-22.txt -- b479, "
                             "THE SEVEN CLASSES AGAINST THE FORM, A READ"),
            "clauses": clauses}
