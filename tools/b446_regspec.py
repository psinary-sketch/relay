# -*- coding: utf-8 -*-
"""b446_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b446_registration_2026-09-12.txt')
SPEC = os.path.join(ROOT, 'data', 'b446_satisfiable.json')
...
CLAUSES = [
    ...
    ('TECHNE-Core writes', 1, 1, 'writes', '### section (W): FLOOR_DOMAIN_RULE.md, local, not pushed.'),
    ...
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### (P5), (P7): the word count and the timings are declared.'),
    ('author rulings ratified and entered', 3, 3, 'rulings', '### section (A): (R58) as amended, the four-part ruling, (R59) with (a) and (b).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b446_addendum.txt.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (K) BAR 10.'),
    ('lanes opened', 1, 1, 'lanes', '### section (A): the instrument lane under (R58).'),
    ...
    ('cells the second doubling runs at', 5, 5, 'cells', '### reading (3).'),
    ('functions of radius fitted', 0, 0, 'fits', '### reading (1).'),
    ('census matchers declared', 4, 4, 'matchers', '### reading (2): M0 to M3.'),
    ('census controls declared', 3, 3, 'controls', '### reading (2): two positive, one negative.'),
    ('rules mechanized', 0, 0, 'rules', '### reading (4): filed as judgement by (P6).'),
    ('errors entered as their owners`', 3, 3, 'errors', '### section (A): two the navigator`s, one the seat`s.'),
    ('chain files edited', 0, 0, 'files', '### (K) BAR 7.'),
    ('new cells or radii', 0, 0, 'cells', '### (R58).'),
    ('factors or tolerances stated after a value', 0, 0, 'rules', '### (K) BAR 2.'),
    ('verdicts forced into a named outcome', 0, 0, 'verdicts', '### (K) BAR 5.'),
    ('estimates at an assumed order printed unlabelled', 0, 0, 'estimates', '### (K) BAR 6.'),
    ('banked figures re-verdicted', 0, 0, 'figures', '### reading (2) and (K) BAR 4.'),
...
def main(argv):
    print('=' * 100)
    print('b446_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b446_registration_2026-09-12.txt -- b446, "
                             "THE FLOOR HAS A DOMAIN"),
            "clauses": clauses}
