...
### ### **THERE IS A DECLARED THRESHOLD, AND IT IS NINE.** ### `(R1)`, the author's ruling
...
def main(argv=None):
    ...
    # ### **REPAIRED AT b403.** ### The three lines this replaces said there was no
    ...
    rec('  ### **AND THE DECLARED THRESHOLD IS NINE** -- `(R1)`, the author`s ruling at `b366`:')
    rec('  ### *THE FOLD THRESHOLD IS NINE ACTS.* ### **THE SPANS ABOVE ARE THE FOLDS THAT HAPPENED;')
    rec('  ### ### THE THRESHOLD IS THE RULE, AND THEY ARE DIFFERENT THINGS.** ### This tool prints both')
    ...
    io.open(os.path.join(D, '%s_span.json' % emit), 'w', encoding='utf-8',
            newline=chr(10)).write(json.dumps(
                dict(folds=F, spans=spans, shortest=spans[0], longest=spans[-1], middle=mid,
                     ...
                     current_span=current, reached_shortest=bool(reached), threshold_declared=True, threshold=9, threshold_ruling='(R1) at b366',
                     emitted_for=emit,
