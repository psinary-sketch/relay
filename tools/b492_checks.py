# -*- coding: utf-8 -*-
"""b492_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE.
...
FACE = os.path.join(D, 'b492_registration_2026-09-23.txt')
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b492_ferry.txt')),
        scan=read(os.path.join(D, 'b492_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b492_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b492_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b492_pins_stepzero.txt')),
        extract=read(os.path.join(D, 'b492_extract.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b492_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b491_closing.txt')),
        addendum=read(os.path.join(D, 'b492_addendum.txt')),
        comp=read(os.path.join(D, 'b492_components.txt')),
        desk=read(os.path.join(D, 'b492_desk_notes.txt')),
        span=read(os.path.join(D, 'b492_span_notes2.txt'))
        + read(os.path.join(D, 'b492_span_notes.txt')),
        res=json.loads(read(os.path.join(D, 'b492_results.json')) or '{}'),
        sc=json.loads(read(os.path.join(D, 'b492_scores.json')) or '{}'),
        sv=json.loads(read(os.path.join(D, 'b492_survey.json')) or '{}'),
        err=read(os.path.join(PP, 'ERRATA.md')),
        rower=read(os.path.join(T, 'corr_row.py')),
        cells=json.loads(read(os.path.join(D, 'b492_cells.json')) or '{}'),
        run=read(os.path.join(D, 'b492_run.log')),
        gen=read(os.path.join(T, 'b437_components.py')),
        ...
        suite=read(os.path.join(T, 'b492_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads',
                                           'mirror-refresh-*-b492.zip'))),
        spantool_clean=(gits(ROOT, 'status', '--porcelain', '--', 'tools/b363_span.py') == ''),
        tools492=''.join(read(os.path.join(T, f)) for f in sorted(os.listdir(T))
                         if f.startswith('b492_') and f.endswith('.py')
                         and f != 'b492_checks.py'),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b492 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b492_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b492 --'):
    ...
    prior = [f for f in os.listdir(D)
             if re.match(r'^b4[0-7][0-9]_|^b48[0-9]_', f) and not f.startswith('b492_') and f not in LIVE]
...
ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry parts',
     lambda S: ('paste ends (part 2 of 2)' in S['ferry'] and 'ACT b492' in S['ferry']
                and 'paste ends (part 1 of 1)' in read(os.path.join(D, 'b493_ferry.txt'))),
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b491`s closing AND the ledger',
     lambda S: ('the commits, each read back by `ls-remote`' in S['prior']
                and S['corr'].count('| 340 |') == 1),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: ('ACT b492' in S['ferry'] and 'b492' in S['face']
                and not os.path.exists(os.path.join(D, 'b494_ferry.txt'))),
     lambda S: cut(S, 'face', 'b492')),

    # -------------------------------------------------- the run
    ('G-RUN-ALL-35-CELLS', 'the cells bank against b477`s own entries',
     lambda S: (len(rows(S)) == 35
                and S['b477'].count('"kind": "diagonal"') == 35),
     lambda S: put(S, 'cells', dict(S['cells'], rows=rows(S)[:30]))),
    ('G-RUN-W-WITHIN-FLOOR', 'the cells bank -- every cell, against the floor',
     lambda S: (all(r['dW'] < (S['cells'] or {}).get('floor', 0) for r in rows(S))
                and res(S, 'worst_dW') == max(r['dW'] for r in rows(S))),
     lambda S: put(S, 'cells', dict(S['cells'], rows=[
         dict(r, dW=1.0) for r in rows(S)]))),
    ('G-RUN-FLOOR-IS-b477S', 'b477`s own runner, for the number this act used',
     lambda S: ('FLOOR = 1.49e-08' in S['gram']
                and abs((S['cells'] or {}).get('floor', 0) - 1.49e-08) < 1e-20),
     lambda S: put(S, 'cells', dict(S['cells'], floor=1.0))),
    ('G-RUN-RECIPE-IS-b477S', 'b477`s runner against this act`s own run tool',
     lambda S: (all(x in S['gram'] for x in ('SQ.autocorrelation(seeds[a])',
                                             'WI.channels(f.v, f.w)',
                                             "ch['prime'] - ch['arch']"))
                and all(x in S['tools492'] for x in ('SQ.autocorrelation(g)',
                                                     'WI.channels(f.v, f.w)',
                                                     "ch['prime'] - ch['arch']"))),
     lambda S: put(S, 'tools492',
                   S['tools492'].replace("ch['prime'] - ch['arch']", "ch['arch']"))),
    ('G-RUN-CHECK-BEFORE-READ', 'the run tool -- the halt precedes the banking',
     lambda S: (0 < S['tools492'].find('NOTHING FURTHER IS READ')
                < S['tools492'].find('rows.append(dict(')),
     lambda S: put(S, 'tools492', S['tools492'].replace('NOTHING FURTHER IS READ', 'x'))),
    ('G-RUN-REHEARSAL-EXACT', 'the survey bank -- the cell whose terms a prior act banked',
     lambda S: ('THE REHEARSAL REPRODUCES A BANK THIS ACT DID NOT WRITE : True' in S['extract']
                and 'WORST PER-TERM |diff| AGAINST b449`S BANK : 0' in S['extract']),
     lambda S: cut(S, 'extract', 'WORST PER-TERM |diff| AGAINST b449`S BANK : 0')),
    ('G-RUN-ONE-GRID-ONLY', 'the run tool -- no nv is chosen by this act',
     lambda S: 'nv=' not in S['tools492'].split('def main')[-1],
     lambda S: put(S, 'tools492', S['tools492'] + NL + '    f = SQ.autocorrelation(g, nv=65537)')),

    # -------------------------------------------------- the terms
    ('G-TERMS-AT-EVERY-CELL', 'the cells bank -- terms present and non-empty at all 35',
     lambda S: all(isinstance(r.get('terms'), dict) and len(r['terms']) >= 0 for r in rows(S))
     and sum(1 for r in rows(S) if r['terms']) == 32,
     lambda S: put(S, 'cells', dict(S['cells'], rows=[
         dict(r, terms={}) for r in rows(S)]))),
    ('G-TERMS-SUM-TO-PR', 'the cells bank -- the terms account for the prime channel',
     lambda S: all(abs(r['terms_sum'] - r['prime']) < 1e-14 for r in rows(S)),
     lambda S: put(S, 'cells', dict(S['cells'], rows=[
         dict(r, terms_sum=r['prime'] + 1.0) for r in rows(S)]))),
    ('G-EXACT-A-BESIDE-ROUNDED', 'the cells bank -- both values at every cell',
     lambda S: (all('exact' in r and 'a' in r for r in rows(S))
                and any(r['exact'] != r['a'] for r in rows(S))
                and all(abs(r['exact'] - r['a']) < 1e-6 for r in rows(S))),
     lambda S: put(S, 'cells', dict(S['cells'], rows=[
         dict(r, exact=r['a']) for r in rows(S)]))),
    ('G-EXACT-MEMBERSHIP-INTEGER', 'the components bank -- the boundary argument, in integers',
     lambda S: ('`n0 <= n0`' in S['comp'] and 'integer arithmetic, no tolerance' in S['comp']),
     lambda S: cut(S, 'comp', 'integer arithmetic, no tolerance')),
    ('G-GENERATOR-QUOTED', 'b437`s own source against the survey bank',
     lambda S: ('NEW_A = sorted(set([round(x, 6) for x in BOUNDARIES + MIDPOINTS]))' in S['gen']
                and 'round(x, 6)' in S['extract']),
     lambda S: cut(S, 'extract', 'round(x, 6)')),

    # -------------------------------------------------- the attribution
    ('G-SPLIT-IS-AN-IDENTITY', 'the results bank -- recomputed from the cells, cell by cell',
     lambda S: len(steps(S)) == 34 and all(
         abs(s['dpr'] - (s['new'] + s['drift'])) < 1e-12 for s in steps(S)),
     lambda S: put(S, 'res', dict(S['res'], steps=[
         dict(s, drift=s['drift'] + 1.0) for s in steps(S)]))),
    ('G-SPLIT-CLOSES', 'the results bank -- the worst closure, against the face`s bar',
     lambda S: res(S, 'worst_closes') < 1e-12
     and res(S, 'worst_closes') == max(s['closes'] for s in steps(S)),
     lambda S: put(S, 'res', dict(S['res'], worst_closes=1.0))),
    ('G-34-ROW-TABLE', 'the components bank -- one printed row per step',
     lambda S: len([l for l in S['comp'].split(NL)
                    if re.match(r'\s+\d+\s+\d+\.\d{6}\s+\d+\.\d{6}\s', l)]) == 34,
     # ### ### **THE CONTROL CUT THE FILE`S TAIL, WHERE THE TABLE IS NOT.** ### It removed forty
     # ### lines from the end and left all 34 rows standing. ### It now deletes a ROW.
     lambda S: put(S, 'comp', NL.join(
         l for l in S['comp'].split(NL)
         if not re.match(r'\s+17\s+\d+\.\d{6}\s+\d+\.\d{6}\s', l)))),
    ('G-ARCH-NOT-SPLIT-SAID', 'the face and the components bank -- the half NOT attributed',
     lambda S: ('THE ARCHIMEDEAN CHANNEL DOES NOT SPLIT THAT WAY' in S['face']
                and 'NOT DECOMPOSED HERE' in S['comp']),
     lambda S: cut(S, 'comp', 'NOT DECOMPOSED HERE')),
    ('G-FRACTION-SCOPE-STATED', 'the components bank -- the denominator is named',
     lambda S: 'THE DENOMINATOR IS `d(pr)`, THE PRIME CHANNEL`S OWN INCREMENT' in S['comp'],
     lambda S: cut(S, 'comp', 'THE DENOMINATOR IS `d(pr)`, THE PRIME CHANNEL`S OWN INCREMENT')),
    ('G-ONE-SENTENCE-NO-WIDER', 'the results bank and the trail -- one sentence, and its limits',
     lambda S: (res(S, 'sentence') and res(S, 'sentence')[:40] in S['ot']
                and 'WHAT IT DOES NOT SAY' in S['comp']),
     lambda S: cut(S, 'comp', 'WHAT IT DOES NOT SAY')),

    # -------------------------------------------------- the expectations
    ('G-N1-SCORED', 'the desk bank against the results bank',
     lambda S: 'REFUTED' in seg(S['desk'], '**(N1)**') and res(S, 'max_small_share') < 0.5,
     lambda S: put(S, 'res', dict(S['res'], max_small_share=0.9))),
    ('G-N2-SCORED', 'the desk bank -- a SPLIT is printed as one',
     lambda S: 'SPLIT' in seg(S['desk'], '**(N2)**') and res(S, 'turn_hi') is not None,
     lambda S: cut(S, 'desk', '**(N2)**')),
    ('G-N3-SCORED', 'the desk bank against the two means',
     lambda S: ('REFUTED' in seg(S['desk'], '**(N3)**')
                and res(S, 'mean_entry') < res(S, 'mean_nonentry')),
     lambda S: put(S, 'res', dict(S['res'], mean_entry=9.0))),
    ...
    # -------------------------------------------------- the standing arms
    ('G-LANE-CLOSED-AT-END', 'the trail -- the lane is said shut, in this act`s own record',
     lambda S: 'is shut at this act' in S['ot'].split('### b492 ')[-1],
     lambda S: cut(S, 'ot', 'is shut at this act')),
    ('G-NOTHING-COMPILED', 'this act`s own tools, for a LEAN BUILD',
     # ### ### **THE ALTERNATION LOST `run`, SO `subprocess.run(` COULD NOT MATCH.** ### The arm
     # ### demanded `subprocess(` -- a call shape that never occurs -- and its own control walked
     # ### straight past it. ### **AN ARM THAT CANNOT MATCH ITS CONTROL HAS NOT BEEN EXERCISED.**
     lambda S: not re.search(r'(?:subprocess|os\.system|Popen|check_output|run)\s*\([^)]{0,80}'
                             r'(?:lake|LEAN_PATH|lean)\b|lean\s+--\w', S['tools492']),
     lambda S: put(S, 'tools492', S['tools492'] + NL + 'subprocess.run(["lake","build","X"])')),
    ('G-NOZENODO-WRITE', 'this act`s own tools, for a platform WRITE CALL',
     lambda S: not re.search(r'requests\.(?:post|put|patch|delete)\s*\(|'
                             r'zenodo.{0,60}/(?:publish|newversion|files)\b', S['tools492'], re.I),
     lambda S: put(S, 'tools492', S['tools492'] + NL
    ...
    ('G-TRAIL-APPEND-ONLY', 'the trail own text',
     lambda S: S['ot'].count('### b492 —') == 1,
     lambda S: put(S, 'ot', S['ot'] + NL + '### b492 — a second record that must not exist')),
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger',
     lambda S: ('| 341 |' in S['corr'] and S['corr'].count('| 341 |') == 1
                and S['corr'].count('b492, under (R101)') == 1),
     lambda S: put(S, 'corr', S['corr'] + NL + '| 341 | a duplicate row |')),
    ('G-WRITELIST-KINDS', 'every b492 commit in three repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: "data/b492_components.txt' in gits(ROOT, 'show'" in S['suite'],
     lambda S: cut(S, 'suite', "data/b492_components.txt' in gits(ROOT, 'show'")),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b492')
              and 'data/b492_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b492 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b492_checks_postpush.txt' if pushed else 'b492_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b492_exercise.json'), 'w', encoding='utf-8', newline=NL),
