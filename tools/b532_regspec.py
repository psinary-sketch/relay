# -*- coding: utf-8 -*-
"""b532_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b531's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): two NEW kernel
### ### files; no grade; FACES_LEDGER written by one appended note.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b532_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b532_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): H2Bridge.lean and AxiomCheckBridge.lean, both NEW.'),
    ...
    ('lanes opened', 1, 1, 'lanes', '### (R142)(5): the kernel lane for this act alone.'),
    ...
    ('grades conferred', 0, 0, 'grades', '### (Z): no grade conferred or moved.'),
    ...
    ('props stated and proved', 0, 0, 'props', '### READING (4): h2_sign_imp_ch is stated, not proved.'),
    ...
    ('ERRATA entries written', 0, 0, 'entries', '### READING (3): drafted to data, not filed.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b532.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R142).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b532_addendum.txt, under the stem glob.'),
    ...
    ('PLACE-papers documents written at all', 2, 2, 'documents', '### (W): OPEN_TRAILS.md and FACES_LEDGER.md, one append each.'),
...
def main(argv):
    print('=' * 100)
    print('b532_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b532_extract.txt'),
    ...
    spec = {"registration": ("data/b532_registration_2026-09-25.txt -- b532, "
                             "THE DEPOSIT READ, THE ERRATUM DRAFTED, THE TRIVIAL BRIDGE"),
            "clauses": clauses}
