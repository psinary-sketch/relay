# -*- coding: utf-8 -*-
"""b339_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b339_registration_2026-09-06.txt')
SPEC = os.path.join(ROOT, 'data', 'b339_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ("PLACE-papers files written", 2, 2, "files", "### section (G): FACES_LEDGER.md, one block; ERRATA.md, one appended block only on RESOLVED."),
    ("ERRATA entries edited", 0, 0, "entries", "### section (D): the entry untouched; an append only."),
    ("ledger rows written (faces)", 0, 0, "rows", "### section (G): an update block, not a row."),
    ("correspondence rows appended", 1, 1, "rows", "### section (G)."),
    ("findings sections edited", 0, 0, "sections", "none."),
    ("bars moved", 0, 0, "bars", "### section (A): no bar moved; must-fail fixture."),
    ("grades conferred by a seat", 0, 0, "grades", "section (A)."),
    ("cells added or moved after the price", 0, 0, "cells", "### section (C): the gate on the run."),
...
def main(argv):
    print('=' * 100)
    print('b339_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": "data/b339_registration_2026-09-06.txt -- b339, THE EXPONENT RESOLVED", "clauses": clauses}
