# -*- coding: utf-8 -*-
"""b430_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b430_registration_2026-09-12.txt')
SPEC = os.path.join(ROOT, 'data', 'b430_satisfiable.json')
...
CLAUSES = [
    ...
    ('author rulings ratified by this paste', 1, 1, 'rulings', '### section (A): (R40), quoted from the banked paste in reading (1).'),
    ...
    ('addresses resolved or documents fetched', 0, 0, 'reads', '### section (Z): leg 2`s addresses are untouched by this act.'),
    ...
    ('existing grades moved by this act', 0, 0, 'grades', '### (K) BAR 7: (R40)`s own last clause.'),
    ...
    ('existing `relay` tools edited in place', 2, 2, 'tools', '### (W): ferry_scan.py repaired and FERRY_STANDING.md amended, BOTH BY THE RE-ISSUE`S OWN ORDER.'),
...
def main(argv):
    print('=' * 100)
    print('b430_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b430_registration_2026-09-12.txt -- b430, THE EXTERNAL GRADING READ"),
            "clauses": clauses}
