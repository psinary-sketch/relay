# -*- coding: utf-8 -*-
"""b534_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b533's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): NEW kernel files
### ### and a comment-only note; grades by statement-read; the trail alone in PLACE-papers; one Prop stated and proved.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b534_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b534_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 3, 3, 'files', '### (W): PowerLimit.lean and AxiomCheckLimit.lean NEW; PowerWindow.lean a comment-only note.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): no lake build; lake env lean per file, READING (1) and (10).'),
    ('chains run, or channel values computed', 0, 0, 'runs', '### (Z): no number computed.'),
    ('lanes opened', 1, 1, 'lanes', '### (R144): the kernel lane for this act.'),
    ...
    ('grades conferred by statement-read on this act`s compiled model theorems', 19, 0, 'grades', '### READING (11): at most the nineteen named; a halted lemma takes none.'),
    ...
    ('props stated and proved', 1, 0, 'props', '### READING (8): zeroSideNeg, stated and discharged by zeroSideNeg_holds; rh_strip_imp_rh and nodes_distinct_nonreal stay unproved.'),
    ...
    ('prior acts` banks edited', 0, 0, 'files', '### (W) and READING (2): the note is a NEW b534 file beside b533`s face.'),
    ...
    ('ERRATA entries written', 0, 0, 'entries', '### (R144)(5): the update act files, not this one.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b534.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R144).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b534_addendum.txt, under the stem glob.'),
...
def main(argv):
    print('=' * 100)
    print('b534_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b534_extract.txt'),
    ...
    spec = {"registration": ("data/b534_registration_2026-09-25.txt -- b534, "
                             "f4, THE POWER-WINDOW ROUTE, ACT TWO OF TWO"),
            "clauses": clauses}
