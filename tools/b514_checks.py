# -*- coding: utf-8 -*-
"""b514_checks.py -- THE SUITE, IN b461's SUCCESSOR SHAPE, CARRIED FROM b513's AND RE-POINTED ARM BY ARM.
...
FACE = os.path.join(D, 'b514_registration_2026-09-24.txt')
OT = os.path.join(PP, 'OPEN_TRAILS.md')
PRIOR_RELAY = '237ad9dc'      # ### b513's closing commit -- relay's tip before this act
KER = os.path.join('D:', os.sep, 'SIDE-explicit-formula')
KER_TIP = '0a9391e'           # ### the kernel's tip, untouched by this act
PRIOR_PP = 'dc2e386'           # ### b513's PLACE-papers commit
NL = chr(10)
BT = chr(96)
L, RES, EX = [], [], []
TRAILH = '### b514 —'
...
HARD = 'no Q0 cell is negative on a closing bank: the converse`s construction is harder than a modulated bump'
...
def sources():
    S = dict(
        face=read(FACE), ferry=read(os.path.join(D, 'b514_ferry.txt')),
        scan=read(os.path.join(D, 'b514_ferry_scan.txt')),
        cens=read(os.path.join(D, 'b514_census_stepzero.txt')),
        fcens=read(os.path.join(D, 'b514_faces_census_stepzero.txt')),
        pins=read(os.path.join(D, 'b514_pins_stepzero.txt')),
        lock=read(sorted(glob.glob(os.path.join(D, 'b514_lockgate_notes*.txt')))[-1]),
        seal=subprocess.run([sys.executable, os.path.join(T, 'reg_seal.py'), '--verify', FACE],
                            capture_output=True, text=True, encoding='utf-8', errors='replace').stdout,
        prior=read(os.path.join(D, 'b513_closing.txt')),
        addendum=read(os.path.join(D, 'b514_addendum.txt')),
        comp=read(os.path.join(D, 'b514_components.txt')),
        desk=read(os.path.join(D, 'b514_desk_notes.txt')),
        report=read(os.path.join(D, 'b514_report.txt')),
        cells=sorted(jsonl(os.path.join(D, 'b514_cells.jsonl')), key=lambda c: c['a']),
        ladder=sorted(c['a'] for c in jsonl(os.path.join(D, 'b511_cells.jsonl')) if c.get('family') == 'bspline'),
        res=json.loads(read(os.path.join(D, 'b514_results.json')) or '{}'),
        fx=json.loads(read(os.path.join(D, 'b514_fixture.json')) or '[]'),
        mt=json.loads(read(os.path.join(D, 'b514_matched.json')) or '[]'),
        sc=json.loads(read(os.path.join(D, 'b514_scores.json')) or '{}'),
        ot=read(OT), corr=read(os.path.join(SIDE, 'CORRESPONDENCE.md')),
        suite=read(os.path.join(T, 'b514_checks.py')),
        ...
        mirror=bool(glob.glob(os.path.join('D:', os.sep, 'MY-DOwnloads', 'mirror-refresh-*-b514.zip'))),
        chain_clean=(gits(ROOT, 'status', '--porcelain', '--', 'tools/b321_window.py', 'tools/e16/carto_atlas.py',
                          ...
                          'tools/b504_chain.py', 'tools/b506_kernel.py', 'tools/b511_families.py', 'tools/b503_zero.py') == ''),
        ker_clean=(gits(KER, 'rev-parse', '--short=7', 'HEAD') == KER_TIP and gits(KER, 'status', '--porcelain', '--untracked-files=no') == ''),
        tracked=(sorted(x for x in gits(PP, 'show', '--name-only', '--pretty=format:', 'HEAD').split(NL) if x.strip())
                 if gits(PP, 'log', '-1', '--pretty=%s').startswith('b514 --')
                 else sorted(p[3:].strip() for p in git(PP, 'status', '--porcelain').split(NL)
                             if p.strip() and not p.lstrip().startswith('??'))),
        kinds=set(), mustfail=not os.path.exists(os.path.join(D, 'b514_mustnotexist.txt')),
        dep_clean=(gits(PP, 'status', '--porcelain', '--', 'outputs/DEPOSITED-v1.1.2') == ''),
        after_lock=all(os.path.exists(p) and os.path.getmtime(p) > os.path.getmtime(FACE)
                       for p in (os.path.join(D, x) for x in ('b514_fixture.json', 'b514_matched.json', 'b514_cells.jsonl'))),
    )
    k = set(os.path.basename(x) for x in S['tracked'])
    for repo in (ROOT, PP, SIDE, KER):
        for l in gits(repo, 'log', '--pretty=%H %s', '-30').split(NL):
            if l.strip() and l.split(' ', 1)[-1].startswith('b514 --'):
    ...
    S['kinds'] = k
    prior = [f for f in os.listdir(D) if re.match(r'^b4[0-9][0-9]_|^b50[0-9]_|^b51[0123]_|^b334_', f)]
...
def khat(a, g0, z):
    """### the window's transform recomputed HERE from READING (1)'s formula, sharing no code with the act's tool."""
    h = 2.0 * math.log(a) / 4.0
    gh = 0.5 * (csinc((z - g0) * h / 2.0) ** 4 + csinc((z + g0) * h / 2.0) ** 4)
    return gh * gh
...
def recount(S, o):
    ...
    return dict(verified=len(V), m_neg=[c['a'] for c in V if c[o]['m'] < -c[o]['B']],
                m_pos=[c['a'] for c in V if c[o]['m'] > c[o]['B']], h2_neg=[c['a'] for c in V if c[o]['h2'] < -c[o]['B']],
                pair_neg=[c['a'] for c in C if c[o]['pair'] < 0], share_over_half=[c['a'] for c in V if c[o]['share'] > 0.5],
                shortfall_over_B=[c['a'] for c in C if c[o]['shortfall'] > c[o]['B']])


def corr_row(S):
    return next((l for l in S['corr'].split(NL) if l.startswith('| 363 |')), '')
...
ARMS = [
    ('G-RECEIPT-IN-FULL', 'the banked ferry -- the ruling and part 1 of 1, each END present',
     lambda S: 'RULING (R123) END' in S['ferry'] and 'FERRY END (part 1 of 1)' in S['ferry'] and 'ACT b514' in S['ferry'],
    ...
    ('G-PRIOR-CLOSED-PUSHED', 'b513`s closing AND the ledger',
     lambda S: 'THE COMMITS, THE MIRROR, THE CENSUSES' in S['prior'] and S['corr'].count('| 362 |') == 1,
     lambda S: put(S, 'corr', S['corr'].replace('| 362 |', '| 3620 |'))),
    ...
    ('G-NUMBER-UNCLAIMED', 'the data directory, and this act`s own banked order',
     lambda S: 'ACT b514' in S['ferry'] and 'ACT b514' in S['face'] and not glob.glob(os.path.join(D, 'b515_registration_*.txt')),
     lambda S: cut(S, 'face', 'ACT b514')),
    ...
    ('G-R123-ENTERED', 'the banked ferry AND the trail', lambda S: 'RULING (R123) END' in S['ferry'] and S['ot'].count('**(R123) ratified') == 1,
     lambda S: put(S, 'ot', S['ot'].replace('**(R123) ratified', '(R123) noted'))),
    ('G-WORKORDERS-FILED', 'the trail`s own record -- both work-orders by name, once each',
     lambda S: trail(S).count('`W-ORD-TABLE-PROFILE-JSON`') == 1 and trail(S).count('`W-ORD-TABLE-SHORTNAME-DEDUP`') == 1
     and 'trigger the next fold' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace('`W-ORD-TABLE-SHORTNAME-DEDUP`', 'a second order'))),
    ('G-FIXTURE-BAR', 'the fixture bank -- the largest difference recomputed from its two columns, both objects',
     lambda S: len(S['fx']) == 2 and all(max(abs(a - b) for a, b in zip(f['closed'], f['numeric'])) <= 1e-10 and f['meets'] for f in S['fx']),
     lambda S: put(S, 'fx', [dict(f, numeric=[v + 1e-8 for v in f['numeric']]) for f in S['fx']])),
    ('G-CLASSK-STATED', 'the fixture bank AND the components -- generating h named, k even, g C^2 with a live probe',
     lambda S: all(f['generating_h'].startswith('g(u) = cos(') and f['even'] < 1e-14 and f['knot_jumps']['D3'] > 1.0
                   and max(f['knot_jumps'][d] for d in ('D0', 'D1', 'D2')) < 1e-4 for f in S['fx'])
     and S['comp'].count('classK (READING (2)): k = weilTest h h with h = g(u) = cos(') == 2,
     lambda S: put(S, 'fx', [dict(f, knot_jumps=dict(f['knot_jumps'], D2=0.3)) for f in S['fx']])),
    ('G-MATCHED-RATIO', 'the matched bank -- three ladder widths, two objects, each ratio recomputed',
     lambda S: len(S['mt']) == 6 and sorted(set(m['a'] for m in S['mt'])) == sorted({S['ladder'][0], S['ladder'][59], S['ladder'][118]})
     and all(rel(m['ratio'], abs(complex(*m['at_gamma0'])) / max(abs(complex(*v)) for v in m['at_nearest'])) for m in S['mt'])
     and all('at_gammaOf_rho' in m for m in S['mt'] if m['object'] == 'q'),
     lambda S: put(S, 'mt', [dict(m, ratio=m['ratio'] * 2) for m in S['mt']])),
    ...
    ('G-WINDOW-AS-WRITTEN', 'the cells` pole term and k-hat recomputed from READING (1)`s formula at three widths',
     lambda S: all(c[o]['gamma0'] == G0[o] and rel(c[o]['P'], (khat(c['a'], G0[o], 0.5j) + khat(c['a'], G0[o], -0.5j)).real)
                   for c in (S['cells'][0], S['cells'][59], S['cells'][118]) for o in ('q', 'xi'))
     and all(rel(m['at_gamma0'][0], khat(m['a'], G0[m['object']], G0[m['object']]).real) for m in S['mt']),
    ...
    ('G-SPLIT-SUMS', 'every cell -- the zero side equals the pair, the other off-line pairs and the on-line part',
     ...
     lambda S: put(S, 'cells', [dict(c, q=dict(c['q'], others=c['q']['others'] + 0.1)) for c in S['cells']])),
    ('G-SHARE-RECOMPUTED', 'every cell -- |pair| over the on-line rest, recomputed',
     lambda S: all(rel(c[o]['share'], abs(c[o]['pair']) / c[o]['on_rest']) for c, o in objs(S)) and bool(S['cells']),
     lambda S: put(S, 'cells', [dict(c, q=dict(c['q'], share=0.1)) for c in S['cells']])),
    ...
    ('G-BOUND-FORM', 'every cell -- B the sum of READING (6)`s four terms, the shortfall not in it',
     ...
     lambda S: put(S, 'cells', [dict(c, xi=dict(c['xi'], B=c['xi']['B'] + c['xi']['shortfall'] + 1e-9)) for c in S['cells']])),
    ...
    ('G-SIGNS-RECOUNTED', 'the results` sign lists against the cells, recounted',
     lambda S: all(S['res'][o][k] == recount(S, o)[k] for o in ('q', 'xi') for k in ('m_neg', 'm_pos', 'h2_neg', 'pair_neg', 'share_over_half')),
     lambda S: put(S, 'res', dict(S['res'], q=dict(S['res']['q'], m_neg=[5.0])))),
    ('G-READING-WORDS', 'the results` reading against the recount AND the trail -- (R122)(2)`s words, one of two forms',
     lambda S: S['res'].get('reading') == (HARD if not recount(S, 'q')['h2_neg'] else S['res'].get('reading'))
     and (HARD in S['res'].get('reading', '') or WIT in S['res'].get('reading', '')) and S['res'].get('reading', '') in trail(S).replace('  ', ' '),
    ...
    ('G-N1-SCORED', 'the desk against the recount', lambda S: 'n1' in S['sc'] and desk_word(S, 'N1') == word_of(S['sc']['n1'])
     and S['sc']['n1'] == (len(recount(S, 'q')['m_neg']) >= 1), lambda S: put(S, 'sc', dict(S['sc'], n1=not S['sc'].get('n1')))),
    ('G-N2-SCORED', 'the desk against the recount', lambda S: 'n2' in S['sc'] and desk_word(S, 'N2') == word_of(S['sc']['n2'])
     and S['sc']['n2'] == (recount(S, 'xi')['verified'] > 0 and len(recount(S, 'xi')['m_pos']) == recount(S, 'xi')['verified']),
    ...
    ('G-N3-SCORED', 'the desk against the recount', lambda S: 'n3' in S['sc'] and desk_word(S, 'N3') == word_of(S['sc']['n3'])
     and S['sc']['n3'] == (len(recount(S, 'q')['share_over_half']) >= 1), lambda S: put(S, 'sc', dict(S['sc'], n3=not S['sc'].get('n3')))),
    ('G-SEAT-EXPECTATIONS-SCORED', 'the face against the desk -- three registered, each word from the cells',
     ...
     and desk_word(S, 'S2') == word_of(all(c['q']['h2'] > c['q']['B'] for c in S['cells'] if abs(c['q']['r']) <= c['q']['B']))
     and desk_word(S, 'S3') == word_of(all(f['meets'] for f in S['fx'])),
    ...
    ('G-NOH2-MOVED', 'the trail`s own text -- THIS act`s record', lambda S: 'where the deposit left it' in trail(S),
     lambda S: put(S, 'ot', S['ot'].replace(TRAILH, '### b514 -'))),
    ...
    ('G-CORR-APPEND-ONLY', 'the correspondence ledger -- row 363 once, six cells as it landed',
     lambda S: S['corr'].count('| 363 |') == 1 and S['corr'].count('(b514, under (R123))') == 1
     and len(corr_row(S).strip().strip('|').split('|')) == 6,
     lambda S: put(S, 'corr', S['corr'] + NL + '| 363 | a duplicate row |')),
    ('G-INSTRUMENTS-UNEDITED', 'relay`s tools against b513`s close -- none modified',
     lambda S: S['tools_edited'] == [], lambda S: put(S, 'tools_edited', ['b511_families.py'])),
    ('G-WRITELIST-KINDS', 'every b514 commit in four repositories, against (R91)`s STEM GLOB',
    ...
    ('G-PUSHED-PREDICATE-THREE-CLAUSED', 'the suite`s own text -- b472`s repair, carried',
     lambda S: ("rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')" in S['suite']
                and "log', '-1', '--pretty=%s').startswith('b514')" in S['suite']
                and "data/b514_components.txt' in gits(ROOT, 'show'" in S['suite']),
     lambda S: cut(S, 'suite', "data/b514_components.txt' in gits(ROOT, 'show'")),
]
...
def main():
    ...
    pushed = (gits(ROOT, 'rev-parse', 'origin/main') == gits(ROOT, 'rev-parse', 'HEAD')
              and gits(ROOT, 'log', '-1', '--pretty=%s').startswith('b514')
              and 'data/b514_components.txt' in gits(ROOT, 'show', '--name-only', '--pretty=format:', 'HEAD'))
    ...
    rec('=' * 104)
    rec('b514 -- THE SUITE. ### **%s READING.** ### every arm exercised on both controls.'
    ...
    rec('=' * 104)
    out = os.path.join(D, 'b514_checks_postpush.txt' if pushed else 'b514_checks.txt')
    io.open(out, 'w', encoding='utf-8', newline=NL).write(NL.join(L) + NL)
    json.dump(dict(exercise=EX, run=len(RES), live_failing=fail, defective=defective,
                   neg_failures=negfail, declared=len(declared), deferred=deferred, stray=stray),
              io.open(os.path.join(D, 'b514_exercise.json'), 'w', encoding='utf-8', newline=NL),
