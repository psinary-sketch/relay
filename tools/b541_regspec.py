# -*- coding: utf-8 -*-
"""b541_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b540's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): no kernel file;
### ### one ERRATA entry filed with one bullet inserted; four PLACE-papers documents written.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b541_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b541_satisfiable.json')
...
CLAUSES = [
    ...
    ('ERRATA entries written', 1, 1, 'entries', '### READING (7): E-2026-09-25-4 filed as drafted.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R151).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b541_addendum.txt, under the stem glob.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('PLACE-papers documents written other than by appending', 1, 1, 'documents', '### READING (7): ERRATA, one bullet inserted.'),
    ('PLACE-papers documents written at all', 4, 4, 'documents', '### (W): ERRATA, the map, FINDINGS, OPEN_TRAILS.'),
...
def main(argv):
    print('=' * 100)
    print('b541_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b541_extract.txt'),
    ...
    spec = {"registration": ("data/b541_registration_2026-09-25.txt -- b541, "
                             "THE MONOGRAPH READ, ACT ONE, UNDER (R151)"),
            "clauses": clauses}
