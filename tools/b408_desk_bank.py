...
def main():
    bar('=')
    rec('b408 -- THE DESK, THE TRAIL BLOCK, THE ROW, THE KEY AND THE BANK.')
    bar('=')
    bar()
    ...
    if MARK in before:
        ...
        rec('  ### the b407 block is present and is not edited : %s' % (PRIOR in before))
        ...
        if not ao:
            ...
            rec('  ### run record : %s' % run_clock.write(D, 'b408_desk_notes', LINES))
            return 1
    seg = after.split(MARK, 1)[-1]
    low = ' '.join(seg.lower().split())
    says = {
        'says_c4_absent': ('c' + chr(0x2084) + '` is absent') in low,
        'says_c3_once': 'examined **once, inside the barrier keystone itself**' in low,
        'says_toolkit_silent': ('does not name `c' + chr(0x2084) + '` either') in low,
        'says_two_numberings': 'two class numberings' in low,
        'says_artefact': 'matcher artefact, not an examination' in low,
        'says_no_chan_claim': 'an unasked question is not an opportunity' in low,
        'says_four_imports': 'four distinct imported premises' in low,
        'says_with_hyp': 'a proof-with-hypotheses, not a proof in the lemma' in low,
        'says_conditional': 'still does not quantify over' in low,
        'says_unchecked': 'unchecked' in low,
        'says_not_written': 'priced and not written: 0 lines' in low,
        'says_span': 'the span is 6 and the threshold is 9' in low,
        'says_one_statement': 'are one statement' in low,
        'says_restatement': 'restatement of the third' in low,
        'says_naming_routed': 'the naming is routed to the author' in low,
        'says_bookkeeping': 'bookkeeping instrument' in low,
        'says_prices': 'routed prices, in one place for the first time' in low,
        'says_core': 'only 8 arms appear in all four' in low,
        'says_measure_limit': 'upper bound on continuity' in low,
    ...
    p = run_clock.write(D, 'b408_desk_notes', LINES)
