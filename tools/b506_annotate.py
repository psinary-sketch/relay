# -*- coding: utf-8 -*-
"""b506_annotate.py -- COMPONENT 4: THE ANNOTATIONS OF (R115)(1) AND (2), CITING b506`S MEASURED COUNT. ### `python tools/b506_annotate.py`
...
MARK = 'ANNOTATION APPENDED AT b506 UNDER (R115)'
...
SHORT = ('the Epstein bank it summed, `b326_epstein_zeros.json` (146 on-line zeros and 2 off-line), is short of its '
         'own count below height 150: b506`s whole-strip argument-principle count is %d against the bank`s %d '
         '(146 + 2 x 2), so the bank lacks %d (%d distinct zeros with sigma > 1/2 not in it, %d of them at sigma > 1; '
         '`relay/data/b506_c2_results.json`).' % (C2['total'], C2['bank_on'] + 4, C2['lacks_strip'], C2['new_off'], C2['beyond1']))
...
def main():
    if 'lacks_strip' not in C2:
        sys.exit('### NO MEASURED COUNT -- NOTHING ANNOTATED.')
    out = []
    ...
    io.open(os.path.join(D, 'b506_annotations.json'), 'w', encoding='utf-8', newline=NL).write(json.dumps(out, indent=1) + NL)
