# -*- coding: utf-8 -*-
"""b507_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b506's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): the numerical
### ### clauses are set to zero because this fold runs no chain, and the instrument clauses to the two
### ### edits (R117) names.
"""
import io
import json
...
REG = os.path.join(ROOT, 'data', 'b507_registration_2026-09-24.txt')
SPEC = os.path.join(ROOT, 'data', 'b507_satisfiable.json')
...
CLAUSES = [
    ...
    ('chains run, or channel values computed', 0, 0, 'runs', '### (Z): the numerical lane opens for b508, not here.'),
    ('lean runs', 0, 0, 'runs', '### (Z).'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z).'),
    ('lanes opened', 1, 1, 'lanes', '### (R117)(2): the instrument lane opens inside the fold and shuts at its close.'),
    ...
    ('scratch trees left', 0, 0, 'trees', '### (Z): the fixture`s temporary ledgers are removed.'),
    ...
    ('rules struck, amended, widened or re-ruled', 0, 0, 'rules', '### section (Z): the bar-floor arm is repaired as (R117)(1) orders, not re-ruled.'),
    ('FERRY_STANDING clauses added', 0, 0, 'clauses', '### section (Z).'),
    ('locked faces edited', 0, 0, 'faces', '### section (Z).'),
    ('prior acts` banks edited', 4, 4, 'files', '### (W) and (R117)(1),(3): ONE appended note each to three components files and b334`s aim map, prefix proved.'),
    ('banked ferries edited', 0, 0, 'files', '### (W): no banked ferry is written; b508`s ferry is banked new.'),
    ...
    ('expectations registered', 'EXPECT', 'EXPECT', 'expectations', '### section (D): NONE, measured off the face.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b507.zip, under (R69).'),
    ...
    ('ferry parts received', 2, 2, 'parts', '### section (A): part 1 of 2 and part 2 of 2, receipt IN FULL.'),
    ...
    ('measurements taken before the lock and not declared', 0, 0, 'measurements', '### section (C): the one rehearsal is declared.'),
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R117).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b507_addendum.txt, under the stem glob.'),
    ...
    ('existing `relay` tools edited in place', 2, 2, 'tools', '### (W) and (R117)(1),(2): registration_gate.py and corr_row.py.'),
...
def main(argv):
    print('=' * 100)
    print('b507_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b507_extract.txt'),
    ...
    spec = {"registration": ("data/b507_registration_2026-09-24.txt -- b507, "
                             "THE FOLD AT SPAN TWENTY"),
            "clauses": clauses}
