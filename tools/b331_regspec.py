# -*- coding: utf-8 -*-
"""b331_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC, ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.** ### Every clause is a forward
### commitment; the spec is emitted BEFORE any line of the section is emitted.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b331_registration_2026-09-06.txt')
SPEC = os.path.join(ROOT, 'data', 'b331_satisfiable.json')

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

CLAUSES = [
    ("PLACE-papers files written", 1, 1, "files", "### section (G): FINDINGS.md, one appended section."),
    ("sections appended to FINDINGS.md", 1, 1, "sections", "### section (G): the fold's own clause; idempotent on a second run."),
    ("grades moved", 0, 0, "grades", "section (A)."),
    ("acts re-verdicted", 0, 0, "acts", "section (A)."),
    ("numbers computed by this act", 0, 0, "numbers", "### section (A): every number banked by an act in the arc, carried with its owner."),
    ("keystones created or edited", 0, 0, "files", "### section (D)."),
    ("TECHNE files written or committed", 0, 0, "files", "### section (D): TECHNE is not touched."),
    ("`.lean` files edited", 0, 0, "files", "### section (D)."),
    ("deposited texts touched", 0, 0, "files", "the deposit is read, never written."),
    ("owner instrument files edited", 0, 0, "files", "### the prior generator is read for its rules, never edited."),
...
def main(argv):
    print('=' * 100)
    print('b331_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": "data/b331_registration_2026-09-06.txt -- b331, THE FOLD b323-b330", "clauses": clauses}
