# -*- coding: utf-8 -*-
"""b492_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b492_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b492_satisfiable.json')
...
CLAUSES = [
    ...
    ('chains run, or channel values computed', 1, 1, 'runs', '### (R101): ONE chain run, at 35 cells, on b477`s own recipe and grid.'),
    ('lean runs', 0, 0, 'runs', '### (Z): none.'),
    ('repositories cloned', 0, 0, 'repositories', '### (Z): every repository this act reads is already on disk.'),
    ('lanes opened', 1, 1, 'lanes', '### (R101): the NUMERICAL lane, open for THIS ONE RUN and closed at this act`s end. ### It is a Python chain run; NO LEAN IS COMPILED AND NO BUILD IS LAUNCHED.'),
    ...
    ('prior acts` banks edited', 0, 0, 'files', '### (W): NONE. The chain tools are IMPORTED AND CALLED and not one byte of any of them is written.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b492.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b492_extract.txt`.'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R102) and (R103), both ratified here and entered by this act`s trail record. ### Neither changes this act`s work: (R102) holds the platform edits for one act; (R103) speaks of the act after b493.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b492_addendum.txt, under the stem glob (R91) permits.'),
    ...
    ('PLACE-papers documents written at all', 1, 1, 'documents', '### (W): OPEN_TRAILS.md alone -- one appended record. NO FINDINGS SECTION: this act folds nothing.'),
    ...
    ('existing `relay` tools edited in place', 0, 0, 'tools', '### (W): NONE. corr_row.py is CARRIED as b488 guarded it and is not edited here. ### THE CLAUSE WAS CARRIED FROM b487 READING 0/0, WHERE IT WAS TRUE, AND IT CONTRADICTED THIS ACT`S OWN SEALED FACE, WHICH NAMES THE EDIT IN (W). ### The face is unchanged; the INSTRUMENT was wrong and is repaired, and both runs are banked.'),
...
def main(argv):
    print('=' * 100)
    print('b492_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b492_extract.txt'),
    ...
    spec = {"registration": ("data/b492_registration_2026-09-23.txt -- b492, "
