# -*- coding: utf-8 -*-
"""b536_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b535's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): two NEW kernel files,
### ### a conditional tag; no platform call; PLACE-papers written by append and by inserted lines.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b536_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b536_satisfiable.json')
...
CLAUSES = [
    ...
    ('bytes written at Zenodo, in any branch', 0, 0, 'bytes', '### (Z) and (R146)(2): the descriptions are not re-edited (R66).'),
    ('platform calls of any kind', 0, 0, 'calls', '### (Z): no call to the deposit platform.'),
    ('.lean files written or edited in any repository', 2, 2, 'files', '### (W): Seam.lean and AxiomCheckSeam.lean, both NEW.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): lake env lean on the new file, READING (9).'),
    ('chains run, or channel values computed', 0, 0, 'runs', '### (Z): no number computed.'),
    ('lanes opened', 1, 1, 'lanes', '### (R146): the kernel lane for this act.'),
    ('disproof lanes opened', 0, 0, 'lanes', '### section (Z): (R51) stands.'),
    ('network remotes contacted', 1, 1, 'remotes', '### READING (5): raw.githubusercontent.com and no other; the pins, the ritual`s pushes and the tag read-back excepted.'),
    ...
    ('addresses resolved or documents fetched', 2, 2, 'reads', '### READING (5): the README by at most two branch names.'),
    ('grades conferred by statement-read on this act`s compiled model theorems', 6, 0, 'grades', '### READING (8): at most the six; a halted lemma takes none.'),
    ...
    ('kernel tags made', 1, 0, 'tags', '### READING (7): v0.2, only if h2_sign_iff_rh compiles.'),
    ...
    ('ERRATA entries written', 0, 0, 'entries', '### READING (4): three LINES inserted; no entry.'),
    ...
    ('mirror zips written outside a repository', 0, 0, 'zips', '### (Z).'),
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R146).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b536_addendum.txt, under the stem glob.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('PLACE-papers documents written other than by appending', 3, 3, 'documents', '### READINGS (4) and (7): ERRATA, README, REGISTRY by inserted lines; no line deleted.'),
    ('PLACE-papers documents written at all', 5, 5, 'documents', '### (W): ERRATA, FINDINGS, README, REGISTRY, OPEN_TRAILS.'),
...
def main(argv):
    print('=' * 100)
    print('b536_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b536_extract.txt'),
    ...
    spec = {"registration": ("data/b536_registration_2026-09-25.txt -- b536, "
                             "THE SEAM, THE HOUSEKEEPING, THE PRIOR ART, UNDER (R146)"),
            "clauses": clauses}
