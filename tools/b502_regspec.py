# -*- coding: utf-8 -*-
"""b502_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b502_registration_2026-09-23.txt')
SPEC = os.path.join(ROOT, 'data', 'b502_satisfiable.json')
...
CLAUSES = [
    ...
    ('.lean files written or edited in any repository', 0, 0, 'files', '### (W): the probe `b502_printaxioms.lean` is written by the launched process into relay`s data directory, which is not a repository of Lean, and is not committed; no `.lean` file of any kernel is written.'),
    ('kernel builds', 0, 0, 'builds', '### (Z): none; the build read is b498`s and is over.'),
    ('chains run, or channel values computed', 2, 2, 'runs', '### (B)(1) and (B)(4): the ladder`s chain over the new cells with b501`s bound at each, and the Epstein control.'),
    ...
    ('lanes opened', 0, 0, 'lanes', '### (R112): the numerical lane was opened at b501; this act runs in it and SHUTS it at its close.'),
    ...
    ('candidate zeros, seeds, families or arrangements constructed', 1, 1, 'candidates', '### (B)(1): ONE family -- the ladder`s new cells, by b437`s generator on exact a.'),
    ...
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-*-b502.zip, under (R69).'),
    ...
    ('pre-face reads declared', 'PREFACE', 'PREFACE', 'reads', '### the survey`s (P1)..(Pn), MEASURED off `b502_extract.txt`.'),
    ...
    ('author rulings ratified and entered', 0, 0, 'rulings', '### (R112) was ratified and entered at b500; this act runs under it.'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b502_addendum.txt, under the stem glob (R91) permits.'),
...
def main(argv):
    print('=' * 100)
    print('b502_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b502_extract.txt'),
    ...
    spec = {"registration": ("data/b502_registration_2026-09-23.txt -- b502, "
                             "THE LADDER PAST sqrt(32), AND THE TAIL OF m(a) READ"),
            "clauses": clauses}
