# -*- coding: utf-8 -*-
"""b539_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
### ### **THE COUNTER IS IMPORTED FROM `b300_regspec.py`, NEVER COPIED.**
### ### **EVERY CLAUSE CARRIED FROM b537's SPEC WAS RE-READ AGAINST THIS FACE** (b456's error): no kernel file,
### ### no zip, no memory write; four PLACE-papers documents written by one append each.
"""
import io
import json
import os
...
REG = os.path.join(ROOT, 'data', 'b539_registration_2026-09-25.txt')
SPEC = os.path.join(ROOT, 'data', 'b539_satisfiable.json')
...
CLAUSES = [
    ...
    ('author rulings ratified and entered', 1, 1, 'rulings', '### the ferry: (R149).'),
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b539_addendum.txt, under the stem glob.'),
    ...
    ('PLACE-papers documents written at all', 4, 4, 'documents', '### (W): the map, SURR, FINDINGS, OPEN_TRAILS.'),
...
def main(argv):
    print('=' * 100)
    print('b539_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    ext = ''
    try:
        ext = io.open(os.path.join(ROOT, 'data', 'b539_extract.txt'),
    ...
    spec = {"registration": ("data/b539_registration_2026-09-25.txt -- b539, "
                             "THE CASCADE, ACT ONE, UNDER (R149)"),
            "clauses": clauses}
