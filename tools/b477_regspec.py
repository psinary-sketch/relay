# -*- coding: utf-8 -*-
"""b477_regspec.py -- THE REGISTRATION'S SATISFIABILITY SPEC. ### **COMPUTED, NOT TYPED.**
...
REG = os.path.join(ROOT, 'data', 'b477_registration_2026-09-22.txt')
SPEC = os.path.join(ROOT, 'data', 'b477_satisfiable.json')
...
CLAUSES = [
    ...
    ('kernel builds', 0, 0, 'builds', '### section (Z): none; the b475 run is not this act`s and is not polled.'),
    ('chains run, or channel values computed', 4, 4, 'runs', '### the survey`s control: cross(g,g), its autocorrelation, one off-diagonal on both chains. ### The RUN is detached and is not this act`s foreground.'),
    ('lean runs', 0, 0, 'runs', '### section (Z): none in the foreground; the detached run is (R80)`s.'),
    ('repositories cloned', 0, 0, 'repositories', '### section (Z): the comparator`s four pinned checkouts are NOT fetched.'),
    ('lanes opened', 0, 0, 'lanes', '### section (A): the kernel lane is closed and is not opened.'),
    ...
    ('mirror zips removed or rewritten', 0, 0, 'zips', '### (W): the re-sync`s 2026-09-21 zip is a different name.'),
    ('mirror zips written outside a repository', 1, 1, 'zips', '### (W): mirror-refresh-2026-09-21-b459.zip, under (R69).'),
    ...
    ('author rulings ratified and entered', 0, 0, 'rulings', '### section (A): none; b459 carries (R26)-(R69).'),
    ...
    ('addendum slots declared in the write list', 1, 1, 'slots', '### (W) and (R50): data/b477_addendum.txt.'),
    ('addendum slots carrying anything but a verbatim quotation', 0, 0, 'slots', '### (R50).'),
    ('expectations registered', 0, 0, 'expectations', '### section (J): none new; b476`s three are scored by the act that reads the log.'),
    ('expectations scored on a population other than the one named', 0, 0, 'expectations', '### section (J): (N2) on the four, not the five.'),
    ...
    ('existing `relay` tools edited in place', 0, 0, 'tools', '### (W): b476`s registration is APPENDED to below its lock block, not edited.'),
...
def main(argv):
    print('=' * 100)
    print('b477_regspec.py -- THE SATISFIABILITY SPEC. ### THE COUNTER IS IMPORTED, NOT COPIED.')
    print('=' * 100)
    ...
    spec = {"registration": ("data/b477_registration_2026-09-22.txt -- b458, "
                             "THE EIGHT MACHINE-CHECKED SENTENCES UNFOLDED"),
            "clauses": clauses}
